...
## 5 定数（Phase A プロトタイプ）を編集・保存・デフォルト復帰できる。
##
## 使い方：シーン（game_map / title_screen）で ConfigEditor.new() して add_child する。
##         F4 入力検出は各シーン側で行い、toggle() を呼ぶ。
##         または親シーンで ConfigEditor.tscn を instance 化し、F4 処理もこのクラスが
##         _unhandled_input で受け取る（表示中のみ）。
...
## 行ごとのウィジェット参照（背景色更新・デフォルトラベル更新に使用）
## row_widgets[key] = { "row": HBoxContainer, "editor": Control, "default_label": Label/ColorRect }
