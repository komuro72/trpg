...
## フロアごとの敵 PartyManager リスト（インデックス = フロアインデックス）
var _per_floor_enemies: Array = []  # Array of Array[PartyManager]
## フロアごとの NPC PartyManager リスト（インデックス = フロアインデックス）
var _per_floor_npcs: Array = []     # Array of Array[PartyManager]
...
var enemy_managers: Array[PartyManager] = []
var npc_managers: Array[PartyManager] = []
## 初期パーティーメンバー（仲間加入済み）用の一時 PartyManager リスト
var _pre_joined_npc_managers: Array[PartyManager] = []
## hero の AI 管理専用マネージャー（is_player_controlled=false 時に UnitAI が動作する）
var _hero_manager: PartyManager
...
var _dialogue_npc_manager: PartyManager  ## 現在会話中の PartyManager
...
func _setup_hero() -> void:
	...
	_hero_manager = PartyManager.new()
	_hero_manager.name = "HeroManager"
	_hero_manager.party_type = "player"
...
func _setup_floor_enemies(floor_idx: int) -> void:
	...
	# enemy_parties の各パーティーごとに別々の PartyManager を生成する
	...
	# 全 PartyManager の敵を結合したリストを各マネージャーに配布する
	...
	if floor_idx == _current_floor_index:
		...
		for em_v2: Variant in fems:
			enemy_managers.append(em_v2 as PartyManager)
...
## player_parties[0] の 2番目以降のメンバーを PartyManager として配置し、
...
## 初期配置の仲間 PartyManager をプレイヤーパーティーに即時合流させる
## _setup_dialogue_system() 後に呼ぶこと
func _merge_pre_joined_allies() -> void:
	for nm: PartyManager in _pre_joined_npc_managers:
...
func _on_dialogue_requested(nm: PartyManager, npc_initiates: bool) -> void:
...
func _merge_player_into_npc_party(nm: PartyManager) -> void:
...
## NPC パーティーのリーダー名を返す
func _get_npc_party_leader_name(nm: PartyManager) -> String:
	for member: Character in nm.get_members():
...
func _connect_npc_combat_signals(nm: PartyManager) -> void:
	for member: Character in nm.get_members():
		if not is_instance_valid(member):
			continue
...
func _check_fought_together(nm: PartyManager, npc_member: Character, other: Character) -> void:
...
## 対象キャラが所属する PartyManager を探し、has_been_healed フラグをセットする
...
func _on_enemy_party_wiped(items: Array, room_id: String, floor_idx: int) -> void:
	...
	for item_v: Variant in items:
		...
		placed += 1
	if placed > 0:
		print("[DBG_DROP] _floor_items id=%d F%d items=%d total_keys=%s" % [
			_floor_items.get_instance_id(), floor_idx, placed, str(_floor_items.keys())])
		queue_redraw()
...
func _check_npc_member_stairs() -> void:
	...
	for pair: Variant in leader_transitions:
		...
		_transition_npc_floor(arr[0] as PartyManager, arr[1] as int)
	# 非リーダーメンバーの個別遷移
	for pair: Variant in member_transitions:
		var arr := pair as Array
		_transition_single_npc_member(arr[0] as PartyManager, arr[1] as Character, arr[2] as int)
...
## PartyManager は全メンバーが遷移完了するまで旧フロアリストに残る。
func _transition_npc_floor(nm: PartyManager, direction: int) -> void:
	...
	# リーダーの新フロアの敵・全メンバーリストを PartyManager に設定（リーダーAI の認識を更新）
	var new_enemies: Array[Character] = []
	var fl_all_members: Array[Character] = []
	for em: PartyManager in (_per_floor_enemies[new_floor] as Array):
		if is_instance_valid(em):
			new_enemies.append_array(em.get_enemies())
			fl_all_members.append_array(em.get_enemies())
	for nm2: PartyManager in (_per_floor_npcs[new_floor] as Array):
	...
	# PartyManager は全メンバー遷移後に旧→新フロアリストへ移動するため、ここでは更新しない
	_update_character_visibility()
	queue_redraw()


## 非リーダーの NPC メンバーを個別に新フロアへ遷移させる。
## 全メンバー遷移完了時に PartyManager の管理フロアを更新する。
func _transition_single_npc_member(nm: PartyManager, member: Character, direction: int) -> void:
	...
	for m: Character in nm.get_members():
		if is_instance_valid(m) and m.current_floor != new_floor:
			...
			break
	if all_on_new_floor:
		# 全員揃ったので PartyManager の管理フロアを更新する
		nm.set_map_data(new_map)
...
func _update_character_visibility() -> void:
	...
	# NPC：PartyManager の管理フロア（fi）ではなく各キャラクターの current_floor で判定する
	# リーダーが先行遷移中は PartyManager が旧フロアリストに残るため fi での判定が不正確になる
	for fi: int in range(_per_floor_npcs.size()):
		for nm: PartyManager in (_per_floor_npcs[fi] as Array):
			if is_instance_valid(nm):
				for ch: Character in nm.get_members():
					if is_instance_valid(ch):
