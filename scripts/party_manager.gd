...
var _room_id:        String = ""            ## このパーティーが属する部屋のエリアID（縄張り部屋）
var _per_member_last_area: Dictionary = {}  ## エリア変化検知用 { Character -> area_id: String }・敵パーティーのみ更新
...
## プレイヤーパーティーと合流済み NPC パーティー両方に設定する
var _party_ref: Party = null
func set_party_ref(party: Party) -> void:
	_party_ref = party
	if _leader_ai != null:
		_leader_ai.set_party_ref(party)
...
## 毎フレーム処理：
## 1. activation gate（VisionSystem 未使用時のフォールバック）
## 2. 敵パーティー：メンバーのエリア変化検知 → 制圧判定（縄張り部屋から離脱を捕捉）
##    判定タイミング = `_on_member_died`（既存）+ エリア離脱（本ポーリング）
##    ポーリング粒度は毎フレーム（O(members) 軽量・新規エリア変化通知機構を増やさない選択）
func _process(_delta: float) -> void:
	# activation gate
	if not _activated and _player != null and not _members.is_empty() and not _vision_controlled:
		for member: Character in _members:
	...
	# 敵パーティーのみ：エリア変化検知 → 制圧判定
	if party_type == "enemy" and not _room_id.is_empty() and _map_data != null:
		var any_changed := false
		for member: Character in _members:
			if not is_instance_valid(member):
				continue
			var cur_area := _map_data.get_area(member.grid_pos)
			var prev_area: String = _per_member_last_area.get(member, "") as String
			if cur_area != prev_area:
				_per_member_last_area[member] = cur_area
				any_changed = true
		if any_changed:
...
func _on_member_died(character: Character) -> void:
	...
	_per_member_last_area.erase(character)
	# リーダーが死亡した場合は再選出
...
## 部屋制圧判定：全メンバーが「死亡 or 個体 FLEE で部屋外」なら party_wiped を発火する
## 呼び出しタイミング：
##   - メンバー死亡時（`_on_member_died` 経由）
##   - メンバーがエリア変化したとき（`_process` のポーリング検知経由）
##
## 制圧条件（2026-04-27 改訂：パーティー戦略 → 個体 FLEE ベース）：
##   - 死亡 → 制圧対象 OK
##   - 生存・縄張り部屋（_room_id）の中 → 制圧未完了（return）
##   - 生存・部屋の外 + UnitAI._strategy == 1（FLEE） → 制圧対象 OK
##   - 生存・部屋の外 + それ以外（ATTACK/WAIT/FALL_BACK） → 制圧未完了（return）
##     ※ FALL_BACK は戦闘継続のため離脱扱いにしない
##     ※ プレイヤーが誘導して部屋外に出した（ATTACK のまま追跡中）もカウントしない
##
## 【敵パーティー専用】呼び出し元が `party_type == "enemy"` でガード済み。
func _check_room_suppression() -> void:
	...
	for member: Character in _members:
		if not is_instance_valid(member):
			continue
		# 生存かつ縄張り部屋の中にいる → 制圧未完了
		var member_area := _map_data.get_area(member.grid_pos)
		if member_area == _room_id:
			return
		# 部屋の外にいる場合：個体 FLEE（_strategy == 1）なら離脱扱い、それ以外は制圧未完了
		var unit_ai: UnitAI = null
		if _leader_ai != null:
			unit_ai = _leader_ai.get_unit_ai(member.name)
		if unit_ai == null or unit_ai._strategy != 1:
			return  # ATTACK / WAIT / FALL_BACK で部屋外 → 制圧対象にしない
	# 全メンバーが「死亡 or 個体 FLEE 離脱」 → 制圧完了
