...
##
## 2026-04-28 改訂：`explore` を OrderWindow から削除（5 値 → 4 値）。
##   `explore` は層 2（リーダー個別行動）専用の値となり、`stairs_*` / `guard_room` と同じ
##   位置づけ。層 1 ベースライン（_global_orders.move）には含めない。
##   旧仕様（5 値）では「全員バラバラ探索」の意味で使われていたが、
##   非リーダーが分散探索する隊形崩壊の温床だったため概念分離した。
const GLOBAL_MOVE:        Array[String] = ["follow", "same_room", "cluster", "standby"]
## 後方互換エイリアス（旧 GLOBAL_COMBAT → GLOBAL_MOVE）
const GLOBAL_COMBAT:      Array[String] = ["follow", "same_room", "cluster", "standby"]
