...
## パーティー FLEE 時にリーダーが決定して `_assign_orders()` 経由で配布する。
## Vector2i(-1, -1) = 推奨なし（個体 FLEE / パーティー FLEE でない）。
## メンバーは `_find_flee_goal()` でこの出口を軽いバイアスで優先するが、脅威コストが
## 高ければ別出口を選ぶ柔軟性を持つ。
var _flee_recommended_goal: Vector2i = Vector2i(-1, -1)

## 2026-04-24 深夜：fall_back 用の個別推奨集合点（`_formation_fallback_goal`）は廃止。
## fall_back は FLEE の `_flee_recommended_goal` を再利用する方式に刷新された。
## メンバー層では `_find_fall_back_goal()` が `_find_flee_goal()` を内部で呼び出し、
## 違いは `_is_far_enough_from_threats()` による早期停止判定のみ。

## 前回処理時に自分がいたエリア ID（エリア変化検知用）
## `_process()` 内で現在エリアと比較し、変化したらリーダーに通知する（FLEE 時の推奨出口再計算）
var _last_area_id: String = ""
...
func receive_order(order: Dictionary) -> void:
	...
	_flee_recommended_goal = order.get("flee_recommended_goal", Vector2i(-1, -1)) as Vector2i
...
func _process(delta: float) -> void:
	...
	# エリア変化検知（2026-04-21 ステップ 3 追加）：FLEE 時の推奨出口再計算のためリーダーに通知
	_notify_area_change_if_needed()
...
## 逃走先を決定する（2026-04-21 ステップ 3 改訂：二段階ロジック）
##
## パーティー FLEE 時：リーダーが決めた推奨出口 `_flee_recommended_goal` を軽いバイアスで優先
## 個体 FLEE 時：推奨情報なし（Vector2i(-1, -1)）で自律判断
##
## アルゴリズム：
##   現エリアの全「内側出口タイル」を列挙 → 各出口への脅威コスト付き A* 経路コスト計算
##   → 推奨出口と異なる場合は FLEE_NON_RECOMMENDED_PENALTY を加算 → 最小コストの出口を採用
...
func _find_flee_goal(threat) -> Vector2i:
	...
	var threat_fn: Callable = Callable(self, "_calc_threat_cost")
	for exit_tile: Vector2i in exit_tiles:
		...
		var bias: float = 0.0
		if recommended != Vector2i(-1, -1) and exit_tile != recommended:
			bias = GlobalConstants.FLEE_NON_RECOMMENDED_PENALTY
		var total: float = reach_cost + bias
		if total < best_cost:
			best_cost = total
			best_exit = exit_tile
...
## fall_back 目標タイルを返す（2026-04-24 深夜・FLEE 再利用方式）
##
## FLEE と同じ出口評価を流用する軽量ラッパー。違いは停止条件だけ：
##   - 最近の脅威との距離が `attack_range + FALL_BACK_MARGIN` 以上なら現在位置を返す（停止）
##   - そうでなければ `_find_flee_goal()` に委譲（現エリアの最善出口を返す）
##
## 敵メンバー・異常ケースは legacy にフォールバック。
##
## 設計根拠：fall_back は「戦闘継続しつつ射程外まで下がる」意図なので、FLEE の
## 「避難先エリアまで完全離脱」とは停止条件のみが異なる。FLEE の推奨出口・出口評価を
## そのまま共用することで計算コストを最小化できる（前の候補タイル×A* 方式は ~560ms
## かかっていたが、FLEE と同じ 2〜6 出口 × A* なら数ms）。
func _find_fall_back_goal(threat) -> Vector2i:
	...
	if _is_far_enough_from_threats():
		return _member.grid_pos
	# まだ近いので FLEE と同じ出口評価に委譲
	return _find_flee_goal(safe_threat)
...
## エリア変化を検知してパーティーリーダーに通知する（2026-04-21 ステップ 3 追加）
## FLEE 時の推奨出口はメンバーが出口を跨ぐと陳腐化するため、リーダーに再評価を促す。
## PartyLeader 側で `FLEE_REEVAL_MIN_INTERVAL` のクールダウンが効くので連続発火しない。
## 初回（_last_area_id が空）は通知せず現エリア記録のみ（起動時の偽通知を防ぐ）
func _notify_area_change_if_needed() -> void:
	if _map_data == null or _member == null or not is_instance_valid(_member):
		return
	var current_area: String = _map_data.get_area(_member.grid_pos)
	if current_area == _last_area_id:
		return
	var prev_area: String = _last_area_id
	_last_area_id = current_area
	# 初回記録 or 空エリア（タイル外）は通知不要
	if prev_area.is_empty() or current_area.is_empty():
		return
	# パーティーリーダーに通知（存在すれば）
	var pl: PartyLeader = get_parent() as PartyLeader
	if pl != null and is_instance_valid(pl):
		pl.on_member_area_changed(_member, current_area)
