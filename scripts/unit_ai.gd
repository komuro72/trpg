...
## 避難先エリア ID（2026-04-25 改訂：リーダー推奨機構廃止後の唯一のリーダー → メンバー伝達経路）
## パーティーリーダーが現在の戦況・自分の位置から最も近い避難先エリア（フロア 0 = 安全部屋 /
## フロア 1 以降 = 上り階段エリア）を選定し、`_assign_orders()` 経由で全メンバーに配布。
## 各メンバーはこの値と自分の位置から `_evaluate_exit_costs()` で出口コストを総合評価する。
## "" = 避難先未決定（敵不在 / マップデータ不在 等）。
var _flee_refuge_area_id: String = ""

## メンバー自身が直近に選定した FLEE / fall_back 出口タイル（PartyStatusWindow 表示用）
## `_find_flee_goal()` / `_find_fall_back_goal()` の戻り値で更新する。Vector2i(-1, -1) = 未選定。
## デバッグ表示専用：実際のキューには各 flee / fall_back アクションがこの座標を goal として持つ。
var _last_flee_goal: Vector2i = Vector2i(-1, -1)
...
func receive_order(order: Dictionary) -> void:
	...
	_flee_refuge_area_id = order.get("flee_refuge_area_id", "") as String
	...
	match action.get("action", "") as String:
		...
		"flee", "fall_back", "keep_distance":
			...
			if not is_instance_valid(_target):
				_target = null
			var act_name: String = action.get("action", "") as String
			var goal: Vector2i
...
func _generate_queue(strategy: int, target: Character) -> Array:
	...
	if strategy == 1:
		# 味方（2026-04-21 ステップ 3 改訂）：
		#   避難先エリア（フロア 0 = 安全部屋 / フロア 1 以降 = 上り階段）に既にいれば待機
		#   そうでなければ flee アクションを積み、`_find_flee_goal` の新ロジックで
		#   脅威コスト付き A* + 推奨バイアスで出口選択 → 段階的に避難先まで移動する
		if _member.is_friendly:
			if _is_in_refuge_area():
				return [{"action": "wait"}]
			var q_ally: Array = []
			for _i: int in range(5):
				q_ally.append({"action": "flee"})
			return q_ally
		# 敵は従来通り脅威から逃走（縄張り帰還は _apply_range_check で処理・新ロジックは別タスク）
	...
	# 2026-04-24 深夜改訂：FLEE と同じ出口評価を共用し、停止条件だけ「射程外判定」に差別化。
	# `_find_fall_back_goal()` が _find_flee_goal() を内部で呼ぶラッパー + 射程外で早期停止。
	if strategy == _STRATEGY_FALL_BACK:
		# 既に射程外まで下がっていれば待機（FLEE の _is_in_refuge_area() と対称）
		if _member != null and is_instance_valid(_member) and _member.is_friendly \
				and _is_far_enough_from_threats():
			return [{"action": "wait"}]
		if target == null or not is_instance_valid(target):
			return [{"action": "wait"}]
		var q: Array = []
	...
	# WAIT: 移動方針に従って行動
	# 撤退指示中（_combat=="flee"）で避難先到達済みなら、move_policy に関わらず待機する。
	# 2026-04-25 改訂：cluster/follow/same_room の通常 WAIT ロジックは、リーダーが
...
## 自分が避難先エリア（フロア 0 = 安全部屋 / フロア 1 以降 = 上り階段エリア）にいるか判定
## 2026-04-21 ステップ 3 追加：味方 FLEE で避難先到達後の待機判定に使用
func _is_in_refuge_area() -> bool:
	if _map_data == null or _member == null or not is_instance_valid(_member):
		return false
	var current_area: String = _map_data.get_area(_member.grid_pos)
	if current_area.is_empty():
		return false
	var refuge_areas: Array[String] = _map_data.get_refuge_area_ids(_member.current_floor)
	return refuge_areas.has(current_area)


## 味方の撤退先を返す（フロア0=安全部屋・フロア1以降=最寄りの上り階段）
## 撤退先が見つからない場合は Vector2i(-1, -1) を返す
## 2026-04-21 ステップ 3 以降は `_find_flee_goal()` の新ロジックが味方 FLEE で使われるため、
## 本関数は廃止候補（直接呼び出し箇所なし）。arg が見つかれば削除してよい
...
## 出口総合コスト評価（FLEE / fall_back / 将来の keep_distance 共通コア・2026-04-25 改訂）
##
## 自分の現エリアの全外側出口タイルを取得し、各出口について以下を計算する：
##   reach_cost       = メンバー位置 → exit_tile の脅威コスト付き A* 距離（_path_cost）
##   refuge_dist_cost = 出口が属するエリアから _flee_refuge_area_id までの BFS 距離
##                      × FLEE_AREA_DISTANCE_WEIGHT
##                      避難先未設定なら 0・到達不能なら大ペナルティ（999.0）
##   total_cost       = reach_cost + refuge_dist_cost
##
## 戻り値の各要素：
##   {exit_tile: Vector2i, total_cost: float, reach_cost: float, refuge_dist_cost: float}
## 経路がない・現エリア不明・MapData 不在の場合は空配列（呼出側でフォールバック処理）。
##
## 設計意図：
##   - 脅威がない場合は `reach_cost` がほぼ等しくなり、`refuge_dist_cost` が支配的
##     → 全メンバーが避難先方向の同じ出口に収束（パーティー分散防止）
##   - 脅威がある場合は `reach_cost` で経路上の敵を避ける別出口を選べる柔軟性
##   - リーダー位置は不要：各メンバーが自分の現エリアから自律判断
func _evaluate_exit_costs() -> Array[Dictionary]:
	...
	if exit_tiles.is_empty():
		return result
	# 脅威コストを事前計算マップに展開して A* の cost_fn を Dictionary 参照に簡略化（パフォーマンス）
	var threat_map: Dictionary = _build_threat_map()
	var threat_fn: Callable = func(tile: Vector2i) -> float:
		return threat_map.get(tile, 0.0)
	var refuge_id: String = _flee_refuge_area_id
	for exit_tile: Vector2i in exit_tiles:
		var path: Array[Vector2i] = _astar_with_cost(
				_member.grid_pos, exit_tile, threat_fn, current_area)
		if path.is_empty() and exit_tile != _member.grid_pos:
			continue
		var reach_cost: float = _path_cost(path, threat_fn)
		var refuge_dist_cost: float = 0.0
		if not refuge_id.is_empty():
			# 外側出口なので、その座標自身が向こう側エリアのタイル
			var exit_area: String = _map_data.get_area(exit_tile)
			if exit_area.is_empty():
				refuge_dist_cost = 999.0
			else:
				var d: int = _map_data.get_area_distance(exit_area, refuge_id)
				if d < 0:
					refuge_dist_cost = 999.0
				else:
					refuge_dist_cost = float(d) * GlobalConstants.FLEE_AREA_DISTANCE_WEIGHT
		result.append({
			"exit_tile":        exit_tile,
			"total_cost":       reach_cost + refuge_dist_cost,
			"reach_cost":       reach_cost,
			"refuge_dist_cost": refuge_dist_cost,
		})
	return result


## 候補配列から total_cost 最小の exit_tile を選ぶ（FLEE / fall_back 共通）
## 候補空 or 異常ケースは Vector2i(-1, -1) を返す（legacy フォールバック用シグナル）
func _pick_min_cost_exit(candidates: Array[Dictionary]) -> Vector2i:
	var best: Vector2i = Vector2i(-1, -1)
	var best_cost: float = INF
	for c: Dictionary in candidates:
		var cost: float = c.get("total_cost", INF) as float
		if cost < best_cost:
			best_cost = cost
			best = c.get("exit_tile", Vector2i(-1, -1)) as Vector2i
	return best


## 逃走先を決定する（2026-04-25 改訂：リーダー推奨機構廃止・自律判断のみ）
##
## メンバーが `_evaluate_exit_costs()` で全出口の総合コストを計算し、最小値を採用する。
## リーダーは `_flee_refuge_area_id` のみ配布し、出口の選定はメンバー側で完結する。
##
## 敵メンバー・現エリア不明・到達可能な出口なし → 旧実装（`_find_flee_goal_legacy`）に
## フォールバック（敵の FLEE 実装は派生タスクで対応予定）
##
...
func _find_flee_goal(threat) -> Vector2i:
	...
	# 味方のみ新ロジック。敵は legacy（派生タスク）
	if _member == null or not is_instance_valid(_member) or _map_data == null \
			or not _member.is_friendly:
		return _find_flee_goal_legacy(safe_threat)
	# 避難先エリアに到達済みなら停止（現在位置を返す）
	# 旧キューに残った flee アクションが消化された後の境界往復を防ぐ。
	# `_generate_queue(FLEE)` 側の wait 分岐はキュー生成時しか効かないため、
	# 既存キュー消化中のフレームではここで早期停止する必要がある。
	if _is_in_refuge_area():
		_last_flee_goal = _member.grid_pos
		return _member.grid_pos
	# 脅威から十分離れているなら前進不要（移動先 = 現在地）
	# 2026-04-25 改訂：これは flee 状態の解除ではなく「flee 中の移動先決定」の問題として
	# 実装する。flee 状態自体は維持され、敵が再接近すれば即座に再出口選択が走る。
	# 解除は HP critical 解除（個体 FLEE）/ battle_policy 切替（パーティー FLEE）に委ねる。
	var nearest_threat_dist: int = _calc_nearest_threat_distance()
	if nearest_threat_dist >= GlobalConstants.FLEE_SAFE_DISTANCE:
		_last_flee_goal = _member.grid_pos
		return _member.grid_pos
	var candidates: Array[Dictionary] = _evaluate_exit_costs()
	if candidates.is_empty():
		return _find_flee_goal_legacy(safe_threat)
	var goal: Vector2i = _pick_min_cost_exit(candidates)
	if goal == Vector2i(-1, -1):
		return _find_flee_goal_legacy(safe_threat)
	_last_flee_goal = goal
	return goal


## 自分の現在位置から最近接の脅威（同フロア・対立陣営・生存）までのマンハッタン距離を返す。
## 脅威が存在しない場合は INT_MAX 相当の大きな値（`_find_flee_goal()` 側で前進不要扱いになる）。
##
## 用途：`_find_flee_goal()` の前段判定（`FLEE_SAFE_DISTANCE` との比較）。
## `_calc_threat_cost()`（出口評価時の per-tile コスト）/ `_build_threat_map()`（事前計算）/
## `_is_far_enough_from_threats()`（fall_back 停止判定）とは用途が異なるため独立に保つ。
##
...
func _calc_nearest_threat_distance() -> int:
	...
	for mv: Variant in _all_members:
		...
		if other == null or other == _member or other.hp <= 0:
			continue
		if other.is_friendly == _member.is_friendly:
			continue
		if other.current_floor != _member.current_floor:
			continue
		var d: int = _manhattan(_member.grid_pos, other.grid_pos)
		if d < best_dist:
			best_dist = d
	return best_dist


## fall_back 目標タイルを返す（2026-04-25 改訂：FLEE と同じ出口評価コアを共用）
##
## 違いは停止条件のみ：
##   - 最近の脅威との距離が `attack_range + FALL_BACK_MARGIN` 以上なら現在位置を返す（停止）
##   - そうでなければ `_evaluate_exit_costs()` で最小コスト出口を採用
##
## 敵メンバー・異常ケースは legacy にフォールバック。
func _find_fall_back_goal(threat) -> Vector2i:
	var safe_threat: Character = threat if is_instance_valid(threat) else null
	if _member == null or not is_instance_valid(_member):
		return _find_flee_goal_legacy(safe_threat)
	# 敵は legacy（スコープ外・FLEE 派生課題 1 と合流予定）
...
##   - `_calc_threat_cost(tile)`：タイルから各敵への距離を計算して総和（per-tile reactive）
##   - `_build_threat_map()`：各敵から影響範囲タイルに貢献を加算（precompute）
##
## 数学的に等価（加算順序の入れ替えのみ）。数値結果は完全一致。
## A* 展開の各ステップで Dictionary 参照のみで済むため、150 体パーティーでも軽量。
##
## 計算量：~150 体 × 影響範囲 ~41 タイル = ~6,150 ops（~2 ms）
func _build_threat_map() -> Dictionary:
...
## fall_back の停止判定：最近の脅威（同フロア・対立陣営・生存）とのマンハッタン距離が
## `attack_range + FALL_BACK_MARGIN` 以上なら true。
## `_all_members` 150 体を反復するが早期 continue が効く（~150μs）。
## 敵ステータス直接参照禁止ルール準拠（位置と is_friendly / current_floor / hp のみ参照）
func _is_far_enough_from_threats() -> bool:
	if _member == null or not is_instance_valid(_member):
		return true  # 異常ケースは停止扱いで安全終了
	var threshold: int = _member.attack_range + GlobalConstants.FALL_BACK_MARGIN
	for mv: Variant in _all_members:
		if not is_instance_valid(mv):
			continue
		var other: Character = mv as Character
		...
		var d: int = _manhattan(_member.grid_pos, other.grid_pos)
		if d < threshold:
			return false  # 射程 + MARGIN 内に脅威あり → 継続
	return true


## 経路上の合計コスト（1 ステップあたり 1.0 + 脅威コスト）を計算する
## path は「start を含まない・goal で終わる」座標配列（_astar_with_cost の戻り値）
func _path_cost(path: Array[Vector2i], threat_fn: Callable) -> float:
	var total: float = 0.0
	for tile: Vector2i in path:
		var extra: float = float(threat_fn.call(tile))
		total += 1.0 + extra
	return total


## 指定タイルに入る際の「脅威コスト」を計算する（敵からの距離ベース）
## 自分と対立する陣営（is_friendly != _member.is_friendly）の生存キャラで、同フロアの全員を対象
## 各敵について max(0, FLEE_THREAT_RANGE - manhattan) × FLEE_THREAT_WEIGHT を合計
##
## 注：敵ステータス直接参照禁止ルール準拠（位置と is_friendly のみ参照）
func _calc_threat_cost(tile: Vector2i) -> float:
	if _member == null or not is_instance_valid(_member):
		return 0.0
	var cost: float = 0.0
	var range_tiles: int = GlobalConstants.FLEE_THREAT_RANGE
	...
	for mv: Variant in _all_members:
		...
		if other.current_floor != _member.current_floor:
			continue
		var d: int = _manhattan(tile, other.grid_pos)
		if d >= range_tiles:
			continue
		cost += float(range_tiles - d) * weight
	return cost


## 旧「脅威から 5 タイル離れる」フォールバック実装（2026-04-21 改名）
## 孤立エリア・出口なしマップ・敵側 FLEE で使用
func _find_flee_goal_legacy(threat: Character) -> Vector2i:
...
## `_astar()` と同じ経路探索ロジックだが、タイル移動コストに cost_fn 戻り値を加算する。
## area_limit_id が非空なら、そのエリア外のタイルは探索から除外する（FLEE 用の現エリア内完結経路）
##
## cost_fn: Callable(tile: Vector2i) -> float
##   タイルに入る際の追加コスト（1.0 は既定・脅威距離に応じて増減）
##
## 既存の `_astar()` は非破壊で保持し、FLEE 経路探索のみが本メソッドを使う
func _astar_with_cost(start: Vector2i, goal: Vector2i,
		cost_fn: Callable, area_limit_id: String = "") -> Array[Vector2i]:
	if start == goal:
		return []
	if _map_data == null:
		return []
	# エリア制限：start がエリア外なら諦める
	if not area_limit_id.is_empty() and _map_data.get_area(start) != area_limit_id \
			and _map_data.get_area(goal) != area_limit_id:
		# start も goal も当該エリア外なら意味がない（念のためガード）
		return []
	var open_set:  Array[Vector2i] = [start]
	var came_from: Dictionary      = {}
	var g_score:   Dictionary      = {start: 0.0}
	var f_score:   Dictionary      = {start: float(_manhattan(start, goal))}
	var max_iter   := 400
	var iter       := 0
	while not open_set.is_empty() and iter < max_iter:
		iter += 1
		var current := open_set[0] as Vector2i
		for p: Vector2i in open_set:
			if float(f_score.get(p, 1e9)) < float(f_score.get(current, 1e9)):
				current = p
		if current == goal:
			var path: Array[Vector2i] = []
			var c := current
		...
		for offset: Vector2i in [Vector2i(1, 0), Vector2i(-1, 0), Vector2i(0, 1), Vector2i(0, -1)]:
			...
			if not _is_walkable_for_self(neighbor):
				continue
			# エリア制限：制限エリア外への進入を禁止（ただしゴール自体は許可）
			if not area_limit_id.is_empty() and neighbor != goal \
					and _map_data.get_area(neighbor) != area_limit_id:
				continue
			# 階段タイルはゴール以外では中間経由地点として使わない
			if _is_stair_tile(neighbor) and neighbor != goal \
					and _move_policy != "stairs_down" and _move_policy != "stairs_up":
			...
			if cost_fn.is_valid():
				step_cost += float(cost_fn.call(neighbor))
			var tentative_g: float = float(g_score.get(current, 1e9)) + step_cost
			if tentative_g < float(g_score.get(neighbor, 1e9)):
				came_from[neighbor] = current
				g_score[neighbor]   = tentative_g
				f_score[neighbor]   = tentative_g + float(_manhattan(neighbor, goal))
				if neighbor not in open_set:
					open_set.append(neighbor)
	return []
...
func _determine_effective_action() -> int:
	...
	# 5. combat=flee（撤退指示）：避難先到達まで FLEE 継続（SAFE より優先）
	#
	# 2026-04-25 改訂：プレイヤーが明示的に battle_policy="retreat" を指示した場合や
	# 個別に combat="flee" を設定した場合、その意図は「敵の有無に関係なく避難先まで
	# 退却する」こと。SAFE 検知で WAIT に切り替わると、敵が視界外に消えた瞬間に
	# 撤退が中断し、敵が再接近すると再開する振動が発生する（実プレイ報告あり）。
	# 避難先到達済みなら自然に WAIT に落ちる（_is_in_refuge_area 経由）。
	if _combat == "flee":
		if _should_ignore_flee():
			return 0  # 逃げない種族（DarkKnight 等）は flee 指示を無視 → ATTACK
		if _is_in_refuge_area():
			return 2  # 避難先到達済み → WAIT で停止
		return 1  # FLEE 継続

	# 6. combat_situation が SAFE なら移動方針に従う（WAIT 相当）
	if _is_combat_safe():
		return 2  # WAIT

	# 7. 戦闘中: combat 方針に従う（"flee" は上で処理済み）
	match _combat:
		"attack", "aggressive":
			return 0  # ATTACK
