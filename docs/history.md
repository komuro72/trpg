...
## 2026-04-26（リーダー `target = same_as_leader` 自己参照解消・実装と表示の一致化）

### 経緯

CLAUDE.md「残タスク（🔥 最優先）」の `tgt=リーダーと同じ` の挙動確認を起点に、まず**コード調査（実装変更なし）** でリーダー本人に `target = same_as_leader` が配布された場合の挙動を確認。続いて修正を実装した。

### 調査結果（修正前）

[`scripts/party_leader.gd`](../scripts/party_leader.gd) `_assign_orders()` の旧実装：
...
- `leader_target` は **policy lookup を経由せず** `_select_target_for()` を直叩きで算出していた
- そのため自己再帰は構造上発生せず、リーダー本人の `target = same_as_leader` は事実上 `nearest` 相当として silently 処理されていた
- ただし以下の未定義経路を抱えていた：
  - (a) リーダー個別の OrderWindow `target` 値（weakest / support 等）が無視される
  - (b) `_party_members[0]` ≠ 実リーダーの場合（リーダー死亡昇格・adopt_member 後）に `leader_target` が別人基準になる
- コメント上の意図（"same_as_leader ポリシー用"）は他メンバー用の事前計算が主目的で、リーダー本人の自己参照を意識した実装ではなかった

### 修正内容

「リーダー個別判断で上書きする」4/26 per-member `_move_policy` 統一の思想と対称な構造を採用。

1. **基底フック新設**（[`scripts/party_leader.gd`](../scripts/party_leader.gd)）：`_decide_leader_target_policy_override() -> String`（既定 ""）
2. **サブクラスで override**：
   - `PartyLeaderPlayer._decide_leader_target_policy_override()` → `"nearest"`
   - `PartyLeaderAI._decide_leader_target_policy_override()` → `"nearest"`（NPC・敵基底に適用）
   - 種族固有 AI（`EnemyLeaderAI` 派生）は必要に応じて再 override 可能（4/26 `_decide_leader_move_override()` と同じ拡張パターン）
3. **`_assign_orders()` の構造変更**：
   - `leader_char` 確定を `leader_target` 算出より前に移動
   - フック戻り値で `leader_tgt_policy` を確定 → match 文で `leader_target` を実リーダー基準で算出
   - per-member ループでリーダー本人の `tgt_policy` を `leader_tgt_policy` で上書き
4. **`leader_target` の参照点修正**：旧 `_party_members[0]` → 新 `leader_char`（`is_leader` フラグ参照）。前回調査で指摘した不整合 (b) も同時解消

### 表示と runtime の乖離問題

修正後、ランタイム挙動は正しく `nearest` になるが、**PartyStatusWindow の `指示:T:` は `current_order.target` フィールドを直読み**するため `same_as_leader` のまま表示される問題が発覚（実機 snapshot で観察）。

#### 二段階の追加修正

1. **OrderWindow の sync 経路の leader 除外**（[`scripts/order_window.gd`](../scripts/order_window.gd)）：
   - `_sync_all_global_to_members()` / `_sync_global_to_members()` で **`target` キーのみリーダーを sync 対象外**にした
   - これがないと、ゲーム起動時に `Party.global_orders.target = "same_as_leader"` がリーダーの `current_order.target` に上書きされ、preset が即塗り潰される
2. **`PartyManager._update_leader_flags()` でリーダー preset を集中管理**（[`scripts/party_manager.gd`](../scripts/party_manager.gd)）：
   - リーダー昇格時に `current_order["target"] = "nearest"` をセット
   - 全パーティー種別（player / NPC / 敵）共通の経路で動作
   - リーダー交代時にも自動的に新リーダーへ伝播
   - 当初は `game_map.gd` の hero スポーン箇所のみで preset を入れていたが、NPC リーダーが漏れていた（snapshot で全 13 NPC リーダーが `T:リーダーと同じ` のままだった）。`_update_leader_flags()` への集中化で全リーダー一律に対応
3. **hero スポーン箇所の preset**（[`scripts/game_map.gd`](../scripts/game_map.gd)）：`_update_leader_flags()` への集中化後も冗長性として保持（OrderWindow 起動時の二重保険）

### 設計判断のまとめ

| 項目 | 採用案 | 理由 |
|---|---|---|
| 上書きの条件 | 無条件で `nearest`（leader が自分で設定した値も無効化） | spec の literal 解釈に従う・同種の `_move_policy` 上書きパターンに一致 |
| 種族 AI による再 override | 余地を残す | 将来「ボス専用 AI が weakest を選ぶ」等のケースに備える |
| `_party_members[0]` 経由 vs `is_leader` 経由 | `is_leader` 経由 | リーダー死亡昇格 / adopt_member 後の不整合を構造的に排除 |
| 合流時の `current_order` 保持 | 引き継ぎ（書き換えない） | 個別カスタマイズの保護 + 合流コードの単純化 |
| 表示の一致策 | preset を `_update_leader_flags()` で集中管理 + OrderWindow sync で leader 除外 | runtime 挙動と display が一致し、OrderWindow / PartyStatusWindow の表示が自然になる |

### 動作確認

実機 snapshot（F7）で確認：

- プレイヤーリーダー（hero）：`指示:T:最近傍`
- NPC リーダー全員：`指示:T:最近傍`
- 非リーダー：`指示:T:リーダーと同じ`（spec 通り）
- ヘッダー `tgt=リーダーと同じ`：`_global_orders.target` の値（パーティー全体方針＝非リーダー継承用）として変わらず

### スコープ外

- **`move = follow` のリーダー自己参照**：作業 2 で並行調査。修正は今回スコープ外（後述の調査結果セクション参照）

---

## 2026-04-26（`move = follow` リーダー自己参照の現状確認・調査のみ）

### 経緯

リーダーの `target = same_as_leader` 自己参照を解消する作業の延長で、**`move = follow` 側でも同種の自己参照経路があるか**を調査。修正は今回見送り、将来の対称化判断材料として記録。

### 結論

**自己参照は発生する**（hero リーダーのみ）。**ただし UnitAI 側の防御コードで安全に処理されている**。実機で問題は起きないが、リーダーが「立ち止まって近隣の敵だけ攻撃する」という退化挙動になる。

### 調査結果

#### 1. リーダー本人が `_move_policy = "follow"` を継承する経路

存在する。4/26 の 3 層構造で：

[`scripts/party_leader.gd`](../scripts/party_leader.gd) `_assign_orders()` の層 2 上書き：
...
層 2 のリーダー上書きが空振りすると、層 1 のベースライン（`_global_orders.move`）が残る。空振り条件：

| パーティー種別 | 空振り条件 |
|---|---|
| プレイヤー | 常に空振り（`PartyLeaderPlayer._decide_leader_move_override()` 未 override・味方は explore/guard_room モードに入らない） |
| NPC | 敵検知中は `_is_in_explore_mode()` が false → 空振り |
| 敵 | `_decide_leader_move_override()` の `guard_room` 判定で空振りすると（縄張り外で chase 圏外でない時）。`_global_orders.move` は `EnemyLeaderAI._build_orders_part()` で `"follow"` 固定 |

→ ベースライン `"follow"` がリーダーに継承される経路は全パーティー種別で常時存在。プレイヤーパーティーでは戦闘中でも常時、NPC では敵検知中、敵では追跡中に発火する。

#### 2. リーダー本人の `_move_policy = "follow"` 時の `_leader_ref` 値

[`scripts/party_leader.gd`](../scripts/party_leader.gd) の `formation_ref` 構築ロジック：
...
| パーティー種別 | リーダー本人の `formation_ref`（= UnitAI の `_leader_ref`） |
|---|---|
| **プレイヤー（hero リーダー）** | `_player = hero == self` → **完全な自己参照** |
| プレイヤー（hero 以外がリーダーになった場合） | hero が `joined_to_player` だが leader_char ≠ _player → `formation_ref = _player`（hero 自身に follow する非自己参照状態） |
| NPC リーダー | `is_friendly = true` だが `leader_char != _player` かつ `joined_to_player = false` → `formation_ref = null`（自己参照なし・「リーダーなし」状態） |
| 敵リーダー | `is_friendly = false` → 全 if スキップ → `formation_ref = null` |

→ **問題の自己参照は実質的に hero リーダーのみ**（hero が AI 操作になった瞬間に発火）。

#### 3. 自己参照時の UnitAI の挙動（防御コードあり）

[`scripts/unit_ai.gd`](../scripts/unit_ai.gd) の formation 関連メソッド全てに `or _leader_ref == _member` の防御チェックが反復されている：

- `_formation_satisfied()` line 1964-1979：自己参照なら `true` を返す（formation 移動なし）
- `_formation_move_goal()` line 2071-2075：自己参照なら現在地を返す（移動目標 = 自分）
- フロア間追従 line 729-734：`_leader_ref != _member` 条件で除外（フロア追従発火しない）
- `_target_in_formation_zone()` line 2015-2019：自己参照チェックなしだが、結果として「自分から 4 タイル以内の敵を攻撃」と意味解釈でき無害

→ クラッシュ・無限ループは発生しない。リーダーは「その場に留まり、隣接敵を攻撃する」退化挙動。

#### 4. 設計意図の有無

`unit_ai.gd` の line 1966 / 1981 / 1986 / 2073 / 2097 の 5 箇所で `or _leader_ref == _member` の防御チェックが**反復されている**。コメントによる説明は無いが、これだけ系統的に並んでいる以上、意図的な防御コードとみて間違いない。

ただし：

- 設計意図のドキュメント化はされていない
- 「safety net で耐える」だけで、退化挙動が望ましいかは別問題

#### 5. 未定義経路としての潜在問題

| シナリオ | 観察される挙動 |
|---|---|
| プレイヤーが hero 以外を操作中、hero が AI 操作になる | hero が `_move_policy = "follow"` + `_leader_ref = self` で立ち止まる。隣接敵には反応するが、能動的に追撃・隊形再構成しない |
| hero が階段の前で停止し、操作中の他メンバーが下のフロアに行く | hero は階段間追従が発火しない（`_leader_ref != _member` 条件で除外）→ **前フロアに取り残される可能性**（要実機検証） |
| プレイヤーが OrderWindow で leader の `move` を `cluster` / `same_room` に変更 | `cluster` / `same_room` も同じ防御パターン（line 1981-1988）が入っているので同様に立ち止まる |

### 将来の対称化判断材料

`target = same_as_leader` と同じ「リーダー個別判断で上書き」パターンで `move = follow` も解消する場合、以下の選択肢がある：

- **案 A**：`_decide_leader_move_policy_self_ref_override()` 等の専用フック新設・自己参照時のフォールバック値（`explore` / `standby` 等）を返す
- **案 B**：`formation_ref` 構築ロジックで自己参照を許可しない（hero リーダー時に `formation_ref = null` を返す）
- **案 C**：現状維持（防御コードを正式仕様として明文化する）

`target` 側との対称性を重視するなら案 A、シンプルさ重視なら案 B、影響範囲最小なら案 C。判断は実機で「シナリオ 2: hero がフロア追従しない」が起きるかどうかの検証結果に依存する。
...
## 階段着地ロジックの一連の修正（修正 X 3 段階）（2026-04-28 後半）

修正 X 関連の実プレイ起点で 3 つの段階的修正が連続して入った。各段階の発見と判断を時系列で記録する。

### 1. 修正 X：階段着地ロジック統一・占有時は遷移見送り・横出し廃止

#### 背景

従来 4 経路（hero / パーティーメンバー / NPC リーダー / NPC 非リーダー）で着地ロジックが別実装になっており、挙動も非対称：

| 経路 | 着地点 | 占有チェック |
|------|--------|------------|
| hero | `spawns[0]` 直接 | なし（重なり許容） |
| メンバー | 階段マス + `_find_free_adjacent_to()` | あり（横出し） |
| NPC リーダー | 旧階段から最近距離 | なし |
| NPC 非リーダー | 旧階段から最近距離 + `_find_free_adjacent_to()` | あり（横出し） |

実プレイで「フロア遷移後、対応する階段マスにキャラがいると `_find_free_adjacent_to()` で隣接マスへフォールバック → そこに別キャラがいると重なる」現象を観測。

#### 修正

全 4 経路で着地ロジックを以下に統一：

1. 着地点は対応する階段マス（`spawns[0]` または同等の取得経路）
2. 対応階段マスが占有されていたら遷移を見送る（return せず・次フレームで再チェック）
3. 横出しロジック（`_find_free_adjacent_to()` 経由）を廃止
4. フォールバック（`base_spawn` / `center` 直接着地）も廃止
5. 占有が解消されたら次フレームの再チェックで自然に遷移発火（既存の毎フレーム再チェック機構を利用）

#### 実装

- 共通の占有判定ヘルパ `_is_landing_tile_occupied(tile, floor_idx, exclude_char)` を [game_map.gd](../scripts/game_map.gd) に新設
  - `_per_floor_enemies` / `_per_floor_npcs` を全フロア走査・`current_floor == floor_idx` でフィルタ
  - 自分以外の任意のキャラ（プレイヤーパーティー / 敵 / NPC）が同フロア・同タイルにいれば true
- 4 経路の `_transition_*` 関数の冒頭で本ヘルパを呼び、占有時は `return`（クールダウンも設定しない）
- 旧 `_find_free_adjacent_to()`（BFS で隣接非階段マスを探索する横出し関数）は他用途なし（grep 確認済み）のため物理削除

#### 重要な設計判断

- **3 ガード（`_stair_cooldown` / `stair_just_transitioned` / `is_moving()`）は触らない**：占有チェックはこれら 3 ガードを通過した後・実際の `_transition_*()` 直前に挿入
- **占有時はクールダウンを設定しない**：次フレームで即再チェックされる挙動を維持
- **永久ブロック対策（タイムアウト等）は今回見送り**：副作用観察後に判断する方針で残タスクへ

### 2. 上り階段部屋の敵初期配置を撤去

#### 発見

修正 X 導入後の動作確認で「NPC が地下二階に降りられない」現象を観測。プレイヤーで同じ階段を下りて確認したところ、F1 の階段マスに敵が立っていることが判明。

調査の結果：

- [docs/spec.md:4139](spec.md) で旧 20 部屋時代から「**上り階段部屋は敵初期配置なし**」が規約として明文化されていた
- しかし 4/28 後半の Floor 1〜3 の 30 部屋拡張時に**規約違反のチェックが漏れた**
- 検査スクリプトで F1〜F3 の 9 部屋（17 体の敵）が違反状態と判明

| フロア | 上り階段部屋 | 敵数 |
|---|---|---|
| F1 | r1_3 / r1_11 / r1_30 | 3 / 3 / 3 |
| F2 | r2_3 / r2_11 / r2_30 | 2 / 2 / 2 |
| F3 | r3_3 / r3_15 / r3_11 | 1 / 1 / 2 |

#### 修正

- `dungeon_handcrafted.json` の該当 9 部屋から `enemy_party` キーを物理削除
- 17 体の敵は**再分配せず単純減**（ユーザー指示）
- CLAUDE.md「マップデータ仕様」配下に「階段部屋の配置規約」セクションを新設し、規約と検証用 Python ワンライナーを明文化（過去に明文化されていなかった本ルールを CLAUDE.md にも昇格）

#### 設計意図の再確認

「上り階段部屋には敵を初期配置しない」のは、修正 X の「占有時は遷移見送り」と組み合わさったとき、初期配置の敵が階段マス周辺にいるだけで上階からの遷移が永久ブロックされる事故を構造的に防ぐため。下り階段部屋は適正フロアサイクル成立のため引き続き敵配置を許可する非対称仕様とする。

### 3. 階段着地ロジックを「ペア座標一致」に統一

#### 発見

修正 X + 上り階段の敵削除を行った後の検証で、ユーザーから「プレイヤーだと階段の上に敵がいても降りられた（敵と重なった）」との報告。NPC は降りられないのにプレイヤーは降りられる非対称が発生。

原因の特定：着地先選択ロジックが 4 経路で非対称だった：

| 経路 | 着地先の決定方法 |
|------|--------|
| hero | `spawns[0]` 固定（`find_stairs()` 走査順最初の stairs_up） |
| パーティーメンバー | `stair_tiles[0]` 固定（同上） |
| NPC リーダー | 旧階段から最近距離の新階段 |
| NPC 非リーダー | 旧階段から最近距離の新階段 |

つまり F1 に複数の上り階段があるとき、hero は常に `spawns[0]`（左上スキャン順最初）に着地し、NPC は自分が踏んだ階段に最も近い新階段に着地する。敵が片方の階段マスに立っていると、NPC のみがブロックされる現象が発生。

#### 重要な前提の確認

ユーザーから「もともと階段ペアは同じグローバル座標を持つ前提で考えていた」との指示。JSON データを検査して確認：

- F0〜F3 の階段はすべて同じ 5 座標 `(6,23) / (28,5) / (28,50) / (50,23) / (50,50)` に配置されており、上下階で「同座標 = 階段ペア」の前提が成立
- F4 のみマップサイズが異なるため別座標。F3 ↔ F4 の境界は (28,5) のみ一致し、残り 2 つは座標不整合（暫定運用）

#### 修正

- 共通ヘルパ `_find_paired_stair_pos(taken_pos, new_map, spawn_type)` を [game_map.gd](../scripts/game_map.gd) に新設
  - 踏んだ階段マスと**同座標**の新フロア階段マスがあればそれを返す
  - 同座標がなければ `spawns[0]`（フォールバック・F3 ↔ F4 等の暫定対応）
  - 該当種別の階段がなければ `taken_pos` そのまま（最終フォールバック）
- 4 経路すべてで本ヘルパを使用：
  - hero: `active_char.grid_pos` を踏んだ座標として渡す
  - パーティーメンバー: `ch.grid_pos`
  - NPC リーダー: `leader_ch.grid_pos`
  - NPC 非リーダー: `member.grid_pos`
- 旧「最近距離」ロジック（NPC 経路のみ）は完全に削除
- CLAUDE.md「マップデータ仕様」配下に「階段ペアの座標規約」セクションを新設（規約として明文化）

#### 期待される挙動

- F0 で `r0_30` の階段を踏んだら、hero / NPC ともに F1 の `r1_30` 階段マス（同座標）に着地
- 敵が `r1_30` の階段マスに立っていれば 4 経路すべてで占有ブロック発火 → 全員一貫して降りられない
- 「同じ階段で行き来している」UX が成立し、修正 X の占有ブロックも全経路で対称的に動作する

### このセッションでの想定外の発見

1. **着地先選択ロジックが 4 経路で非対称だった**
   - 修正 X 実装前の調査では着地点取得関数の差異までは把握していたが、「同じ階段を使っても hero と NPC で違う場所に着地する」UX 上の問題までは認識していなかった
   - ユーザーの「同じ階段で行き来する前提」という指摘で初めて顕在化
2. **マップ拡張時の規約違反チェック漏れ**
   - 「上り階段部屋に敵を配置しない」規約は spec.md にあったが CLAUDE.md には独立規約として書かれておらず、4/28 後半の F1〜3 拡張で見落とされた
   - 同種の漏れを防ぐため、CLAUDE.md「マップデータ仕様」配下に検証用 Python ワンライナー付きで規約を昇格
3. **F3 ↔ F4 の階段座標不整合**
   - F3 stairs_down (28,50) / (50,50) と F4 stairs_up (6,5) / (17,14) / (28,5) はほぼ座標が一致しない
   - 現状は `spawns[0]` フォールバックで動作するが、ボス戦のフロア間挙動に違和感が出れば対応する方針で残置

### 次セッションへの引継ぎ

- **副作用観察**：修正 X 導入後の永久ブロック問題（CLAUDE.md 残タスク「階段マス占有による永久ブロック問題」）。実プレイで頻度を観察し、必要なら対策（タイムアウト機構・階段マスを goal 候補から除外・占有元への強制移動指示・敵フロア間移動の設計）
- **F3 ↔ F4 の座標不整合**：実プレイで違和感が出れば F4 のマップサイズを F3 と揃える等の対応
- **NPC が階段を素通りする現象の調査**（既存残タスク）：今回の修正でも直接解消しておらず、別問題として残置

### 採用しなかった案

- **修正 X で goal 除外（階段マスを goal 候補から除外）も同時実装**：実装規模が大きく、NPC リーダーの `stairs_*` モードと整合させる例外処理が必要なため、今回スコープ外。永久ブロック観察後に判断する方針
- **着地先選択を JSON で明示的に階段ペア ID を持たせる方式**：「同座標 = ペア」の暗黙規約で十分シンプルに表現できることが判明したため、JSON スキーマ拡張は不要と判断
- **F3 ↔ F4 の座標不整合を今セッションで解決**：マップ再構成は別タスクとして切り分け、コード側はフォールバックで暫定対応
