...
**HP/MP/SP ゲージの表示スケール（2026-04-22 変更）**：
- `HP_REF = MP_REF = SP_REF = 100.0`（絶対値 100 スケール）
- 値が 100 でバー全幅・50 で半分・25 で 1/4 と、キャラの `max_hp` / `max_energy` に関わらず一定基準で描画
- 旧実装（キャラ個別の max 値に対する割合表示）から変更。狙いは (1) HP 50 のキャラが「半分しかない」と誤認される不安感の解消、(2) キャラ個性（max HP の高低）の可視化
- 適用範囲は**左パネルのみ**。内部データ（`Character.hp` / `max_hp` / `energy` / `max_energy`）の意味・計算は従来どおり。ダメージ計算・戦況判断には影響しない
- **HP 上限 100 キャップは意図的に実装していない**（描画レイヤーのみの変更）
...
- **NpcLeaderAI の承諾/拒否判断（スコア比較方式）**:
  ...
  - **ランク集計値**: `GlobalConstants.RANK_BASE_OFFSET + CharacterGenerator.RANK_VALUE[rank]`（C=+0, B=+1, A=+2, S=+3 → C=3, B=4, A=5, S=6）
  - `RANK_BASE_OFFSET`（既定 3）は ConfigEditor PartyLeader カテゴリで編集可能
  - 乗数・ボーナスは `NpcLeaderAI` の定数（`RANK_SCORE_PER_RANK / FOUGHT_TOGETHER_BONUS / HEALED_BONUS`）で管理
...
|-------------|-----------|------|
| melee       | 0.2       | 近接攻撃（剣士・斧戦士・斥候） |
| ranged      | 0.1       | 遠距離物理（弓使い） |
| dive        | 0.2       | 降下攻撃（ハーピー） |
| magic       | 0.1       | 遠距離魔法（魔法使い系・goblin-mage 等） |

**設計思想（2026-04-22 調整）**：近距離グループ（melee / dive）と遠距離グループ（ranged / magic）の比率を **2 : 1** に設定。旧値（1.5 : 1 相当）では遠距離が「安全なのに火力も高い」状態でリスクとリワードの対応が崩れていたため、倍率差を広げて「リスクを取る見返り」を明確化した。絶対値（0.2 / 0.1）は実プレイで即死問題がほぼ解消した値。
...
| 項目 | 内容 |
|------|------|
| ランク和スコア | 全メンバーの集計用 rank 値の**合計**。各メンバー = `RANK_BASE_OFFSET + RANK_VALUE[rank]`（C=3, B=4, A=5, S=6） |
...
```
1. ランク和 = Σ(RANK_BASE_OFFSET + RANK_VALUE[member.rank])  ← C=3, B=4, A=5, S=6
...
```
戦力値 = ランク和 × HP充足率
ランク和 = Σ(GlobalConstants.RANK_BASE_OFFSET + CharacterGenerator.RANK_VALUE[member.rank])  ← C=3, B=4, A=5, S=6
...
### ランク集計値の定数化
旧 `RANK_VALUES` 辞書（`{C:3, B:4, A:5, S:6}`）は廃止。
ステータス生成側の `CharacterGenerator.RANK_VALUE`（`{C:0, B:1, A:2, S:3}`）と
`GlobalConstants.RANK_BASE_OFFSET`（既定 3・ConfigEditor PartyLeader カテゴリ）の
加算で表現する方式に統一（戦力計算と NPC 合流交渉の両方で使用）。
