...
### 特殊攻撃の発動ロジック
`_generate_special_attack_queue(target)` を UnitAI に追加:
- `_should_use_special_skill()` で `special_skill` 指示と `power_balance` / `hp_status` を照合
- クラスごとの使用条件（`SPECIAL_ATTACK_MIN_ADJACENT_ENEMIES = 2`、隣接判定は8マス）:
  - fighter-sword: 隣接敵2体以上 かつ `_can_rush_slash_through()` が true（前方最大2マスに敵＋着地可能マス）
  - fighter-axe: 隣接敵2体以上
  - scout: 隣接敵2体以上（包囲脱出兼ダメージ）
  - archer / magician-water: ターゲットがいれば使用
  - magician-fire: 隣接敵2体以上（炎陣）
  - healer: `_generate_buff_queue()` 内で `_should_use_special_skill()` を参照
- `_count_adjacent_enemies()`: 8方向（斜め含む）の敵カウント
- `_can_rush_slash_through()`: facing方向の pos1/pos2 に敵がいて、pos2（または pos1）が空きFLOORなら true
...
- アイテム名統一: HPポーション / MPポーション / SPポーション（上級ポーション廃止）
...
- 部屋名：「魔王の玉座の間」
- ドロップ：魔王の大剣（power28/block_right_front20）、深淵の鎧（phys28/mag14）、HPポーション×3
...
### 独立した閾値（変更なし）
- `NEAR_DEATH_THRESHOLD = 0.25`: HPポーション自動使用・heal "aggressive" モード対象選定で使用。状態ラベルとは別用途
...
## アイテム名称統一（2026-04）
- HPポーション / MPポーション / SPポーション（`HP回復ポーション` 等の長い表記は廃止）
