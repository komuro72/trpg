...
### 装備の補正値
- **武器**：`power`・`skill` を補正（物理系武器は物理威力・物理技量、魔法系武器は魔法威力・魔法技量に対応）
- **防具（鎧・服・ローブ）**：physical_resistance・magic_resistance を補正
- **盾**：physical_resistance・magic_resistance を補正
- 補正がかからないもの：defense_accuracy（防御技量）・move_speed・leadership・obedience・max_hp・max_mp・防御強度（block_right_front / block_left_front / block_front）
