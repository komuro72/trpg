...
#### keep_distance との関係（2026-04-25 専用ロジックに分離）
...
実装：ターゲットからのマンハッタン距離が `[KEEP_DISTANCE_MIN, KEEP_DISTANCE_MAX]` の同心円帯から候補タイルを列挙し、`threat_cost + reach_cost × KEEP_DISTANCE_REACH_WEIGHT` 最小のタイルを選ぶ。候補なしなら現在地に留まる。
...
- **rear 隊形の動作**（2026-04-25 改訂）：理想範囲内 + 射程内なら attack、それ以外は keep_distance。「後衛前進問題」（敵接近時に下がるロジックなし）を解消。近接クラスを rear 指定すると理想範囲内でも攻撃できない（仕様・プレイヤー責任）
