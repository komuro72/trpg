...
| C/Xホールド：消耗品選択モード | C | X | ホールド中にLB/RBで消耗品を循環選択、リリースで使用（「なし」選択時は不使用） |
...
- [ ] Phase 10: アイテム・装備システム
  ...
  - [x] Phase 10-3: 消耗品の使用（Phase 12-5 で操作体系を変更）
    - HP回復ポーション・MP回復ポーション
    - C/Xホールド：消耗品選択モード。前回選択位置からスタート（初回は「なし」枠）
    - ホールド中のLB/RB：消耗品リスト（先頭に「なし」枠）を前/次に循環
    - C/Xリリース：「なし」以外が選択中なら使用。「なし」なら何もしない。選択位置を保持
    - 固定スロット管理なし。inventory内の消耗品（category=="consumable"）を循環選択
    - 使用条件：HPポーション→HP満タンでない、MPポーション→MP満タンでない
    - 使用後：inventoryから削除。MessageLogにシステムメッセージ
    - **消耗品バー UI**（ConsumableBar.gd）：画面上部・部屋名ラベルの左側に常時表示
      - 消耗品を種類ごとにアイコン（カラーブロック）＋「×個数」で横並び表示
      - 選択中の種類は白い枠でハイライト
      - 選択モード中（C/Xホールド）は先頭に「—」（なし）枠を追加表示
...
- [ ] Phase 12: ステージ・バランス調整
  ...
  - [x] Phase 12-6: 防御バフのバリアエフェクト実装
    ...
    - **C/Xリリース**：「なし」以外が選択中なら使用。選択位置を `_last_consumable_index` として保持
...
- inventory内の消耗品（category=="consumable"）を選択して使用（固定スロット管理なし）
- C/Xホールド：消耗品選択モード。LB/RBで「なし」枠込みの循環選択。C/Xリリースで使用
- LB/RB（通常時）：パーティーメンバーを表示順で循環切り替え
- LB/RB（攻撃ホールド時）：ターゲット循環
