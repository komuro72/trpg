...
| AIデバッグパネル ON/OFF | F1 | — |
| シーン再スタート | F5 | — |
| OrderWindow 名前列サブメニュー | Z（名前列上） | 同左 |
...
- 名前列で Z を押すとサブメニュー表示：操作切替 / 装備 / アイテム受渡
  - 操作切替：そのキャラを操作キャラにする（常時有効）
  - 装備：所持アイテム一覧 → 選んで Z で装備（常時有効、誰を操作中でも全メンバー変更可）
  - アイテム受渡：渡すアイテム選択 → 渡す相手選択（リーダー操作中のみ）
...
- [ ] Phase 10: アイテム・装備システム
  ...
  - [ ] Phase 10-2: 装備システム
    ...
    - OrderWindow 改修
      - 操作列を廃止 → 名前列サブメニューに統合
        - サブメニュー項目：操作切替 / 装備 / アイテム受渡
        - 装備：所持アイテム一覧から選んで Z で装備（誰を操作中でも全メンバー変更可）
        - アイテム受渡：リーダー操作中のみ（渡すアイテム選択 → 渡す相手選択）
      - ステータス表示を2列化（縦幅を縮小）
      - テーブルの列構成：名前 / 移動 / 隊形 / 戦闘 / ターゲット / 低HP / アイテム取得
