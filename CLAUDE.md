...
- **bonus / tier 概念分離**：`ITEM_BONUS_*_RATIO`（強さ）と `tier`（格付け整数）を分離。tier=0 は初期装備専用（4/19）
...
##### 戦力判断における装備（tier_sum）の扱い確認（次セッション最初に実施）

4/27 セッション末の議論で残された確認タスク。Claude Code に実装調査を依頼し、結果を claude.ai 側で議論する流れ。実装変更の前に**まず仕様確認**。

戦力計算式：`strength = (rank_sum + tier_sum × ITEM_TIER_STRENGTH_WEIGHT) × HP率`

確認したい点：
- `tier_sum` はパーティー全員の装備の tier 合計か（メンバーごとの集計方法）
- 装備していない（未装備・インベントリ持ち）アイテムも `tier_sum` に含まれるか
- 2 ステータス装備の tier は `ITEM_TIER_POLICY = "max"` で算出されるが、戦力計算でもその tier を使うか
- 初期装備（tier=0）は実質的に戦力に寄与しないか
- フロア毎の tier 分布（深い階で高 tier が出やすい設計）と戦力評価の整合性

確認結果を踏まえて議論：
- 装備の扱いが設計意図と合っているか
- 「ポーション収集 → 再挑戦」サイクルでの装備強化が戦力評価に反映されるか
- NPC が装備を拾って tier_sum が上がる → 適正フロア判定が下がる、というサイクルが成立しているか
...
- 装備強化による戦力底上げが実際に観察できるか（`F(R+T)s` 表示で確認）
- 4/26 改修で「敵が部屋に留まる」「NPC が敵を連れ回さない」が解消されているか
- 気になる挙動があれば随時議論項目に追加

調整候補：`POTION_DROP_MIN/MAX_PER_PARTY`・`POTION_DROP_HEAL_RATIO`・`ITEM_TIER_STRENGTH_WEIGHT`・`FLOOR_*_RANK_THRESHOLD`・`NPC_FLOOR_DOWNGRADE_HP_RATIO`。
...
##### NPC 自動装備の比較基準を bonus 段階合計に変更

現状 `NpcLeaderAI._auto_equip_members()` は `_item_stats_sum()`（item.stats 辞書の値を単純合計）で装備優劣を判定している。`power+30` と `block_front+30` を等価扱いし、ステータス種類による重み付けがない（4/27 調査で確認）。問題例：
- 「power+10/skill+10」と「power+15/skill+5」が両方 sum=20 で同価値判定
- magician-fire にとって `skill+30` のローブと `magic_resistance+30` のローブの価値差が見えない
- 補正値合計だと max 値の差が混入してフラットな強さ指標にならない（armor_robe の `magic_resistance` max=30 と armor_cloth max=15 等）

代替案：bonus 段階（low/mid/high → 1/2/3）の合計で比較する方式に変更。事前生成セットの `tier` フィールドが既に「装備の格付け」として機能するので、これを比較に使う方が妥当。

claude.ai 議論論点：
- bonus 段階の合計で十分か、クラス適性も考慮すべきか
- 「強い装備を拾ったら確認ウィンドウを出す」仕様（次項目）との連動方法
- 自動装備のタイミング（現状 2 秒ごと）はそのままで良いか
