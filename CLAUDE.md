...
| 攻撃（ホールド・攻撃タイプ自動判定） | Z | A（Xbox） |
| メニュー戻る / フィールド予約 | X | B（Xbox） |
...
- [x] Phase 4: 攻撃バリエーション
  - 攻撃スロット
    - Z/A：攻撃（攻撃タイプはクラスのスロット定義から自動判定。melee=近接/ranged=遠距離）
    - X/B：当面メニュー戻る（フィールドでは未使用・将来の防御・回避用に予約）
    - C/V：空（将来用）
...
- [ ] Phase 10: アイテム・装備システム
  ...
  - [ ] Phase 10-2: 装備システム
    ...
    - ドロップ処理（部屋制圧方式）
      ...
      - アイテムが部屋の床タイルにランダム散らばり（1マス1個、1回きり）
      - 同じマスに移動で自動取得（拾ったキャラ個人の inventory へ）
      - プレイヤー操作キャラはフィルタなし
      - AIキャラは item_pickup 指示に従う
    - item_pickup 指示を指示システムに追加（6項目目）
      - aggressive / passive / avoid の3値
      - 全体方針プリセットにも反映
    - AI自動装備は将来実装（当面は拾って持つだけ）
    - OrderWindow 改修
      - 操作列を廃止 → 名前列サブメニューに統合
        - サブメニュー項目：操作切替 / アイテム
        - アイテム：未装備品一覧 → Z でアクションメニュー（装備する / 渡す）
          - 装備する：誰を操作中でも全メンバー変更可。旧装備は未装備品として手元に残る
          - 渡す：リーダー操作中のみ。渡す相手をメンバー一覧から選択
      - ステータス表示を2列化（縦幅を縮小）
      - テーブルの列構成：名前 / 移動 / 隊形 / 戦闘 / ターゲット / 低HP / アイテム取得
      - 最下行に「ログ」行を追加（「閉じる」行を置き換え）
      - メッセージログ機能
        - MessageWindow が内部で最新50件のログを配列保持
        - メンバー行選択時：下部にステータス表示（従来通り）
        - ログ行選択時：下部にログ表示に切り替わる
        - ログ行で Z を押すとスクロールモード（上下キーでスクロール）
        - Esc または Z でスクロールモードを抜けてテーブルに戻る
        - ログ閲覧中もゲーム進行はポーズしない
  - [ ] Phase 10-3: 消耗品の使用
    - HP回復ポーション・MP回復ポーション
...
- 方向判定ロジック：防御側の向き（facing）を基準に、攻撃者の相対位置から atan2 で角度を計算し4象限（90°ずつ）で判定
  ...
  - 近接攻撃・遠距離攻撃で共通のロジックを使用（隣接4マスの近接攻撃では結果的に4方向の単純判定と一致）
