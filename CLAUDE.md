...
代替案：bonus 段階（low/mid/high → 1/2/3）の合計で比較する方式に変更。事前生成セットの `tier` フィールドが既に「装備の格付け」として機能するので、これを比較に使う方が妥当。後日「装備取得時の確認ウィンドウ仕様」と連動して設計する。
