...
##### `mv=` と `move:` の不整合調査（4/25 NPC 階段ナビ修正後の派生）

PartyStatusWindow / F7 スナップショットでヘッダー行 `mv=追従` に対し、非リーダーの個別行が `move:密集` になるケースが観察された：
...
本来 `_global_orders.move == "follow"` なら非リーダーも `_move_policy = "follow"` のはず。考えられる原因：
- NPC ベースラインの `_global_orders.move` が `cluster` で、ヘッダー `mv=追従` 表示が別フィールド由来の可能性
- `_assign_orders()` の `_is_in_explore_mode()` 分岐で非リーダーを `cluster` 固定化している箇所の影響（探索モード中は強制 cluster）
- ヘッダー表示ロジック自体の不具合

調査 → 原因特定 → 修正の順で対応。NPC 階段ナビ修正（4/25）と並行して残った疑問。
...
- **敵**：`[種別]  生存:x/y  area:<id>  戦況:<ENUM>(...) 戦力:<ENUM> ...  strategy=<ENUM>`
  ...
  - 敵の行動は UnitAI 指示フィールドではなく、パーティー戦略 + 種族フック + ハードコード種族ロジックで決まる
...
3. **移動方針プレフィックス**（詳細度 >= 1・黄緑・**味方メンバーのみ**）：`  move:cluster`
   - 全体指示だが UnitAI / PartyLeader._assign_orders で per-member に書き換えられうる唯一の項目（explore/guard_room モードでリーダー→`explore` / 非リーダー→`cluster` / 階段ナビ中はリーダー→`stairs_down/up` / 非リーダー→`cluster` に per-member 差し替え）のため、個別指示グループとは別枠で先頭に表示する
   - メンバー本人の `_move_policy`（UnitAI 側 per-member 値）を直読みしているため、リーダーは `move:↓階段` / 非リーダーは `move:密集` のように行ごとに別の値が表示される
   - **`_global_orders.move`（パーティー全体指示）の値域は OrderWindow の 5 値（`follow / cluster / same_room / standby / explore`）に限定**。`stairs_down / stairs_up` は per-member の `_move_policy` のみで使われ、`_global_orders.move` には書き込まない（ヘッダー行の `mv=` に階段値が出るのは規約違反）
