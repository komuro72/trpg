...
| 消耗品スロット循環選択 | 未定 | LB / RB | LBで前、RBで次（ターゲット選択中以外） |
...
- [ ] Phase 10: アイテム・装備システム
  ...
  - [x] Phase 10-3: 消耗品の使用
    - HP回復ポーション・MP回復ポーション
    - C/X：消耗品スロットで選択中のアイテムを使用
    - LB/RB：消耗品スロット循環選択（LBで前、RBで次。ターゲット選択中はターゲット循環に使用）
    ...
    - **消耗品バー UI**（ConsumableBar.gd）：画面上部・部屋名ラベルの左側に常時表示
      ...
      - 選択中の種類は白い枠でハイライト
      - 操作キャラ切替・アイテム取得・LB/RB押下・使用後に自動更新
      - 消耗品が0個の場合は非表示（描画なし）
...
- inventory内の消耗品（category=="consumable"）を循環選択して使用（固定スロット管理なし）
- C/Xボタン：選択中の消耗品を使用
- LB/RB：消耗品を前/次に循環選択（ターゲット選択中はターゲット循環に使用）
