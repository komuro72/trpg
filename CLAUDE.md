...
- **NPC 階段ナビの `_global_orders.move` 汚染廃止**：旧実装は NpcLeaderAI の `_build_orders_part()` が `hint["move"]` に `stairs_down` / `stairs_up`、`hint["target_floor"]` を書き込んでいたため、PartyStatusWindow ヘッダーに OrderWindow 定義外の `mv=↓階段(F1)` が表示される規約違反だった（`_global_orders.move` の想定値域は OrderWindow の 5 値 `follow / cluster / same_room / standby / explore` のみ）。`_assign_orders()` 側にはリーダー → `stairs_*` / 非リーダー → `cluster` の per-member 配布ロジックが既に存在していたため、内部の階段ナビ動作はそのまま機能していた（規約と実装が乖離）。修正は `_build_orders_part()` で `stairs_*` と `target_floor` の hint 書き込みを廃止するだけで完結（`explore` は OrderWindow 定義値の範囲内なので維持）。プレイヤーパーティーとの対称性が回復（リーダー 1 人が階段ナビ・他は `_global_orders.move` 由来の追従系で動く）。バトルポリシー自動切替（`_global_orders["battle_policy"] = "retreat"`）は OrderWindow 定義値内なので問題なし（4/25）
...
- 装備強化による戦力底上げが見えるか（`F(R+B)s` 表示で確認）
...
##### `_global_orders.move` への `explore` 書き込み

NpcLeaderAI が探索モード時に `_global_orders.move = "explore"` を書き込んでいる箇所が残っている（4/25 NPC 階段ナビ修正でスコープ外とした）。`explore` は OrderWindow 定義値ではあるが、書き込み経路自体が `stairs_*` と同種の「NpcLeaderAI が `_global_orders` を上書きする」設計問題を抱えている。`stairs_*` と同じく per-member 配布化するか検討。
...
OrderWindow の `move` 値「リーダーと同じ部屋に行く」（`same_room`）の挙動は実用度が低い。廃止して OrderWindow 4 値（`follow / cluster / standby / explore`）にスリム化する案を検討。

廃止する場合の影響範囲：
- `_global_orders.move` の値域（OrderWindow 4 値に縮小）
...
- **味方（プレイヤー・NPC）**：`[種別]  生存:x/y  area:<id>  戦況:<ENUM>(自軍×自軍HP率 / 敵×敵HP率 = 比) 戦力:<ENUM> F(R+T)s C(R+T)s E(R+T)s  mv=... battle=... tgt=... hp=... item=...`
  - 例：`戦況:EVEN(9.0×1.00 / 9.0×0.85 = 1.18) 戦力:EVEN F(...) C(...) E(...)`
  - `戦況:` / `戦力:` の値は CombatSituation / PowerBalance enum 名そのまま（2026-04-25 に日本語化を廃止し enum 名表示に統一）
  - 戦況の括弧内：`<自軍戦力 base>×<自軍 HP 率> / <敵戦力 base>×<敵 HP 率> = <戦況比>` の形式で計算過程を完全可視化
  - HP 独立表示（旧 `HP:満(...)`）は廃止（2026-04-25・`HpStatus` enum 廃止に伴う）
  - `mv / battle / tgt / hp / item` は `_global_orders` の実値（OrderWindow 正規表記の日本語）
  - NPC は `item=` を含む（プレイヤー経由の item_pickup 指示）。敵は `item=` なし（item_pickup 指示を持たないため）
- **敵**：`[種別]  生存:x/y  area:<id>  戦況:<ENUM>(...) 戦力:<ENUM> ...  strategy=<ENUM>`
  ...
  - `strategy=` は `PartyLeader._party_strategy` の素値（`ATTACK` / `FLEE` / `WAIT` / `DEFEND` / `EXPLORE` / `GUARD_ROOM`）。enum 名のまま表示
...
3. **移動方針プレフィックス**（詳細度 >= 1・黄緑・**敵味方共通**）：`  move:cluster`
   ...
   - メンバー本人の `_move_policy`（UnitAI 側 per-member 値）を直読みしているため、リーダーは `move:↓階段` / 非リーダーは `move:追従`（`_global_orders.move` のベースライン値・NPC は `"follow"` 既定）のように行ごとに別の値が表示される
...
`follow / cluster / same_room / standby / explore`（OrderWindow 5 値に限定）
...
OrderWindow 5 値 ＋ `stairs_down / stairs_up / guard_room`
