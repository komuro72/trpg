...
### 装備の補正値
- **武器**：`power`・`skill` を補正（物理系武器は物理威力・物理技量、魔法系武器は魔法威力・魔法技量に対応）。加えて `block_power`（武器防御強度）も持つ
- **防具（鎧・服・ローブ）**：physical_resistance・magic_resistance を補正
- **盾**：physical_resistance・magic_resistance を補正。加えて `block_power`（盾防御強度）も持つ
- 補正がかからないもの：defense_accuracy（防御技量）・move_speed・leadership・obedience・max_hp・max_mp

### ダメージ計算への装備補正反映
- 物理威力／魔法威力 = キャラ素値 + 武器 power
- 物理技量／魔法技量 = キャラ素値 + 武器 skill
- 物理耐性  = キャラ素値 + 防具 physical_resistance + 盾 physical_resistance
- 魔法耐性  = キャラ素値 + 防具 magic_resistance + 盾 magic_resistance
- 武器防御強度 = 武器 block_power（盾なしクラスは武器のみ）
- 盾防御強度  = 盾 block_power（戦士クラスのみ）
- OrderWindow のステータス表示は素値・補正値・最終値の3列（例：物理威力 25 +5 → 30）
- block_power は OrderWindow で「武器防御強度」「盾防御強度」として武器・盾を別行で表示
...
| 防御強度 | （装備側） | 防御成功時に無効化できるダメージ量。武器・盾に付くパラメータ。OrderWindowで「武器防御強度」「盾防御強度」として別行表示 |
...
- クリティカル時: ダメージを power × 2.0 倍で計算（通常の2倍威力）
- エフェクト・SE は既存アセット流用（HitEffect + hit_physical/hit_magic SE）

**被ダメージ計算フロー**（着弾後）:
1. **クリティカル判定**（skill からクリティカル率を算出。クリティカル時は power × 2.0 倍でダメージ計算）
2. **防御判定**（クラスと攻撃方向に応じて判定。各防具は独立してロール。背面攻撃は常にスキップ）
   - 成功した防具の block_power をダメージからカット（複数の防具が成功した場合は合計）
3. **耐性適用**（物理 or 魔法耐性の能力値から軽減率を算出して軽減。軽減率 = 能力値 / (能力値 + 100)）
   - 残ダメージ × (1 - 耐性%)
4. **最終ダメージ確定**（最低1）

**防御判定のクラス別ルール**:

近接クラス（剣士・斧戦士・斥候）：武器と盾は独立した判定
| 攻撃方向 | 武器判定 | 盾判定 |
|---------|---------|--------|
| 正面 | ○（defense_accuracy ロール） | ○（defense_accuracy ロール） |
| 左側面 | — | ○（defense_accuracy ロール） |
| 右側面 | ○（defense_accuracy ロール） | — |
| 背面 | — | —（スキップ） |

遠距離クラス（弓使い・魔法使い・ヒーラー）：正面のみ武器判定
| 攻撃方向 | 武器判定 | 盾判定 |
|---------|---------|--------|
| 正面 | ○（defense_accuracy ロール） | —（盾なし） |
| 側面・背面 | —（スキップ） | — |

- 各防具判定は独立して `defense_accuracy` でロール（パーセント成功判定）
