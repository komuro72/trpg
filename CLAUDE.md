...
CLAUDE.md は **状態軸** で構成する。時系列の積み上げ（日次エントリ）は使わない。
これは「セッション開始時に現状を素早く把握する」「情報の陳腐化を防ぐ」「docs/history.md
との情報重複を避ける」ことが目的。

### 読み手の違い

- **CLAUDE.md**：claude.ai（チャット AI）+ Claude Code + Komuro が読む。AI が「現在仕様を理解するため」に必要な情報を集約
- **docs/history.md ほか**：Claude Code + Komuro のみが読む（claude.ai は基本読まない）。時系列の経緯・反省・廃止された実装の記録

つまり CLAUDE.md は claude.ai のコンテキストとしても流し込まれる前提で書く。docs/history.md
は「過去をたどりたいときに読む参考資料」であり、毎回 AI に読ませる前提ではない。

### セクションの 3 つの性格

| 性格 | 該当セクション | 意図 | 更新ルール |
|------|---------------|------|-----------|
| **実装状況** | 「セッション開始時のガイド」配下の項目 | 最近の仕様変更を AI が把握し、変更直後の不安定期に経緯を理解するための場所 | 仕様レベルの変更で項目追加。規約セクションに統合されたら卒業（削除） |
| **残タスク** | 「残タスク」セクション | 次に何をやるか（優先度別） | 完了したら削除（履歴は docs/history.md 側） |
| **規約・仕様** | 設計原則 / アーキテクチャ方針 / クラスシステム / 戦闘仕様 等 | 現在のゲーム仕様の本体 | 仕様変更時に書き換え。歴史は残さない |

### 「実装状況」セクションの目的

- AI が「最近の仕様変更」を把握し、変更直後の不安定期に経緯を理解するための場所
- 規約セクションには**最新仕様しか書かない**ので、それを補完する位置づけ
- 仕様変更直後は「以前との違い」を AI が知っている方が、関連実装の不具合に気づきやすい
...
**書かない（docs/history.md のみで管理）：**
- バグ修正
- デバッグツール追加（必要ならキー操作・デバッグ用ロガー等の既存仕様セクションに簡潔に書く）
- 一回限りの作業（警告掃除・dead code 削除等）

### 日付の扱い

- 日付エントリ（`#### 2026-04-XX`）を時系列で積み上げる方式は**廃止**
- 必要に応じて、各項目の末尾に**補足情報**として日付を付記する
  - 例：「FLEE 二段階化（4/21 完了）」「fall_back 実装（4/24 完了）」
- 時系列での詳細な経緯・反省・設計論点は **`docs/history.md`** に記録

### docs/history.md との役割分担

- **CLAUDE.md** = 「今この瞬間の真実」を要約。AI / 人間がセッション開始時に読む
- **docs/history.md** = 「いつ・なぜ・どう変わったか」の時系列詳細。原因分析・反省・廃止された実装の記録。**過去エントリの書き換え禁止・末尾追記のみ許可**
- 同じ情報を両方に書かない。CLAUDE.md は要点、history.md は深掘り

### 「実装状況」からの卒業ルール

- 新規実装直後は「実装状況」に項目を立てる（短いサマリ + 日付）
- その実装の知識が **規約セクションに統合されて常識化したら**、「実装状況」からは削除する

#### 削除の運用

- **削除タイミング**：Claude Code が総合判断（鮮度切れ・規約整理状態・AI 理解度を考慮）
- **削除前チェック**：必ず以下を確認
  - 規約・仕様・アーキテクチャセクションに最新仕様が反映されているか
- **未反映の場合の対応**：Claude Code が **自動で規約セクションへ移植** してから実装状況を削除（Komuro への確認不要・報告は任意）

### 「残タスク」の優先度分類

- 🔥 **最優先**：次セッションで着手したいもの
- **優先度：中**：いずれ取り組むが急がないもの
- **優先度：低**：将来構想・棚卸し継続項目

完了したタスクは削除。詳細な作業ログは docs/history.md 側に残る。
...
- **明示指示時**：Komuro が「md に反映してください」「CLAUDE.md を更新して」等を発したとき
- **仕様変更時**（実装と同期して即時更新）：
  - 規約・仕様・アーキテクチャセクションに該当する変更を実装したら、その実装と同じタイミング（同じ commit）で当該セクションを更新する
  - 記載漏れリスクを防ぐため、変更直後に書くのが原則
  - 「仕様レベルの変更」の判定は編集方針セクション「実装状況に書く / 書かないもの」の基準に準じる
- **作業中断・終了シグナル時**（明示指示がなくても更新する）：
  - 「今日はここまで」「一旦やめる」「明日続きをやる」「セッション終了」「コミットしてプッシュして」等
  - このタイミングで「実装状況」と「残タスク」をまとめて更新する。仕様変更の記載漏れがないかも併せて確認する
...
Phase 1〜13 完了・Phase 14（Steam 配布準備）未着手。
直近は「リファクタリング・定数整理・設計明示化・legacy 一掃」フェーズ。
実プレイ検証で即死問題がほぼ解消し、FLEE システムが活かされる耐久性を確保。

本セクションは**仕様・規約・アーキテクチャレベルの変更**を集める。
バグ修正・デバッグツール追加・一回限りの作業は記載せず、docs/history.md で管理する。

#### AI システム
- **SkillExecutor 抽出**：10 種スキルの Player/AI 計算統一（4/18）
- **戦力計算・戦況判断統合 `_evaluate_strategic_status()`**：3 集合 × 距離フィルタで戦力・戦況を一括算出（4/19）
- **距離ベース連合**：`COALITION_RADIUS_TILES` マンハッタン半径で味方連合判定（エリアベースを廃止）（4/19）
- **戦略システムの設計原則確立**：味方は `battle_policy` 駆動・敵は `_party_strategy` 駆動の非対称設計（4/21）
- **FLEE 二段階化（ステップ 1〜3 完了）**：リーダー推奨出口 → メンバー個別評価。脅威コスト付き A* + 推奨バイアス（4/21）
- **個体アクション 3 種分離**：`keep_distance`（カイティング）/ `fall_back`（後退）/ `flee`（戦闘離脱）（4/21）
- **fall_back 実装**：FLEE 計算結果再利用方式（FLEE の途中停止バージョン）+ 脅威マップ事前計算（4/24）
- **FLEE 立ち往生バグ修正・出口タイル定義を内側→外側に変更**：`MapData.get_exit_tiles_from()` が「area_id 内のタイルで隣接エリアに接するもの」から「隣接エリアに踏み込んだ最初のタイル」を返す形へ変更。goal 到達 = エリア境界跨ぎになり既存通知機構に乗る（4/25 朝）
- **FLEE/fall_back 出口選択ロジック統一・リーダー推奨機構廃止**：境界往復バグ（メンバー先行時に推奨タイル座標が exit_tiles に含まれずバイアスが中立化される）の根本対策として、リーダー推奨出口配布を廃止。共通コア `UnitAI._evaluate_exit_costs()`（reach_cost + refuge_dist × WEIGHT）でメンバー自律判断に統一。リーダーは避難先エリア ID のみ配布。`_notify_area_change_if_needed` / `on_member_area_changed` / `FLEE_NON_RECOMMENDED_PENALTY` / `FLEE_REEVAL_MIN_INTERVAL` を物理削除（4/25）
- **`combat == "flee"` 意味論変更 + flee 中の前進不要判定追加**：旧「敵検知 → flee / 未検知 → wait」の自動応答型ロジックが境界往復・WAIT 切替で前進中断する温床だったため、明示指示尊重型に変更（敵の有無に関わらず指示中は避難先まで退却）。加えて「脅威から `FLEE_SAFE_DISTANCE` 以上離れたら移動先 = 現在地」の前進不要判定を `_find_flee_goal()` 冒頭に追加。これは flee 解除ではなく移動先決定ロジックの拡張で、flee 状態自体は維持される（解除は HP critical 解除 / battle_policy 切替で自然に行われる）（4/25）
- **ヒーラーの回復対象選定に射程内フィルタ追加**：旧実装では `_find_heal_target()` に射程チェックがなく、ヒーラーは常に最遠の対象を追って移動していたため、move 指示・`combat == "flee"` 指示が無視されていた（追従しない・撤退しない）。射程内フィルタ追加により、射程外の対象を無視 → 通常メンバーと同じ move/combat フローに合流する。「射程内対象あれば回復・なければ move 指示に従う」のシンプルな対称性を確立。詳細は「ヒーラーの行動規約」セクション（4/25）
- **rear 隊形を keep_distance ベースに変更・keep_distance 実装をターゲット中心ロジックに刷新**：旧 rear は「背後を取る位置取り」のみで敵接近時の退避ロジックなし（後衛前進問題）。新仕様では rear メンバーは「理想範囲（`KEEP_DISTANCE_MIN`〜`MAX`）内に入ってから攻撃」の階層構造で動く。`keep_distance` アクションを `_find_flee_goal()` 流用から `_find_keep_distance_goal()`（ターゲット中心・脅威コスト最小タイル選定）に刷新。GoblinArcher / Salamander も新ロジックで動作。理想範囲は遠距離射程基準の絶対値（クラス非依存）で、近接が rear 選択時は理想範囲内にいても攻撃できない（仕様）（4/25）
- **NPC 階段ナビの `_global_orders.move` 汚染廃止**：旧実装は NpcLeaderAI の `_build_orders_part()` が `hint["move"]` に `stairs_down` / `stairs_up`、`hint["target_floor"]` を書き込んでいたため、PartyStatusWindow ヘッダーに OrderWindow 定義外の `mv=↓階段(F1)` が表示される規約違反だった（`_global_orders.move` の想定値域は OrderWindow の 4 値 `follow / cluster / same_room / standby` のみ・**4/28 に `explore` も削除**）。`_assign_orders()` 側にはリーダー → `stairs_*` / 非リーダー → `cluster` の per-member 配布ロジックが既に存在していたため、内部の階段ナビ動作はそのまま機能していた（規約と実装が乖離）。修正は `_build_orders_part()` で `stairs_*` と `target_floor` の hint 書き込みを廃止するだけで完結。同種の設計問題（`hint["move"] = "explore"` 書き込み）が `explore` でも残っていたが、4/28 の概念分離（OrderWindow から `explore` 削除・層 2 専用値化）と合わせて修正済み。プレイヤーパーティーとの対称性が回復（リーダー 1 人が階段ナビ・他は `_global_orders.move` 由来の追従系で動く）。バトルポリシー自動切替（`_global_orders["battle_policy"] = "retreat"`）は OrderWindow 定義値内なので問題なし（4/25・4/28 に explore 概念分離追記）
- **PartyStatusWindow 表記整理 + HpStatus 廃止**：日本語/英語が混在していたデバッグ表示を方針に従って整理（OrderWindow 由来は日本語・コード内部値はコード上の名前）。短縮形（`T:same_as_` / `S:積極` 等）を廃止し OrderWindow 正規表記に統一。NPC フラグ（合流/拒絶/共闘/回復/床固定）をコード識別子表記に変更。`HpStatus` enum は用途が `_should_use_special_skill()` の "disadvantage" モード 1 箇所のみで段階表現の意味が薄れていたため廃止し、`SPECIAL_SKILL_DISADVANTAGE_HP_RATIO` 定数（ConfigEditor 調整可能）に置換。`HP_STATUS_STABLE` (NPC フロア降下判定で使用) は `NPC_FLOOR_DOWNGRADE_HP_RATIO` に改名。戦況表示を構造化（`戦況:EVEN(9.0×1.00 / 9.0×0.85 = 1.18)`）し計算過程を可視化。`HP:` 独立表示は廃止。アクション表示（→攻撃/距離確保/逃走 等）をコード識別子（move_to_attack/keep_distance/flee 等）に統一（4/25）
...
- **リーダーの `_move_policy = follow` 自己参照解消**：4/26 per-member `_move_policy` 統一の積み残し課題。層 2 のリーダー上書きが空振りすると層 1 のベースライン `_global_orders.move = "follow"` がリーダーに継承され、hero リーダー時に `_leader_ref = self`（自己参照）になる。UnitAI の防御コードでクラッシュは回避されるが、`_formation_satisfied()` が常に true を返してリーダーが立ち止まる退化挙動になる。target 側と同じ「リーダー個別判断で上書き」パターンで解消：`PartyLeaderPlayer._decide_leader_move_override()` は常に `"explore"` を返す（プレイヤーパーティーの階段移動はプレイヤーの意思で行うため `stairs_*` 判定はしない）。`NpcLeaderAI._decide_leader_move_override()` は戦闘中（敵検知中）は `"explore"` フォールバック・探索中は `""` を返して既存の `_get_explore_move_policy()` 経路（`stairs_*` / `explore`）に委ねる。EnemyLeaderAI は今回スコープ外（追跡中の `follow` 退化挙動は実機検証案件として残置）（4/26）
- **target 死亡後の即時再選定**：NPC リーダーが敵 A を攻撃 → A 死亡 → キュー消化 → `receive_order(_order)` 再発火 で stale な target を null として受け取り、`_generate_queue` の `target == null` 経路で `_generate_move_queue` → `move_to_explore` に流れて同部屋の他の敵を素通りする現象（最大 1.5 秒の窓）を解消。`PartyLeader.select_target_for(member)` 公開ラッパーを新設し、`UnitAI.receive_order()` で「ATTACK 戦略 + ordered_target が null」のとき即時再選定する。`get_parent()` 経由で参照（UnitAI は PartyLeader.add_child で配置されるため）。FLEE / FALL_BACK / WAIT 戦略には影響なし・全パーティー種別で動作（4/26）
- **OrderWindow から `explore` 削除・概念分離（層 2 専用値に純化）**：旧実装では `explore` が OrderWindow 5 値の 1 つ（層 1 ベースライン）と「リーダー個別行動」（層 2 動的目標）の **2 つの異なる概念に同時使用**されていた。`NpcLeaderAI._build_orders_part()` が探索モード時に `hint["move"] = "explore"` を書き込み、これが層 1 ベースラインとして非リーダーにも継承され、非リーダーが独自に `_find_explore_target` を呼んで分散する隊形崩壊が発生していた。OrderWindow から `explore` 値を削除（5 値 → 4 値：`follow / cluster / same_room / standby`）し、`explore` を `stairs_*` / `guard_room` と同じ層 2 専用値に純化。`NpcLeaderAI._build_orders_part` の `hint["move"]` 書き込みも撤去。リーダーの層 2 上書きは既存ロジック（`_assign_orders` の `_get_explore_move_policy()` 経由）で動作。`PartyManager.set_global_orders` に旧仕様セーブ互換の正規化（"explore" → "follow"）を追加。`PartyStatusWindow._label("move", ...)` の特例リストに `explore` を追加（`stairs_*` / `guard_room` と同じ位置づけ）（4/28）
- **`_find_explore_target` のハッシュオフセット選択撤廃・純粋な最寄り採用**：旧実装は `offset = abs(_member.name.hash()) % candidates.size()` で NPC ごとに異なる候補を選びメンバー間分散を狙っていたが、中央安全部屋を持つマップでハッシュにより遠方候補（マップ反対側）が選ばれ、リーダーが「中央部屋を突っ切って反対側へ向かう → 殲滅 → 戻る」のように見える挙動が観察された。OrderWindow から `explore` 削除（4/28）後は `_find_explore_target` を呼ぶのはリーダーのみ（非リーダーは `follow` で追従）でメンバー間分散が不要になったため撤廃。各 NPC リーダーの初期位置がフロア全体に散らばるためパーティー間分散は緩やかに成立する見込み。集中現象が観察されたら同点候補のランダム要素導入を「NPC スタック問題」タスクで検討する（4/28）

#### パーティーシステム
- **NPC・プレイヤー初期生成の分類ベース自動構成**：JSON 直書き（`character_id` / `class_id` / `image_set` / `items`）を廃止し、`PartyComposer.RoleCategory`（`MELEE` / `RANGED` / `HEALER`）の役割分類でランタイムに編成する方式に移行。フロア 0 の構成は **NPC 14 人**：1 人 P × 4（主人公分類に応じて 4 枠 = 近接 + 遠距離 + ヒーラーから主人公分類を 1 つ減らす）+ 2 人 P × 2 + 3 人 P × 2。2 人 P はヒーラー入り 25%（重み付き）・3 人 P は等確率 3 パターン（A: 近遠癒 / B: 近近遠 / C: 近遠遠）・同分類内クラス重複なし。副次効果として画像 28 種が満遍なく使われる（旧仕様で常に未使用だった adult_medium 系も登場）+ `apply_image_set_override` の sex 変化時の名前再抽選で性別と名前の不整合バグも解消（4/26）
- **加入処理の完全化**：`adopt_member` / `release_member` API で AI 管理主体を `_hero_manager` に一本化（4/24）
- **Character ステータス最終値統一**：13 ステータスを Character に最終値フィールド集約・装備補正取得を `get_equipment_bonus(stat_name)` 単一 API 化（4/21）

#### ゲーム挙動・バランス
- **階段着地ロジックを「ペア座標一致」に統一**：従来 4 経路で着地先選択ロジックが非対称（hero / パーティーメンバー = `spawns[0]` / `stair_tiles[0]` 固定、NPC リーダー / 非リーダー = 旧階段から最近距離）だった問題を解消。マップデータ規約「階段ペアは上下階で同座標」に従い、踏んだ階段マスと同座標の新フロア階段マスに着地する共通ヘルパ `_find_paired_stair_pos(taken_pos, new_map, spawn_type)` を [game_map.gd](scripts/game_map.gd) に新設し、4 経路すべてで使用。同座標が見つからないとき（F3 ↔ F4 のようにマップサイズが異なる暫定配置等）は `spawns[0]` / `(5,5)` のフォールバックに委ねる。修正 X 導入後に観察された「hero と NPC で異なる着地マスを選ぶため、敵が片方の階段マスに立つと NPC だけブロックされる現象」を解消。CLAUDE.md「マップデータ仕様」配下に「階段ペアの座標規約」セクションを新設（4/28 後半）
- **上り階段部屋の敵初期配置を撤去（修正 X 副作用対策の本対応）**：4/28 後半の F1〜F3 拡張（30 部屋化）で 9 部屋（F1: r1_3 / r1_11 / r1_30、F2: r2_3 / r2_11 / r2_30、F3: r3_3 / r3_11 / r3_15）に上り階段の規約違反で敵 17 体が配置されていた（`docs/spec.md:4139` で旧 20 部屋時代から明文化されていた「上り階段部屋は敵初期配置なし」規約への違反）。修正 X（着地占有チェック・横出し廃止）導入後、これらの敵が階段マスに立つと上階からの遷移が永久ブロックされる事故が観測されたため、9 部屋から `enemy_party` キーを物理削除（敵 17 体は再分配せず単純減）。あわせて CLAUDE.md「マップデータ仕様」配下に「階段部屋の配置規約」セクションを新設し、上り階段部屋への敵配置禁止と検証用 Python スニペットを明文化。下り階段部屋への敵配置は適正フロアサイクル成立のため引き続き許可（4/28 後半）
- **階段着地ロジック統一（修正 X）**：従来 4 経路（hero / パーティーメンバー / NPC リーダー / NPC 非リーダー）で別実装だった着地ロジックを統一。「対応する階段マスに着地・占有時は遷移見送り（次フレームで再チェック）・横出し廃止・隣接フォールバック廃止」の挙動に変更。共通の占有判定ヘルパ `_is_landing_tile_occupied(tile, floor_idx, exclude_char)` を [game_map.gd:1941](scripts/game_map.gd:1941) に新設し、4 経路すべての `_transition_*` 関数の冒頭で呼ぶ。占有時は `return` のみ（クールダウンも設定しない）で、既存の毎フレーム再チェック機構（`_check_stairs_step` / `_check_party_member_stairs` / `_check_npc_member_stairs`）が次フレームで再判定する。旧 `_find_free_adjacent_to()`（BFS で隣接非階段マスを探索する横出し関数）は本判定への置換で物理削除（他用途なし・grep 確認済み）。永久ブロック対策（タイムアウト等）は副作用観察後に判断（残タスク「階段マス占有による永久ブロック問題」として残置）（4/28 後半）
- **A* 経路探索の動的占有考慮化（NPC が行く手をはばむ問題の構造的解消）**：`UnitAI._astar()`（[行 1927](scripts/unit_ai.gd:1927)）が地形のみを障害物として扱っていたため、味方が follow 隊形で進路に立つと迂回経路を組めず direct fallback も「主軸方向の 1 マス再試行」のみで停止する問題（マリアケース）を解消。新規 [`_build_occupied_set()`](scripts/unit_ai.gd:1982) で全パーティー全メンバーの占有タイル（`get_occupied_tiles` + `is_pending` 予約）を 1 回事前計算し、A* の各 neighbor 評価で `occupied.has(tile)` を O(1) チェック。`neighbor != goal` のとき占有を障害物扱い、goal タイル自体は例外（keep_distance / flee / fall_back / explore goal は本質的にキャラ占有マスになりうるため）。`_is_passable()` と意味論を完全一致（自分自身除外・別フロア除外・`_floor_following` 中の友好すり抜け・飛行/地上レイヤー分離・pending 反映）。`_get_next_step()` のフォールバック（`_next_step_direct`）は経路が真に存在しない壁囲み等の最終足掻きとして残置（4/28）
- **PartyStatusWindow 表示範囲仕様変更（味方系全フロア・敵は追跡対象フロア・F7 同期）**：旧仕様は「操作キャラ（または追跡対象）のフロアにいるパーティーのみ表示」で、味方系の動向（フロア降下中の他メンバー・別フロア NPC）が見えない問題があった。新仕様：プレイヤー = 全メンバー全フロア表示・NPC = 全パーティー全フロア表示・敵 = 追跡対象がいるフロアのみ表示。追跡対象は `_get_tracking_target()` ヘルパが真実源（PSW で上下キー選択した `_selected_leader` が有効ならそれ・無効なら `_hero`）。追跡対象消失時は `_process()` で自動クリア + `leader_selected.emit(null)` でカメラを操作キャラに戻す。F7 スナップショットも同表示範囲ロジック共有 + 追跡対象ヘッダー追加（後で見返したときに「これは誰を追跡していたログか」が分かるように）。タイトル行は `Floor: <N>  操作:<名前>@F<N>`（同フロア時）/ `操作:<名前>@F<N>  追跡:<名前>@F<N>`（別追跡時）（4/28）
- **`_check_stairs_step` の `world_time_running` ガード除去（修正 W・既存バグ解消）**：プレイヤー操作キャラが階段マスに到着して停止すると、無操作 → `world_time_running = false` で `_check_stairs_step()` が永遠にスキップされ、フロア遷移が発火しない既存バグを解消。`_move_buffer = ZERO; return`（[player_controller.gd:502](scripts/player_controller.gd:502)）の階段ガードと組み合わさってプレイヤーが永久に動けない状態になっていた。修正は冒頭の `if not world_time_running: return` 2 行を削除のみ（[game_map.gd:1996](scripts/game_map.gd:1996)）。time stop 中の意図しない遷移は既存の 3 ガード（`_stair_cooldown` / `stair_just_transitioned` / `is_moving()`）で十分抑止される。本セッションの修正（A* 動的占有化・案 G・wait 特例除外・F1〜F3 拡張）とは無関係の race condition バグ（4/28）
- **NPC リーダーのフロア間 FLEE 経路確立（案 G・F1+ retreat → 上り階段ナビ）**：F1 で `battle_policy=retreat` 配布中にリーダーが上り階段エリアに到達すると wait に落ちて停止する問題（マリアケース）を解消。原因は (a) `_decide_leader_move_override()` が敵検知中に常に `"explore"` を返して退避経路（stairs_up）を上書きしていた、(b) wait 特例（`_combat=="flee"` + `_is_in_refuge_area()` で wait）が階段ナビ中の refuge エリア進入時にも発火していた、の 2 点。修正：(1) `_decide_leader_move_override()` の優先順位を「retreat + F1+ + refuge 外 → stairs_up / target_floor < current → stairs_up / target_floor > current → stairs_down / それ以外 → explore」のラダー判定に変更（敵検知中も target_floor ベースで動作する）。(2) [unit_ai.gd:889](scripts/unit_ai.gd:889) の wait 特例に `_move_policy not in ["stairs_up","stairs_down"]` 除外を追加し、階段ナビ中は refuge エリア進入時も `_generate_move_queue()` 経由で階段タイルへの移動を継続。F0 retreat は条件 4 にフォールバックし、既存の個体 FLEE（`_find_flee_goal` → safe_tile）に委ねる。非リーダーは [unit_ai.gd:773](scripts/unit_ai.gd:773) の「リーダーが別フロアなら戦略無視で階段ナビ」既存経路で追従する（4/28）
- **戦力（strength）の HP 抜き化・FLOOR_RETREAT_RATIO 廃止（用語ガバナンス適用）**：旧設計では戦力 = `(rank_sum + bonus_sum × WEIGHT) × HP率` で HP を含んでおり、(a) 戦力比（`my_rank_sum / enemy_rank_sum`・HP/装備抜き）と計算式が非対称で「戦力比 = 戦力 / 敵戦力」と読めない、(b) メンバー死亡で戦力が下がっても HP 回復で戻り目標フロアが揺らぐ、という 2 つの問題があった。「用語ガバナンス（2026-04-28 確立）」のルール 2「用語間の関係を素直に成立させる」に従い、戦力を HP 抜きに変更した。新仕様：戦力 = `rank_sum + bonus_sum × ITEM_BONUS_STRENGTH_WEIGHT`（HP 抜き・装備込み）。戦況比は計算経路で別途 `× hp_ratio` を適用（戦況 = `(my_strength × my_hp_ratio) / (enemy_strength × enemy_hp_ratio)`）。戦力比は既存どおり rank_sum のみ。あわせて `_get_target_floor()` の判定ロジックを「ラダー判定（戦力 ≥ 閾値の最上階）」に簡素化し `FLOOR_RETREAT_RATIO`（昇降ヒステリシス用 0.5 倍係数）を廃止。HP 補正（`NPC_FLOOR_DOWNGRADE_HP_RATIO`）は残置（戦闘で疲弊した一時的退避を表現）。PartyStatusWindow 表示は `F=s(R+B)` で常に `R+B=s` が成立し、戦況表示の `(s×HP / s×HP = ratio)` も計算過程が素直に読める形に（4/28）
- **NPC explore 目的地選定を `_visited_areas` ベースに統一**：旧 `_find_explore_target()` は `VisionSystem.is_area_visited()`（**プレイヤーパーティー視点**）を参照しており、(a) NPC が自パで踏んだ部屋を「未訪問」扱いし続ける、(b) 全 NPC パーティーが共通の `unvisited` リストから選ぶ、(c) NPC の探索計画がプレイヤーの探索進度に左右される、という副作用があった。`PartyLeader._visited_areas`（パーティー固有・既存の `_generate_stair_queue` と同一 dict）を参照するよう変更し、視界ベース思想（自パが見たエリアを既知扱い）と整合させた。書き込み（`_generate_queue` 冒頭の `_visited_areas[cur_area] = true`・1.5 秒間隔）と読み込みが同一 dict に集約される単一管理ルールを確立（4/28）
- **Floor 0 を 30 部屋に拡張**：旧 4 列×5 行=20 部屋では NPC が視界ベース階段探索を完了する前に敵を全滅させてしまい、適正フロアサイクル（弱 NPC が上階で装備を集める → 降りる）が機能しなかったため、5 列×6 行=30 部屋に拡張。階段は 3 個 → 5 個に増やし 4 方向 + SE に分散配置（`r0_3` 北 / `r0_15` 東 / `r0_28` 南 / `r0_11` 西 / `r0_30` SE）。安全部屋（`r0_13`・`entrance_room`）は中央 row 3 col 3 に再配置（15×11・原型と同サイズ）。マップ寸法は 44×45 → 55×54（約 25% 拡大）。敵パーティー 19 → 29（密度同等）。NPC パーティー 9 構成（1 人×5 + 2 人×2 + 3 人×2 = 15 メンバー定義・runtime で 1 人 P 1 つを skip し 14 人化）は維持。フロア 1〜4 は未変更（プレイヤー/NPC 遷移は `spawns[0]` を使うため F1+ の階段数とは独立に動作する設計を確認済み）。F4 はボス部屋であり拡張しない方針（4/28）
- **Floor 1〜3 を 30 部屋に拡張（Floor 0 と同レイアウト）**：Floor 0 拡張後にサイクル後半（F1〜F3）の探索密度が相対的に低くなる懸念があったため、F1〜F3 も同寸法 55×54・30 部屋・5 階段に統一。F0 のレイアウトを基本テンプレートとして採用し ID をリネーム（`r0_X` → `rN_X`、`c0_X` → `cN_X`）、安全部屋（旧 `r0_13` 相当）は通常部屋化、player_party / npc_parties_multi / is_safe_room / is_entrance フラグを除去。階段配分は **3 上り + 2 下り**（F1: r{N}_3/r{N}_11/r{N}_30 を上り・r{N}_15/r{N}_28 を下り / F2: 同 / F3: r3_3/r3_11/r3_15 を上り・r3_28/r3_30 を下り）。`entrance_room` は最初の上り階段部屋（プレイヤー/NPC が上階から降りてきたとき到着する位置・spawns[0] 仕様準拠）。敵パーティー数は各 17 → 29（密度を F0 と揃え）。敵構成は各フロアの既存敵 mix を 29 パーティーに再分配（F1: goblin/hobgoblin/goblin_archer/goblin_mage/wolf / F2: salamander/wolf/dark_priest/harpy/zombie/hobgoblin / F3: dark_mage/dark_knight/lich/dark_priest/skeleton/skeleton_archer）。Floor 4（ボス）は変更なし。F1-3 と F4 の境界は spawns[0] 規約で接続（4/28 後半）
- **PartyStatusWindow に目標フロア表示・全体指示の優先度対称化**：(1) NPC パーティーヘッダーに `目標F:N` を追加（戦力 F と `FLOOR_X_Y_RANK_THRESHOLD` の比較結果・優先度「高」・0 始まり）。`NpcLeaderAI._build_orders_part` が `_get_target_floor()` の結果を hint に乗せる。プレイヤーパーティーは自由意思でフロア移動するため非表示・敵パーティーには概念がないため非表示。(2) 全体指示（`mv=` / `battle=` / `tgt=` / `hp=` / `item=` / 敵の `strategy=`）の優先度を「高」→「中」に降格し、メンバー行の個別指示グループ（既に「中」）と優先度を対称化。詳細度 0（高のみ）モードでは戦況・戦力・戦力比・目標F の戦力情報に集約される（4/28）
- **PartyStatusWindow 戦力表示の整理（戦況/戦力/戦力比 の 3 指標を分離）**：旧 `戦力:OVERWHELMING F(14+0.7)14.1 C(13+0.7)13.1 E(3+0.0)3.0` は (a) 表示の B 部分が WEIGHT 適用前の bonus_sum 素値で `R+B ≠ s` に見える、(b) PowerBalance enum と strength 内訳が同じ「戦力:」ラベル下に同居して用語衝突、の 2 つの問題があった。新表示は `戦力: F=14.1(14+0.1) C=13.1(13+0.1) E=3.0(3+0.0)  戦力比:OVERWHELMING` で、(a) `F=s(R+B)` の B を `bonus_sum × ITEM_BONUS_STRENGTH_WEIGHT`（WEIGHT 込み寄与値）に変更し常に `R+B = s` が成立、(b) PowerBalance を末尾の `戦力比:` 独立ラベルに分離。3 指標（CombatSituation = 戦況 / strength = 戦力 / PowerBalance = 戦力比）の用途は「戦力計算の 3 指標と用途」セクション参照（4/28・同日後に戦力 HP 抜き化により `R+B=s` が常時成立に変化）
- **敵 keep_distance に縄張り距離ペナルティ追加**：プレイヤーが GoblinArcher / Salamander を追跡すると毎サイクル keep_distance のリングがプレイヤー中心に再計算され、弓兵が縄張りから少しずつ引きずり出される現象を修正。`_find_keep_distance_goal()` のスコア計算に `home_dist_cost = area_dist × KEEP_DISTANCE_HOME_WEIGHT`（敵専用・既定 10.0）を追加し、候補タイルが属するエリアと縄張りエリア（`PartyManager._room_id`）の BFS 距離をペナルティ化。縄張り内に候補があれば必ず縄張り内優先・プレイヤーが詰めた場合は縄張りに最も近い隣接エリアの候補から自然に fallback。味方 rear 隊形には適用しない（`is_friendly == false` ガード）。「賢い敵は縄張りを考慮する」設計感覚と整合（4/27）
- **部屋制圧判定を個体 FLEE ベースに変更・エリア離脱検知を追加**：旧仕様では `_party_strategy == FLEE`（パーティー戦略）かつメンバー死亡時のみ判定 → (a) 全員逃走で死亡ゼロのときは判定が走らない、(b) DarkKnight 等狂暴系種族が個体 flee で部屋外に出ても「追跡中」扱いになり制圧されない、の 2 つの抜けがあった。`UnitAI._strategy == 1`（個体 FLEE のみ・FALL_BACK は戦闘継続なので除外）で判定し、判定タイミングに「メンバーがエリア変化したとき」を追加（`PartyManager._process` で `_per_member_last_area` 辞書を毎フレーム監視）。設計意図：「敵を撃退すれば報酬が得られる」前提を成立させ、プレイヤーが敵を誘導しただけ・敵が追跡で部屋外に出ただけではドロップしない。これによりポーション収集 → 装備強化サイクルの基盤が成立する（4/27）
- **戦力計算と NPC 自動装備を bonus 段階合計ベースに統一・tier の責務をドロップ重み付け専用に純化**：旧設計では戦力計算が tier ベース（2 ステータスは `ITEM_TIER_POLICY="max"` で導出した tier 平均）、NPC 自動装備が `_item_stats_sum`（補正値の単純合計）で評価指標が分裂していた。tier は max を取るので「中+中」と「大+無」で評価が逆転する不具合があり、stats 合計は max 値の差（`magic_resistance_max=30` vs `15` 等）が混入する不公平があった。両者を **bonus 段階合計**（各ステータスの 0=無 / 1=小 / 2=中 / 3=大 を合計）に統一。`generated/*.json` 75 エントリに `bonuses` フィールドを追加し `tier` フィールドを削除（tier はランタイムで `ItemGenerator.calc_tier()` が `bonuses` から `ITEM_TIER_POLICY` で導出）。`ITEM_TIER_STRENGTH_WEIGHT=0.33` を `ITEM_BONUS_STRENGTH_WEIGHT=0.165`（旧 tier 平均 2 ≒ 新 bonus 平均 4 と等価）にリネーム・カテゴリは PartyLeader 維持。戦力式 = `rank_sum + bonus_sum × ITEM_BONUS_STRENGTH_WEIGHT`（4/28 後半の用語ガバナンス改訂で HP 抜き化）。bonus_sum はメンバーごとの装備 bonus 合計の平均をパーティー全員で合計する設計（剣士の剣+盾 vs 魔法使いの杖のみのクラス間装備本数差をフラットに吸収）。PartyStatusWindow ヘッダーの `F(R+T)s C(R+T)s E(R+T)s` 表示も `F=s(R+B) ...` 形式に変更（4/27 に bonus_sum 化・4/28 に B を WEIGHT 込み寄与値表示へ。常に `R + B = s` が成立する設計）（4/27）
- **戦力計算式の透明化（`RANK_BASE_OFFSET` 導入）**：旧 `RANK_VALUES = {C:3, B:4, A:5, S:6}` の +3 オフセットがコード固定で「式から見えない」状態だったため、`GlobalConstants.RANK_BASE_OFFSET = 3`（PartyLeader カテゴリ・ConfigEditor 編集可）として独立定数化。集計用 rank 値 = `RANK_BASE_OFFSET + CharacterGenerator.RANK_VALUE[rank]` の加算式に統一。戦力計算（`party_leader._calc_stats`）と NPC 合流交渉スコア（`npc_leader_ai`）の両方で使用。ステータス生成側の「base + rank × rank_amount」と概念が並ぶ設計（4/27）
- **NPC が能動的にアイテム取得・ドロップ組成を装備＋ポーションに分離**：適正フロアサイクル（目標階で苦戦 → 退避 → ポーション・装備を集めて再挑戦）を成立させるため。(1) `item_pickup` デフォルトを `passive` → `aggressive` に変更（プレイヤー P・NPC P 共通・3 箇所）。(2) ドロップ組成を「装備 1 つ確定 + ポーション 1〜2 個（ランタイム生成）」に変更。`dungeon_handcrafted.json` から全ポーション 37 行を物理削除し装備 2 つ以上のパーティー 27 件は最初の 1 つに絞った。ポーション生成は `_on_enemy_party_wiped` で `POTION_DROP_MIN/MAX_PER_PARTY` の一様乱数 + `POTION_DROP_HEAL_RATIO` で種別判定する。3 定数とも ConfigEditor Item カテゴリで調整可能（4/27）
- **MP/SP を `energy` に統合**：内部単一フィールド・UI でクラス別表示切替（4/18）
- **ポーション刷新**：ヒールポーション / エナジーポーション（MP/SP 統合）（4/18）
- **HP 状態ラベル閾値リネーム**：「HP 率がこの値を下回ったら次の悪い状態」になる自然な命名へ（旧 `CONDITION_HEALTHY/WOUNDED/INJURED_THRESHOLD` → 新 `WOUNDED/INJURED/CRITICAL_THRESHOLD`）（4/23）
- **ヒーラー回復モード 4 モード再設計**：`aggressive` / `lowest_hp_first`（既定）/ `own_party_first` / `conservative`（4/23）
- **ポーション INJURED 拡張**：HP ポーション自動使用が `critical` だけでなく `injured` でも発動するように（4/23）
- **ATTACK_TYPE_MULT 比率 2:1 化**：近距離（melee/dive）= 0.2、遠距離（ranged/magic）= 0.1。即死問題が解消（4/22）
- **HP/MP/SP ゲージ 100 スケール化**（左パネル）：個体の max 値に依らず一定基準で描画（HP=100 で全幅）（4/22）
- **攻撃フローの 1 発 1 押下モデル**：押下中ターゲット閲覧・離した瞬間に発動。4 内部ステート（`PRE_DELAY` / `PRE_DELAY_RELEASED` / `TARGETING` / `POST_DELAY`）（4/20）
- **向き変更 PRE_DELAY 並行消化**：押下から TARGETING までが pre_delay 一定（クラス毎に違う TURN_DELAY が体感に響かない）（4/22）

#### データ管理・JSON 規約
- **フロア / area_id 0 始まり統一**：JSON の `floor: 0〜4` と area_id `r0_*〜r4_*` をコード内部表現と一致。詳細は「フロア番号と area_id の表現規約」（4/25）
- **アイテム事前生成方式**：定数ベース 2 ステータス × 3 段階 = 9 パターン × フロア重み選択。`generated/*.json`（75 エントリ）（4/19）
- **bonus / tier 概念分離**：`bonuses` フィールド（各ステータスの 0/1/2/3 段階・generated JSON の一次情報）と `tier`（派生値・`calc_tier()` で `bonuses` から導出・ドロップ重み付け専用）を分離。bonus_sum=0 のエントリが初期装備（4/19 導入・4/27 整理）
...
#### デバッグ・運用
- **PartyStatusWindow にグローバル座標表示追加**：各キャラ行の `@<area_id>` の直後に `(x,y)` グローバル座標を併記（旧 `@r0_10` → 新 `@r0_10(40,25)`）。`Character.grid_pos.x/y` を直読み。ヒーラー射程内フィルタの挙動・FLEE/fall_back の進行度・パーティー集結度などをデバッグ表示で追跡可能に。スナップショット（F7）にも自動反映（4/26）
- **右パネル 14 人収容化**：`row_h = gs × 0.74` 固定係数 → パネル全高 `(vh - 4) / TARGET_TOTAL_ROWS`（14）の動的計算に変更し、安全部屋で 14 NPC 全員が表示される。`TARGET_TOTAL_ROWS = 14` 定数で UI 収容数を明示化（4/26）

#### 編集方針
- **md ファイル更新タイミングの 3 段階ルール**：(1) 明示指示時 / (2) 仕様変更時（実装と同期して即時 commit）/ (3) 作業中断・終了シグナル時。仕様変更時の即時更新ルールが追加されたことで、規約・仕様・アーキテクチャセクションの記載漏れリスクを最小化する。実装状況・残タスクは終了シグナル時にまとめて反映する運用（4/26）
...
##### 次セッション議論候補（4/27 セッション終了時に整理）

claude.ai 側で設計議論 → 方針確定 → Claude Code に指示プロンプト作成、の流れで進める。
実プレイ観察を継続しつつ、未着手の検討事項を進める。優先度の高いものから議論：

1. **装備取得時の確認ウィンドウ仕様（プレイヤー操作キャラ向け）** — 詳細は「優先度：低」セクションの同名項目を参照。bonus 段階合計ベースの「強さ」判定は実装済み（4/27）なので、今回は UI / 確認対象範囲 / 時間停止扱い / デフォルト挙動の議論に集中できる
2. **ドロップアイテムの JSON 記載範囲・設計意図確認** — 詳細は「優先度：低」セクションの同名項目を参照。実プレイで装備強度の傾向が見えてきたタイミングで議論
3. **他の遠距離種族への keep_distance 採用検討** — 詳細は「優先度：中」セクションの同名項目を参照。実プレイで「魔法使い系が近接されても距離を取らない違和感」が出るかを観察
4. **`_room_id` と `_home_area_id` の統合検討** — 詳細は「優先度：低」セクションの同名項目を参照。`_party_strategy` 廃止と一緒に行う候補（優先度低・タイミング判断）

##### アイテム取得サイクル動作観察（4/27 改修後・コア成立確認済み）

4/27 改修（item_pickup aggressive 化・ドロップ組成「装備 1 + ポーション 1〜2」分離・個体 FLEE ベース制圧判定・敵 keep_distance 縄張りペナルティ）後の継続観察。

**コアサイクル成立を実プレイで確認済み**：
- ゴブリンアーチャーが縄張り内で距離維持しながら戦う
- HP 低下で個体 FLEE 状態になり部屋を出る
- 部屋から敵が消えるとアイテムドロップ
- 「敵の縄張り → 撃退 → 報酬 → 装備強化」のサイクルが機能

**継続観察項目**：
- ヒーラー不在 P のサイクル成立（目標階苦戦 → 退避 → 装備・ポーション収集 → 再挑戦）
- ポーション 1〜2 個ドロップが足踏みサポートとして十分か
- NPC が上階に長く滞在しすぎないか（過剰退避）
- 装備強化による戦力底上げが見えるか（`F=s(R+B)` 表示で確認）
- Salamander の挙動が GoblinArcher と同様に縄張り考慮で改善されているか
- 他の遠距離種族（GoblinMage / DarkMage / Lich / Demon / SkeletonArcher）が近接されても距離を取らない挙動の違和感（議論項目 3 と連動）
- 戦力閾値（F1=8 / F2=13 / F3=18 / F4=24）が bonus 段階合計化後も適切か
- 装備強化が戦力評価に反映されて適正フロア判定が下がるサイクル
- **Floor 0〜3 拡張（20 → 30 部屋・4/28）後の探索完遂時間・敵殲滅率・適正フロアサイクル成立度**（F0 は 4/28 前半、F1〜3 は 4/28 後半に拡張）
- **explore 目的地のパーティー固有 visited 化（4/28）と最寄り選択（4/28）後、NPC パーティー間で目的地集中が起きていないか**（PartyStatusWindow の `move_to_explore:r0_X(...)` を並べて観察）

調整候補：`POTION_DROP_MIN/MAX_PER_PARTY`・`POTION_DROP_HEAL_RATIO`・`ITEM_BONUS_STRENGTH_WEIGHT`・`KEEP_DISTANCE_HOME_WEIGHT`・`FLOOR_*_RANK_THRESHOLD`・`NPC_FLOOR_DOWNGRADE_HP_RATIO`。
...
##### FLEE / fall_back 動作確認（2026-04-25 大改修後）

リーダー推奨機構廃止・自律判断統一の実機検証。
- `battle_policy = "retreat"` で FLEE 発動 → 安全部屋方向に**境界往復せず連鎖移動**すること
- 進路上に敵がいる場合、別出口（敵を避ける方向）を選ぶこと
- `on_low_hp = fall_back` メンバーが critical 状態で射程外（`attack_range + FALL_BACK_MARGIN`）まで下がって停止すること
- PartyStatusWindow ヘッダーが `避難先:r0_10` のみで座標表示なし
- メンバー行 `状態:flee→(x,y)` / `fb→(x,y)` がメンバー本人の選んだ出口座標を表示すること
- 必要に応じて `FLEE_AREA_DISTANCE_WEIGHT`（既定 10.0）を調整：大きいほど避難先方向に強く収束し脅威回避が弱まる
...
##### 他の遠距離種族への keep_distance 採用検討

現状 `keep_distance` 行動を持つ敵種族は **GoblinArcher / Salamander の 2 種のみ**。他の遠距離種族（GoblinMage / DarkMage / Lich / Demon / SkeletonArcher）はプレイヤーに近接されても距離を取らず、その場で魔法を撃ち続ける挙動になっている。種族ごとの性格に応じて keep_distance 採用を判断する：
- 賢い／臆病な遠距離（GoblinMage / DarkMage 等）：採用候補
- 強気な遠距離（Lich / Demon 等のボス級）：採用しない選択肢もあり
- アンデッドの一部（SkeletonArcher）：性格設計次第

採用時は GoblinArcher / Salamander と同じパターン（`_generate_queue` override で距離 ≤ 2 のとき `keep_distance` をキューに積む）でよい。4/27 の縄張り考慮も自動的に効く（`_find_keep_distance_goal` 共通実装のため）。

##### 敵リーダーの追跡中 follow 退化挙動の実機検証

4/26 の `_move_policy = follow` 自己参照解消で味方側（`PartyLeaderPlayer` / `NpcLeaderAI`）には `"explore"` フォールバックを入れたが、敵側（`EnemyLeaderAI`）は今回スコープ外とした。`_decide_leader_move_override()` の `guard_room` 判定で空振りした場合（縄張り外で追跡中など）に層 1 の `"follow"` がリーダーに残り、`_leader_ref = null` で `_formation_satisfied()` が常に true を返して立ち止まる退化挙動が起きうる。実機で観察して、深刻なら `EnemyLeaderAI` 基底に `"chase"` 等の独自フォールバック値を入れるか `"explore"` を流用するかを判断する。

##### 他フロア（B2F〜B4F）の NPC 配置検討

4/26 の分類ベース自動構成はフロア 0 のみ対象（他フロアには `npc_parties_multi` 自体が存在しない）。他フロアにも同方式で NPC を配置するか、別の配置設計（人数を減らす・特定クラスを配置する等）にするかを議論。実装するならフロア毎の人数構成テンプレート（1 人 P 数 / 2 人 P 数 / 3 人 P 数）を JSON か定数で定義する。

##### VisionSystem の別フロアスキップ（性能最適化候補）

[vision_system.gd:138](scripts/vision_system.gd:138) `_process` が毎フレーム全フロア合算の `_enemy_managers` / `_npc_managers` に対して `update_visibility` を呼び出している。`em.get_floor() != _current_floor_index` で早期 continue を入れれば走査量が **1/4 に削減**できる（F0〜F3 が 30 部屋に拡張された後の 4 フロア合算 → 現フロアのみ・F4 は 20 部屋のままで影響小）。Floor 1〜3 拡張（4/28 後半）で敵総数が増え、最も効果が出やすい改修。`_per_floor_enemy_managers` への置き換えで実装可能。

##### `_all_members` のフロア別分離

[game_map.gd:579](scripts/game_map.gd:579) で `set_all_members(all_combatants)` に全フロア合算を渡しているが、`_evaluate_strategic_status`（[party_leader.gd:1138](scripts/party_leader.gd:1138)）も `_within_coalition_radius` も同フロアフィルタで弾くため、最初から同フロアのみ渡せば走査量が **1/5 に削減**できる。フロア遷移時に再構築する既存ロジックを拡張するだけ。

##### REEVAL_INTERVAL のジッタ分散

全 NPC パーティー（~8 個）が同じ 1.5 秒境界で同時に `_evaluate_strategic_status` を呼ぶと、特定フレームに負荷が集中する。`_reeval_timer` の初期値を `randf() * REEVAL_INTERVAL` で散らせば負荷が時間軸に均される。実装数行・低リスク・効果は局所的だが、Floor 0 拡張で NPC 数が増えても変わらず効く改善（Floor 1〜3 拡張時にも有効）。

##### 適正フロア算出の見直し（元々の本題）

4/26 の NPC 構成整理（1 人 P × 4 + 2 人 P × 2 + 3 人 P × 2）を前提に、フロア降下判定（`FLOOR_*_RANK_THRESHOLD` / 装備 bonus 反映）を見直す。3 人 P パターン A（近遠癒）は鉄板構成だが C ランクばかりだと B1F でも苦戦する実例（`snapshot_20260426_162348_136.log` で観測：ノエルパーティーが CRITICAL）が既に観測されている。実プレイで「適正フロア = 戦力閾値」と「実際の戦闘テンポ」のキャリブレーションが必要。

調整時の観察ツールとして PartyStatusWindow ヘッダーの NPC 行に `目標F:N` 表示が利用可能（4/28 追加・優先度「高」）。`戦力: F=s(R+B)` の `s` と `目標F:` を並べて見れば閾値判定の挙動が直接観察できる。

Floor 0 拡張後の観察項目（4/28）：30 部屋・5 階段化により NPC が階段を発見するまでの探索時間が伸びている。サイクル成立確認（弱 NPC が F0 で装備を集めて強くなる → F1 へ降りる → 苦戦したら F0 に戻る）の挙動を実プレイで継続観察。`F=s(R+B)` の strength `s` が `FLOOR_1_RANK_THRESHOLD`（既定 8）に届くまでの時間、`目標F:0/1` の切り替わり頻度などを追跡する。

派生課題（命名乖離）：`FLOOR_X_Y_RANK_THRESHOLD` の定数名は「ランク和の閾値」を示唆するが、実態は **weighted strength**（`rank_sum + bonus_sum × ITEM_BONUS_STRENGTH_WEIGHT`・4/28 後半に HP 抜き化）との比較（[npc_leader_ai.gd:159](scripts/npc_leader_ai.gd:159) `_get_target_floor()` 経路）。`_STRENGTH_THRESHOLD` への改名か、計算ルールを命名に合わせて純粋ランク和比較に戻すか、本タスクと一緒に判断する（4/28 追記）。

##### 部屋内敵殲滅動機の強化（将来構想）

target 即時再選定（4/26）で 1.5 秒の窓は埋めたが、「敵を全滅させてから次の行動に移る」という積極的な殲滅動機は別軸の課題として残る。今後 NPC パーティーもアイテム（ポーション・装備）の重要度が上がる場合、敵を残したまま部屋を離脱する現挙動が不利になる可能性。アイテムシステム成熟後（フロア降下バランス・ドロップ確率調整等が落ち着いた段階）に再検討する。

##### `_party_strategy` 廃止検討

敵パーティー専用の `_party_strategy` enum（ATTACK / FLEE / WAIT / DEFEND / EXPLORE / GUARD_ROOM）について、調査（`docs/investigation_party_strategy_transition.md`）と設計議論の結果、以下の理由で「将来廃止」方針が確認された：

- `ATTACK` / `FLEE` → `_global_orders.battle_policy` で統一可能（味方側は既にこの方式）
- `WAIT` → 戦略ではなく処理停止フラグ（敵不在時のみ）
- `GUARD_ROOM` → リーダーの `_move_policy` 値に移管済み（4/26 area_id ベース実装で対応）
- `EXPLORE` → リーダーの `_move_policy = "explore"` で統一可能
- `DEFEND` → 未使用

廃止時に整理すべき項目：
- `territory_range` / `_apply_range_check()` / `_all_members_out_of_range()` / `_any_member_can_engage()` / `_all_members_at_home()` の物理削除
- `party_fleeing` ブールフラグの再設計（`battle_policy == "retreat"` で代替）
- `_is_in_explore_mode()` / `_is_in_guard_room_mode()` フック廃止（リーダー `_move_policy` 値で統一）
- 種族 AI の FLEE ロジックの統合方針決定（`global_orders.battle_policy` 経由か、種族個別管理か）
- 敵 AI への「CRITICAL 時自動 FLEE」を全種族共通化するか、種族 opt-out 方式（ホブゴブリン狂暴設定など）にするか

参考：4/21 改訂で味方側の `_party_strategy` は既に廃止済み。敵側の廃止はその対称作業として位置づける。

関連：既存残タスク「敵パーティーへの FLEE 自律ロジック適用」「敵側の `_party_strategy` enum 直接配布」は本タスクに統合される。

##### 敵 territory_range の物理削除検討

敵 GUARD_ROOM の area_id ベース実装（2026-04-26）以降、`territory_range` は実質デッドコード。個別敵 JSON（16 ファイル）から削除し、`character_data.territory_range` フィールド・`_apply_range_check()` 関連メソッドも撤去する。`_party_strategy` 廃止と一緒に対応するのが自然（同じ `_apply_range_check()` 経路を撤去するため）。

##### `PartyManager._room_id` と `EnemyLeaderAI._home_area_id` の統合検討（優先度：低）

両者は同じ概念（敵パーティーの初代スポーン位置 area_id = 縄張り部屋）を別レイヤで独立に保持している。`_room_id` は PartyManager が `_setup_enemy()` で `spawn_list[0]` の位置から、`_home_area_id` は EnemyLeaderAI が初代リーダーから設定する。値は実質同じ。今回（2026-04-27）の制圧判定改修では `_room_id` を流用したが、将来的にはどちらか一方に統合するのが望ましい。`_party_strategy` 廃止検討と一緒に整理する候補。

##### 敵 FLEE 戦略の方針決定

敵パーティー全体の FLEE 戦略について、以下のいずれかで対応する方針を決める：

- **案 A**：`_global_orders.battle_policy = "retreat"` に統合（味方 NPC と同じ `_evaluate_and_update_battle_policy` 相当を敵にも導入）
- **案 B**：種族個別ルーチン（UnitAI レベル）で個体ごとに FLEE 判断（「敵は知能・性質によりリーダー指示に従わない」設計観に整合）
- **案 C**：両方を併用（ベースは `battle_policy`・種族個別で上書き）

現状は Goblin / Wolf のみ「半数死亡」で `Strategy.FLEE`、他種族はパーティー全体 FLEE の経路なし。個体レベルでは `UnitAI._should_self_flee()`（HP < 30%）で動いている。

`_party_strategy` 廃止と一緒に方針決定する。

##### 4/21 調査結論「敵では参照されない指示」の再検証

per-member `_move_policy` 決定ロジック統一（2026-04-26）で、敵の非リーダーが `_global_orders.move` を継承するようになり、4/21 調査時点の結論「敵では参照されない 8 項目」が部分的に古くなった。`_global_orders` の他フィールド（`battle_policy` / `target` / `on_low_hp` / `hp_potion` / `sp_mp_potion` / `item_pickup` 等）について、per-member 配布経路で敵に効く可能性を再調査する。

調査結果を踏まえて、敵指示グループ表示の方針を再決定する：
- **A**：効くフィールドだけ厳選表示
- **B**：敵にも味方と同じ指示グループを完全表示
- **C**：現状維持（`mv=` のみ表示）

参照：`docs/investigation_enemy_order_system.md` / `docs/investigation_enemy_order_effective.md`（4/21 当時の調査）

##### 合流済みパーティー周辺の実装整理

2026-04-24 の改修で「合流済み NPC パーティー」は独立実体から `_hero_manager` への
メンバー移管方式に変更されたが、関連コードに整理対象が残っている：

- `_merge_player_into_npc_party`（逆パターン）の存在意義の確認・実プレイで発火する経路の有無（実態としては残っていないが、コードは残存）
- `_follow_hero_floors` のセット経路の不透明さ（`_merge_npc_into_player_party` で
  `set_follow_hero_floors()` が呼ばれていない・本当に必要か検証）
- `_generate_floor_follow_queue()` の現状の発動経路の確認・必要性の判定（デッドコードの可能性）
- `joined_to_player` フラグの存在意義の整理（hero 個人マーカー / 逆パターン用 /
  UnitAI 連動）
- CLAUDE.md の旧実装ベース記述の更新（「合流済みパーティーは別途
  `_generate_floor_follow_queue()` が...」等の記述が古い設計のままになっている可能性）
- PartyStatusWindow ヘッダーの `joined_to_player` 表示の意義（NPC パーティーで
  発火するシーンが現実装でほぼ存在しないなら、表示削除候補）
- 関連調査済み：項目 1〜7 の詳細な調査結果は docs/history.md（2026-04-25 エントリ）
  を参照

##### 後衛キャラがリーダーで先陣を切る問題

プレイヤーパーティー・NPC パーティー共通で、後衛クラス（弓使い・魔法使い・ヒーラー）がパーティーリーダーになると、戦闘時に前衛より先に敵に近づくことがある。リーダー選定基準の話で、4/25 keep_distance/rear 隊形修正の議論で「後回し」と判断したもの。リーダー選定アルゴリズムの見直し or rear 隊形のリーダー対応が候補。

##### 装備取得時の確認ウィンドウ仕様（プレイヤー操作キャラ向け）

プレイヤー操作キャラがアイテムを拾った際、自動装備するか・確認ウィンドウを出すかの仕様。現状は自動装備機構自体がない（OrderWindow 経由で手動装備のみ）。仕様検討案：
- 拾った直後に「装備しますか？」確認ウィンドウを表示（ただし時間停止が増えてテンポを損ねる懸念）
- 自動装備するが、明確に劣位の場合（特定ステータスで負け越し等）に確認を出す
- 装備差分を簡易プレビュー（OrderWindow 開かずに判断できる UI）

claude.ai 議論論点：
- 確認対象の範囲：操作キャラのみ / プレイヤーパーティー全員（操作キャラ以外も装備候補）/ NPC は含めない（自動装備のまま）
- 「強い」の判定基準（NPC 自動装備と同じ `_item_bonus_sum`（bonus 段階合計）を流用するか）
- 時間停止の扱い（アイテム UI 中の時間停止と同じ扱いか）
- デフォルト挙動の設定（確認するか自動装備かの切替を ConfigEditor or OrderWindow に出すか）
- 確認ウィンドウの UI 設計（既存の OrderWindow / アイテム UI との一貫性）

NPC 自動装備は既に bonus 段階合計（`_item_bonus_sum`）で判定している（4/27 統一）ので、プレイヤー側もこれと整合させる前提で設計する。

##### ドロップアイテムの JSON 記載範囲・設計意図確認

現状の装備ドロップは「JSON に item_type 文字列のみ → 事前生成セット（generated/*.json 75 エントリ）から item_type 一致のものを重み付き選択」という間接的な設計。

確認したい点：
- このアーキテクチャで意図通りか（同じ部屋で違う装備が出る挙動の設計意図確認）
- JSON に tier も指定できるようにすべきか（特定の部屋に高 tier 装備を確実に配置する設計が必要か）
- フロア重み選択の現状（`FLOOR_X_Y_BASE_TIER` で深い階ほど高 tier が出やすい仕組みが意図通りに動いているか）
- 実プレイで「装備が強くなりすぎる/弱すぎる」傾向はあるか

実プレイ観察と並行して claude.ai で議論する。
...
FLEE システムは 4/21 ステップ 1〜3 + 4/24 fall_back 実装 + 4/25 出口外側化 + 4/25 リーダー推奨機構廃止（自律判断統一）+ 4/25 rear 隊形を keep_distance ベースに変更（後衛前進問題解消）+ 4/28 NPC リーダーのフロア間 FLEE 経路確立（案 G・F1+ retreat → stairs_up）まで完了。残課題：
1. **敵パーティーへの FLEE 自律ロジック適用**：現状 `_find_flee_goal_legacy`（脅威から 5 タイル離れる）のまま。味方と同じ `_evaluate_exit_costs()` 経由に切り替え。避難先は敵種族ごとに異なる可能性（縄張り戻り・階段逃走）の設計検討必要。フロア間 FLEE（敵が階段で別フロアに逃げる）は味方側で 4/28 に確立した経路（リーダー move_policy = stairs_up）と対称的に実装可能だが、敵に「適正フロア」概念がないため別ロジックが必要
...
3. **戦況判断と避難先計算の統合**：`_evaluate_strategic_status()` と `_update_flee_refuge_area_id()` を統合し、避難先情報を `combat_situation` 辞書に含める設計検討
4. **fall_back 戦略での階段ナビ対応**：HP critical で `on_low_hp=fall_back` 発動時は FALL_BACK(3) 戦略で fall_back アクションが優先され、リーダーの `_move_policy=stairs_up` 上書きが効かない。F1+ で critical 状態の NPC リーダーが上り階段に向かわず fall_back で停止する可能性。実プレイ観察案件（4/28 追記）

##### フロア基準値の実プレイ調整

`FLOOR_*_RANK_THRESHOLD` の調整。装備 bonus 戦力反映（4/27 bonus_sum 化）と HP 抜き化（4/28 用語ガバナンス）により、戦力 = `rank_sum + bonus_sum × WEIGHT` の純粋な合計値で判定される。PartyStatusWindow の `戦力: F=s(R+B)` の `s` と `目標F:N`（4/28 追加）を並べて観察しながら調整できる。`FLOOR_RETREAT_RATIO`（昇降ヒステリシス用）は HP 抜き化と同時に廃止され、ラダー判定（戦力 ≥ 閾値の最上階）になっているため昇降が対称。Floor 0 拡張（4/28・20→30 部屋）に伴いサイクル所要時間が変わる可能性があるため、合わせて再評価する。

##### NPC フロア遷移判定の戦況判断拡張

旧 `NPC_HP_THRESHOLD` / `NPC_ENERGY_THRESHOLD` 廃止により、現在は `full_party_hp_ratio` のパーティー平均HP率で判定している（閾値 `NPC_FLOOR_DOWNGRADE_HP_RATIO`）。元の「最低HP率」「エネルギー率」ベースの判定は NPC 行動として意味があるため、戦況判断（`_evaluate_strategic_status()`）の副情報として最低HP指標・エネルギー指標を追加することを検討。別系統の判定を走らせず、戦況判断に一元化する設計方針を維持する。

##### NPC スタック問題（ランダム要素検討）

NPC パーティーで複数メンバーが固まって動かない現象が散発する。原因は不明（同 goal 押し合い / 経路探索失敗 / 優先順位など複数要因可能性）。対症療法として goal 選定にランダム要素導入を検討。`keep_distance` / `move_to_attack` / `move_to_explore` など各種 goal 計算で同点候補がある場合に微小なランダム値で破るアプローチ。本タスク（4/25）で `keep_distance` の goal 選定が脅威コスト + 到達コストになるが、同点ケースでスタックが起きるか実プレイで観察。

2026-04-28 追記：`_find_explore_target` のハッシュオフセット選択を撤廃し純粋な最寄り選択に変更（中央部屋経由問題の解消）。これにより複数 NPC パーティーが同じ目的地に集中する可能性が（理論的には）増加した。実プレイで観察し、必要なら同点候補のランダム要素導入を本タスクの一環として検討する。

##### NPC が行く手をはばむ問題（A* 動的占有化で部分解消・継続観察中）

プレイヤーや NPC リーダーが移動しようとした際、進路上に他の味方キャラが立ち止まっており通行できない現象。2026-04-28 に **A* 経路探索を動的占有考慮型に変更**（[unit_ai.gd:1927](scripts/unit_ai.gd:1927) `_astar()` + [`_build_occupied_set()`](scripts/unit_ai.gd:1982)）して構造的に解消した：A* が中間タイルの占有を障害物として扱い迂回経路を返すため、部屋内に迂回路がある限り進路を塞ぐ友好キャラを A* が自然に避ける。`goal == 占有マス` のケースは例外的に通す（keep_distance / flee 等で goal がキャラ占有マスになる正当な経路のため）。

残課題（実プレイ観察）：
- 狭い通路（迂回路がない）で味方が立ち止まると依然通行不可。これは構造的に避けられない（A* が経路なしを返す）
- swap（位置入れ替え）の導入は別軸の解決策として残る
- 「立ち止まっている側を能動的に避けさせる」アプローチ（通せんぼ検知）は実装規模大きく後回し

「NPC スタック問題（複数メンバーが固まって動かない）」とは別現象。本タスクは「他のキャラの動線を塞いでいる」現象。

##### 階段マス占有による永久ブロック問題（修正 X 副作用対策）

修正 X（着地占有チェック・横出し廃止・2026-04-28）導入後の副作用として、対応階段マスの占有元キャラが動かない場合に**遷移要求側が永久にブロックされる**可能性がある。実プレイで観察すべき項目：

- 敵が階段マス上に長時間立ち止まる頻度（特に敵 keep_distance / WAIT 戦略で階段マスに留まるパターン）
- NPC が follow 隊形で階段マスに立ち止まる頻度
- プレイヤー操作キャラが階段の向こう側で停止していて、追従メンバーが渡れない頻度
- 階段ナビ中の NPC 同士が片側で詰まり合う（リーダーが先客 → 後続メンバーが永久待機）

頻発するなら以下のいずれかで対策：

- **タイムアウト機構**：占有チェック失敗が N 秒続いたら強制的に隣接マスへ着地（修正 X 部分撤回）
- **階段マスを goal 候補から除外**（全パーティー対象）：階段は「通過のみ」で「最終目的地」にしない設計に変更。NPC 階段ナビは「階段の 1 マス手前」を goal にする
- **占有元への強制移動指示**：階段上に長居しているキャラに「動け」シグナルを送る（既存の [unit_ai.gd:765-769](scripts/unit_ai.gd:765) `move_policy not in ["stairs_*"]` 強制移動ロジックの拡張）
- **待機表示 UI**：「階段で待機中」を PartyStatusWindow に表示してデバッグ性向上
- **敵フロア間移動の設計とセット**：敵が階段を使う経路が整備されれば、敵が階段マスを能動的に通過する → 占有が短時間で解消される

関連：「敵のフロア間移動の設計検討」（後述）と統合検討する候補。

##### 敵のフロア間移動の設計検討

敵が階段を能動的に使う仕組みは未実装。現状は「敵は同フロア内のみ動作」が暗黙の前提。検討項目：
- **敵 FLEE のフロア間遷移**：怪我した敵が他フロアに逃げるか
- **敵探索のフロア間遷移**：敵がフロアを移動して新天地で活動するか
- **フロア間追跡**：プレイヤーが階段で逃げたら追ってくるか

味方側の `_decide_leader_move_override()`（案 G・4/28）のような単純な対称作業にはならない。敵に「適正フロア」概念がないため、別ロジック（例：HP 低下時の上り階段選好・特定種族の階段使用許可・階段周辺で立ち止まらない設計）が必要。

既存残タスク「敵パーティーへの FLEE 自律ロジック適用」（FLEE 派生課題 1）と統合または相互参照する候補。

##### NPC が階段を素通りする現象の調査

階段ナビ中の NPC リーダーが階段マスを通過して向こう側で立ち止まる現象を 2026-04-28 に観測。設計上はリーダーが `_move_policy = stairs_up` / `stairs_down` で階段マスを最終目的地にし、踏んだ瞬間にフロア遷移するべき。現象の真因不明・調査必要。

調査ポイント：
- `_check_party_member_stairs()`（[game_map.gd:1522](scripts/game_map.gd:1522)）/ `_check_npc_member_stairs()`（[game_map.gd:1607](scripts/game_map.gd:1607)）の発火条件
- 修正 W（`_check_stairs_step` の `world_time_running` ガード除去・4/28）の **NPC 版** が必要か（NPC 用の `_check_*_stairs` 関数にも同様のガードが残っていないか）
- A* 経路で階段マスが終点として選ばれているか
- リーダーが `_move_policy = stairs_*` でも `_battle_formation` ベースの戦闘移動が優先されている可能性（戦闘中は move_policy がバイパスされる仕様の影響）

##### プレイヤー戻り問題（修正 Y）

プレイヤーが階段を下りた後、階段マスに乗り続けると `_stair_cooldown=1.5s` 経過後に `_check_stairs_step()` が再発火し、戻る方向の階段で再遷移する可能性が指摘された（修正 W の事前調査時）。実プレイで検証したところ症状は確認されず（2026-04-28）。`stair_just_transitioned` フラグが [player_controller.gd:506](scripts/player_controller.gd:506) で「階段タイルから出るまでリセットされない」設計のため、実際には抑止されている可能性が高い。

念のため監視継続。再現したら以下を検討：
- `_check_stairs_step` の「踏んだ瞬間」と「踏み続け」の区別を導入
- `stair_just_transitioned` のリセット条件見直し

##### HP 表現の命名整理（リファクタリング）

`Character.get_condition()` が個体レベル HP 段階を返す関数だが、命名から用途が読み取りにくい。`get_estimated_condition()` または用途を反映した名前への改名を検討。影響範囲が広い（呼出元すべて）ので低優先度。

##### state 表示の実装（PartyStatusWindow）

UnitAI のステート（IDLE / MOVING / ATTACKING_PRE 等）の表示が未実装（`VAR_PRIORITY` には登録済み）。実装するにはサブクラス間のステート定義の統一が前提。timer 表示は既存の `[ATKp 0.34s|q3]` で実質統合済み。

#### 優先度：低

##### hero の class_id が JSON で無視される暫定状態

`game_map.gd:_setup_hero` で JSON の `class_id` を読むがランダム上書きする現状（[game_map.gd:280](scripts/game_map.gd:280) 参照）。「ゲーム開始時 UI でクラス選択」を実装する際に整理する想定。それまではコメントで暫定状態を明示するなどの対応のみ。CLAUDE.md「プレイヤー（hero）生成の暫定仕様」セクションにすでに規約化済み。

##### `same_room` 指示の廃止検討

OrderWindow の `move` 値「リーダーと同じ部屋に行く」（`same_room`）の挙動は実用度が低い。廃止して OrderWindow 3 値（`follow / cluster / standby`）にスリム化する案を検討。

2026-04-28 改訂：本タスク提案当時は OrderWindow が 5 値（`follow / cluster / same_room / standby / explore`）だったが、4/28 の `explore` 概念分離（OrderWindow から削除・層 2 専用値化）で 4 値になっている。`same_room` 廃止が実現すれば 3 値まで絞れる。同種の整理が進んだため優先度判断を見直す余地あり（要議論）。

廃止する場合の影響範囲：
- `_global_orders.move` の値域（OrderWindow 3 値に縮小）
- `UnitAI._formation_satisfied()` / `_target_in_formation_zone()` / `_formation_move_goal()` の `same_room` 分岐
- セーブデータ互換（既存セーブの `same_room` 設定をどの値にマップするか）
- `PartyLeader._assign_orders()` のフォールバックチェーン（`order.get("move", "same_room")` 等）

##### `_is_in_explore_mode()` フック廃止検討

現状、敵側は基底実装（`_party_strategy == EXPLORE` で判定・実質発火しない）、NPC 側は `_has_visible_enemy()` で判定する別経路の override が共存している。本来「リーダーの `_move_policy = "explore"` で統一」が自然な設計。`_party_strategy` 廃止と一緒に整理する。

##### `battle_policy` 自動切替の設計再考

NpcLeaderAI が戦況 CRITICAL/SAFE で `_global_orders.battle_policy` を自動切替する仕組みは OrderWindow 定義値の範囲内（attack/defense/retreat）で問題ないが、思想として「NPC リーダー判断結果を `_global_orders` に書き込む」という構造は残っている。同種の設計問題（4/25 階段ナビで解消した stairs_* 書き込み）の延長として将来整理対象。

##### `_can_attack_target` のヒーラー対応

4/25 のヒーラー射程内フィルタ修正で、ヒーラーが `_can_attack_target` の判定で「攻撃可能」とされる場面でのスキル不在を考慮しない問題が想定外発見として挙がった。ヒーラーが ATTACK strategy に入った時の挙動確認・修正が必要なら対応（アンデッド特効の Z 攻撃は持つので完全な攻撃不能ではない・要切り分け）。

##### 壁越し攻撃の禁止

現在は壁越しに攻撃が可能な状態。本来「壁越しは攻撃できない」が正しい仕様。射程判定にレイキャストまたは A* 距離を使う方向で修正検討。
...
- **`game_map.gd:_calc_party_strength()` 削除**：`max_hp` 合計を返す独自実装。grep で呼び出し元なし（4/28 確認）。`_combat_situation` 系の strength 計算（`PartyLeader._calc_stats`）と紛らわしい命名なので削除候補
...
- **Config Editor 全タブの棚卸し**：(a) 表示フィールドの網羅性（味方クラス・敵クラス・ステータス各タブ）、(b) hidden 候補（PartyLeader / NpcLeaderAI / SkillExecutor / Effect / Item 等）を定期確認
...
- **ログ参照の改善（OrderWindow のログ）**：現在は最新 50 件をそのまま表示するだけ。フィルタリング・検索・スクロール操作の改善を検討
- **NpcLeaderAI のアイテム収集動的切り替え**：目標フロアに到達している場合（余裕がある状態）、`item_pickup` を "passive"（近くなら拾う）から "aggressive"（積極的に拾う）に切り替える。装備強化のために能動的にアイテム回収する行動
- **ポーション自動使用（プレイヤー操作キャラ）**：現状は AI 操作キャラ向けのみ自動使用機構あり。プレイヤー操作キャラにも同等の自動使用を導入（操作中も `on_low_hp` / `sp_mp_potion` 指示に従う）
- **装備品による `move_speed` 補正**：現状 `move_speed` は装備補正の対象外。将来「重装で遅くなる」「素足で速くなる」等の追加余地

### 参照順序の推奨
1. 本ファイル冒頭「編集方針」で CLAUDE.md の編集ルール・docs/history.md との役割分担を確認
2. 「実装状況」「残タスク」（共に直上）で最近の仕様変更と次の作業を把握
3. 「アーキテクチャ方針」「設計原則」「パーティーシステムのアーキテクチャ」「AI と実処理の責務分離方針」で現行仕様の本体を確認
4. 「将来構想」（末尾近く）で長期検討事項を確認（必要時のみ）
...
**フロア間の階段ペアは「上下階で同じグローバル座標」を持つ前提**で配置する。

- F0 の `stairs_down` が (28, 5) にあるなら、F1 の対応する `stairs_up` も (28, 5) に配置する
- フロア間の往復は「踏んだ階段と同座標の階段マスに着地」する挙動が一意に保証される
- マップサイズが異なるフロア境界（現状 F3 ↔ F4）では座標が一致しないペアが残るが、その場合はコード側のフォールバック（`spawns[0]`）に委ねる暫定運用
- マップ拡張・新フロア追加時は、既存フロアと**同サイズ・同階段座標**で揃える

実装：[game_map.gd:_find_paired_stair_pos()](scripts/game_map.gd) が「踏んだ階段マスの座標と一致する新フロアの階段マス」を返す共通ヘルパで、4 経路（hero / パーティーメンバー / NPC リーダー / NPC 非リーダー）すべてで使う。これにより着地先選択ロジックが完全に対称化される。

### 階段部屋の配置規約

**上り階段部屋には敵を初期配置しない**（到着時の安全地帯として機能させる）。

- 適用範囲：全フロアの全上り階段部屋（`stairs.type == "stairs_up"` の `room` 全件）
- **下り階段部屋には敵を初期配置してよい**（適正フロアサイクル成立のため意図的に対象外）
- 「初期配置なし」の意味：JSON の該当 room に `enemy_party` キー自体を持たせない（空オブジェクトではなくキー欠落）
- 追跡してきた敵が後から進入することは妨げない（ランタイム挙動）

**設計意図**：プレイヤー / NPC がフロア間を遷移するとき、対応する階段マスは「修正 X」（4/28 後半）の着地占有チェックでブロック対象になる。上り階段部屋に敵を初期配置すると、敵が階段マス上で keep_distance / WAIT 等の挙動を取った瞬間に上階からの遷移を永久に塞ぐ事故が起きる。この事故を構造的に避けるため、上り階段部屋は「初期配置の段階で空ける」ことをマップデータの規約として固定する。

**マップ拡張時の注意**：フロアごとの敵パーティー数を増やす作業を行う場合、上り階段部屋（`stairs.type == "stairs_up"` の `room`）には敵を入れないことをチェックする。チェック手段：

```bash
python3 -c "
import json
with open('assets/master/maps/dungeon_handcrafted.json', encoding='utf-8') as f:
    data = json.load(f)
for fd in data['dungeon']['floors']:
    for s in fd.get('stairs', []):
        if s.get('type') != 'stairs_up': continue
        room = next((r for r in fd['rooms'] if r.get('id') == s['room']), None)
        ep = (room or {}).get('enemy_party') or {}
        cnt = len(ep.get('members', []))
        if cnt > 0:
            print(f'VIOLATION F{fd[\"floor\"]} {s[\"room\"]}: {cnt} enemies')
"
```
...
- 全パーティー状態をリアルタイム表示（0.2秒ごとに更新）
- サイズ：画面幅 85% × 高さ 85%（中央配置・単独表示なので従来の 55% 上半分制約から解放）
- 表示対象：プレイヤーパーティー（青系）→ NPCパーティー（緑系）→ 敵パーティー（赤系）の順
- **表示範囲**（2026-04-28 改訂）：
  - **プレイヤーパーティー**：全メンバー・全フロア表示（フロア降下中・フロア間追従中も全員表示）
  - **NPC パーティー**：全パーティー・全フロア表示（協力勢力の動向把握優先）
  - **敵パーティー**：追跡対象がいるフロアのみ表示（戦況集中）
  - 別フロアのメンバーは名前頭に `[Fx]` を付加（操作キャラのフロアを基準に注釈）
...
- **追跡対象**（2026-04-28 確立）：`_get_tracking_target()` ヘルパが真実源。`_selected_leader`（PSW で上下キー選択した対象）が有効ならそれ、無効なら操作キャラ（`_hero`）。敵パーティーのフロアフィルタ・F7 スナップショットの両方で本ヘルパを使用
- **追跡対象消失時のフォールバック**：`_process()` で `_selected_leader` の生存チェックを行い、freed/死亡なら自動でクリア + `leader_selected.emit(null)` でカメラを操作キャラに戻す
- F1 で閉じる／F2 で切替時は選択リセット・カメラは操作キャラの追跡に戻る
- `vision_system.debug_show_all` は PartyStatusWindow の可視状態と連動（上段表示中のみ未探索エリアも含めて全敵可視化）
- **タイトル行の表記**：追跡対象 == 操作キャラのとき `Floor: <N>  操作:<名前>@F<N>`、追跡対象が別キャラのとき `操作:<名前>@F<N>  追跡:<名前>@F<N>`
...
- **味方（プレイヤー・NPC）**：`[種別]  生存:x/y  area:<id>  戦況:<ENUM>(自軍×自軍HP率 / 敵×敵HP率 = 戦況比) 戦力: F=s(R+B) C=s(R+B) E=s(R+B) 戦力比:<ENUM> [目標F:N(NPCのみ・高)]  [mv=... battle=... tgt=... hp=... item=...](中以上)`
  - 例（NPC・詳細度「高+中」以上）：`戦況:EVEN(9.0×1.00 / 9.0×0.85 = 1.18) 戦力: F=14.1(14+0.1) C=13.1(13+0.1) E=3.0(3+0.0) 戦力比:OVERWHELMING 目標F:1  mv=追従 battle=攻撃 tgt=最近傍 hp=後退 item=積極的に拾う`
  - 例（詳細度「高のみ」）：`戦況:EVEN(...) 戦力: F=14.1(14+0.1) C=13.1(13+0.1) E=3.0(3+0.0) 戦力比:OVERWHELMING 目標F:1`（mv= 以降は中以上で表示）
  - `戦況:` の値は CombatSituation enum 名（OVERWHELMING / ADVANTAGE / EVEN / DISADVANTAGE / CRITICAL / SAFE）。`戦力比:` の値は PowerBalance enum 名（OVERWHELMING / SUPERIOR / EVEN / INFERIOR / DESPERATE）。2026-04-25 に日本語化を廃止し enum 名表示に統一
  - 2026-04-28 改訂：(a) 旧 `戦力:<PB enum> F(R+B)s ...` を `戦力: F=s(R+B) ... 戦力比:<PB enum>` に分離。strength 内訳と PowerBalance enum を別ラベルに切り出し用語衝突を解消。(b) NPC パーティー限定で `目標F:N` を追加（戦力 F と `FLOOR_X_Y_RANK_THRESHOLD` の比較結果・優先度「高」・0 始まり）。プレイヤーパーティーは自由意思でフロア移動するため非表示・敵パーティーには概念がないため非表示。値は `NpcLeaderAI._get_target_floor()` の結果を `_build_orders_part` で hint に乗せたもの。(c) 全体指示（mv/battle/tgt/hp/item）と敵 strategy を優先度「中」に降格（メンバー行の個別指示グループと優先度を揃える対称化）
  - `戦力:` の `F=s(R+B)` 内訳：F=full_party / C=nearby_allied / E=nearby_enemy。s=strength（HP 抜き・小数 1 桁）、R=rank_sum（素値）、B=`bonus_sum × ITEM_BONUS_STRENGTH_WEIGHT`（**WEIGHT 込みの strength 寄与値・小数 1 桁**）。**常に `R + B = s` が成立**（4/28 用語ガバナンス改訂で HP 率を strength から分離）。検算が見た目に通る
  - 戦況の括弧内：`<自軍戦力>×<自軍 HP 率> / <敵戦力>×<敵 HP 率> = <戦況比>` の形式で計算過程を完全可視化（戦力は HP 抜きなので、戦況計算で × HP 率を適用する素直な式）
  - HP 独立表示（旧 `HP:満(...)`）は廃止（2026-04-25・`HpStatus` enum 廃止に伴う）
  - `mv / battle / tgt / hp / item` は `_global_orders` の実値（OrderWindow 正規表記の日本語）。詳細度 1（高+中）以上で表示
  - NPC は `item=` を含む（プレイヤー経由の item_pickup 指示）。敵は `item=` なし（item_pickup 指示を持たないため）
- **敵**：`[種別]  生存:x/y  area:<id>  戦況:<ENUM>(...) 戦力: F=s(R+B) C=s(R+B) E=s(R+B) 戦力比:<ENUM>  [strategy=<ENUM>](中以上)`
  - 敵の `_global_orders.move` は `follow` で固定（`EnemyLeaderAI._build_orders_part()` で初期化・種族 AI が書き換えない規約）のため、ヘッダーには表示しない。動的に変化する個体実値はメンバー行の `move:<ラベル>` で表示する（2026-04-26 改訂方針）
  - `strategy=` は `PartyLeader._party_strategy` の素値（`ATTACK` / `FLEE` / `WAIT` / `DEFEND` / `EXPLORE` / `GUARD_ROOM`）。enum 名のまま表示。詳細度 1（高+中）以上で表示（味方の全体指示 mv= 等と対称・2026-04-28）
  - 敵の行動は UnitAI 指示フィールドではなく、パーティー戦略 + 種族フック + ハードコード種族ロジックで決まる（ただし `_move_policy` は per-member ロジックでリーダーの `stairs_*` / `explore` / `guard_room` 上書きや種族 AI 上書きで動的に変化する）
- リーダー名・クラス名は載せない（識別は `[種別]` + 色分け + メンバー行の `★` / `(クラス)` で行う）
- 優先度「高」（常に表示・戦力情報に集約）：戦況・戦力・戦力比・目標F（NPCのみ）
- 優先度「中」で追加：`mv=... battle=... tgt=... hp=... item=...`（味方）/ `strategy=...`（敵）/ `re:0.8s 探索:5`（`_reeval_timer` / `_visited_areas` サイズ）
- 優先度「低」で追加（NPC パーティーのみ該当・2026-04-25 にコード上のフィールド名表記に統一）：`joined_to_player:Y was_refused:N has_fought_together:Y has_been_healed:N suppress_floor_navigation:N`
- **避難先情報**（2026-04-25 改訂・味方パーティー限定・敵検知中のみ）：
  - 形式：`避難先:<area_id>` — `PartyLeader._flee_refuge_area_id` のみ表示
  - 2026-04-25 のリーダー推奨機構廃止に伴い、距離（d:N）・座標（@x,y）表示は撤廃
  - 各メンバー個別の出口は本人の `_last_flee_goal` から `flee→(x,y)` / `fb→(x,y)` 形式でメンバー行に表示
  - 敵不在・敵パーティー・避難先到達不能のときは表示なし
...
2. **目的**（常時・シアン）：` attack[ATKp]:Goblin[0.34s q3]` のように `UnitAI._state` / `_timer` / `_queue.size()` を付加。**2026-04-25 改訂**：日本語アクションラベル（`→攻撃` / `攻撃準備` / `逃走` / `距離確保` 等）はコード上のアクション名（`move_to_attack` / `attack` / `flee` / `keep_distance` 等）に統一。`get_debug_goal_str()` の戻り値が「コード上の識別子そのまま」になる方針
   - **2026-04-28 改訂（explore 目的地の可視化）**：
     - `move_to_explore:<area_id>(x,y)` — 目標タイルの area_id を座標に併記（例：`move_to_explore:r0_22(23,47)`）。目標タイルが階段（`STAIRS_DOWN` / `STAIRS_UP`）の場合は area_id ではなく `stairs_down` / `stairs_up` を維持（既存仕様）
     - `[explore]queue_empty(IDLE) target:<area_id>(x,y)` — `_move_policy = "explore"` でキュー空のとき、`_find_explore_target()` が次に選ぶであろう目標を peek 表示（例：`[explore]queue_empty(IDLE) target:r0_18(20,40)`）。実発行は次の receive_order で行われるため、表示は予測値・実値が異なるケースは原則ない（`_visited_areas` 等の状態が同フレーム内で変わらない限り）
     - area_id が空（壁・境界・未割当タイル）のときは座標のみ表示（`(x,y)`）でフォールバック
3. **移動方針プレフィックス**（詳細度 >= 1・黄緑・**敵味方共通**）：`  move:追従`
   - 全体指示だが、リーダーのみ `PartyLeader._assign_orders()` で層 2 専用値（`stairs_*` / `explore` / `guard_room`）に上書きされる。非リーダーは常に `_global_orders.move` を継承する（2026-04-26 per-member ロジック統一・3 層構造）
   - メンバー本人の `_move_policy`（UnitAI 側 per-member 値）を直読みしているため、リーダーは `move:↓階段` / `move:探索` / `move:部屋を守る` / 非リーダーは `move:追従`（`_global_orders.move` のベースライン値・NPC は `"follow"` 既定）のように行ごとに別の値が表示される
   - **2026-04-28 改訂**：`explore` は OrderWindow から削除されて層 2 専用値に純化。NPC 探索モード時にリーダーのみ `move:探索` 表示・非リーダーは `move:追従` 維持の対称形になる
   - **2026-04-26 改訂：敵メンバー行にも `move:` を表示する**（リーダーの `guard_room` 上書き・種族 AI の per-member 上書きを可視化するため。敵 `_global_orders.move` は `follow` 固定のため、ヘッダーの `mv=` ではなくメンバー行の `move:` でこそ動的値が観察できる）
   - 値域・規約は本セクション末尾の「`_global_orders.move` と `_move_policy` の値域規約」を参照
4. **指示グループ**（詳細度 >= 1・黄緑・**味方メンバーのみ**）：`  指示: T:リーダーと同じ C:攻撃 F:包囲 S:強敵なら使う H:瀕死度優先`
   ...
   - **2026-04-25 改訂**：`_shorten()` による省略形を廃止し、OrderWindow 正規表記の日本語ラベルに統一（`_label(key, val)` 経由）。文字数増による表示崩れは F3 詳細度トグルで対応する方針
   ...
   - ※ ただし `_global_orders.move` は per-member `_move_policy` 決定ロジック統一（2026-04-26）以降、敵の非リーダーも継承するため実質的に効くフィールドとなった。本値は敵ヘッダーに `mv=` として可視化済み。他の `_global_orders` フィールド（battle_policy / target / on_low_hp / hp_potion / sp_mp_potion / item_pickup）についても同様に「per-member 配布で敵に効く」可能性があるため、再調査が残タスク化されている（「優先度：中」）
...
8. **持ち物グループ**（詳細度 >= 2・茶系・**味方メンバーのみ**）※ 2026-04-28 追加：` [剣:bs4 鎧:bs2 盾:bs1] hp×2 mp×1`
   - 12 ステータスグループの直後に続けて表示（敵静的属性グループと排他位置・敵には出さないので順序衝突なし）
   - 装備セクション `[<装備種類>:bs<N> ...]`：装備中スロット（武器 → 防具 → 盾の順）の `bonuses` 辞書値合計を `bsN` で表示。空辞書（未装備）スロットは省略
   - 装備種類は日本語ラベル（剣 / 斧 / 弓 / ダガー / 杖 / 鎧 / 服 / ローブ / 盾）。マッピング不明な item_type は item_type 文字列をそのまま表示
   - ポーションセクション `hp×<N> mp×<N>`：所持ポーション数を 0 本でも常時表示（略号は `mp`、`sp_mp` ではなく）。`potion_heal` → hp / `potion_energy` → mp。同じ item_type 複数行は `quantity` を加算
   - **目的**：戦力表示 `F=s(R+B)` の装備 `B`（WEIGHT 込み寄与値）の元になる装備内訳を可視化し、ポーション残量も合わせて確認するためのデバッグ補助

**実装面**：セグメント単位（行動ボディ / 目的 / 指示プレフィックス + 各フィールド / 種プレフィックス + 敵動的判断 / 状態プレフィックス + 各フラグ / 各ステータス部品 / 持ち物 / 敵静的属性）でチャンク化し、行幅を超えたセグメント境界で自動折返しする。折返し後の継続行ではセグメント先頭の空白を削除して頭から描画する。
...
- **出力内容（本体ファイル）**：
  - 区切り線 + ヘッダー部（時刻・操作キャラフロア・操作キャラ・追跡対象・ゲーム速度）
  - プレイヤーパーティー → NPC パーティー → 敵パーティー（画面表示と同順序・同表示範囲：味方は全フロア・敵は追跡対象のフロア）
  ...
  - **追跡対象** ヘッダーには `<名前>（<クラス>）@F<N> <area_id>` を記録（後で見返したときに「これは誰を追跡していたログか」が分かるように・追跡対象が操作キャラと同じなら「同上」）
...
### 用語ガバナンス（2026-04-28 確立）

仕様書・コード・デバッグ表示・ConfigEditor 説明文の 4 ソース間で用語を一貫させるためのルール。
過去に「戦況・戦力・戦力比」の 3 指標で関係性が崩れていた経緯（戦力比はランク和のみ・戦力は HP 込み）から、再発防止のために明文化する。

**1. 同じ言葉を異なる意味で使わない**

例：「戦力比（ランク和のみ）」のような記述は「戦力比は複数の意味があり、その一種が『ランク和のみ』」と誤読される。「戦力比」が一意の定義（ランク和のみ）であることを明示する。括弧書きで補足するときも「定義の要約」であって「複数バリアントの一つ」ではないことを文脈で示す。

**2. 用語間の関係を素直に成立させる**

例：「戦力」と「戦力比」は「比」という関係性を持つので、計算式も整合させる。「戦力比」がランク和ベースなら「戦力」もランク和ベースで定義する。瞬間的な力関係を表現したい場合は別の用語（「戦況」など）を導入し、「戦力 × HP 率」のように既存用語を流用しない。

**3. 仕様変更時の用語整合性チェック**

新しい指標・概念を追加するとき、既存の用語との関係を意識する。同じ語幹（戦・力・況・比など）を共有する用語間で計算式の整合性が取れているかを確認する。整合しないなら新しい用語を導入する。

**4. CLAUDE.md / 実装 / デバッグ表示 / ConfigEditor 説明文の 4 者整合**

仕様変更時はこの 4 ソースのすべてで定義・計算式・表示を同期させる。1 ソースだけ更新して他が古い状態にならないよう、変更時のチェックリストとして 4 者を明示的にレビューする。

### UI の表示原則
- **アイテム関連表示は閲覧キャラのクラスで絞らない**：他メンバーに渡す操作があるため、アイテムの全 stats / effect を常に表示する
- **stats 辞書は全キー反復**：固定キーリストでフィルタせず、将来の新ステータス追加に自動対応する

### フロア番号と area_id の表現規約（2026-04-25 確定）

#### 内部表現

- `floor_id` / `current_floor` / `_current_floor_index` / `floor_idx`：**0 始まり**
  （配列インデックスとしてプログラミングの慣例に従う）
- `_all_map_data` / `_all_floor_data` / `_per_floor_enemies` / `_per_floor_npcs` / `_floor_visited`：
  floor_id（0 始まり）をキーまたはインデックスとして使う
- `MapData.get_refuge_area_ids(floor_id)`：引数は 0 始まり
  - `floor_id == 0` → 安全部屋エリア
  - `floor_id >= 1` → 上り階段エリア
- `GlobalConstants.FLOOR_0_RANK_THRESHOLD`〜`FLOOR_4_RANK_THRESHOLD`：
  配列インデックス対応（0 始まり）

#### JSON（dungeon_handcrafted.json）

- `"floor": 0〜4`（0 始まり・内部 floor_id と一致）
- area_id 命名規則：`r{floor_id}_{room_number}`（0 始まり・例：`r0_10`）
- JSON の `floor` フィールドは single source of truth として機能する（2026-04-25 以降）

#### UI 表示の値変換ルール

**プレイヤー向け UI は `+1` して 1 始まり表示**：
- `area_name_display.gd:51` — フロアタイトル「第N階層」
- `game_map.gd:1543` / `game_map.gd:2086` — メッセージ「dF」
- `main_menu.gd:507` — セーブスロット表示「Floor N」

**開発者向けデバッグ UI は内部値そのまま（0 始まり）**：
- `PartyStatusWindow`（ヘッダー・メンバー別フロア表示・スナップショット）
- `CombatLogWindow` 系のログ：
  - `game_map.gd:1685` / `1744`（`[NPC遷移]` ログ）
  - `npc_leader_ai.gd:168-175`（`[NPCフロア判断]` ログ。`次F%d` は次フロアの 0 始まり内部インデックスを表示）
  - `npc_leader_ai.gd:581`（`[合流判定]` ログ）
  - `party_manager.gd:561`（`[NPC死亡]` ログ）

#### 判断基準

新しく UI で floor を表示する場面では、
「プレイヤーが通常プレイ中に見る UI か、開発者がデバッグ中に見る表示か」で
+1 するか否かを決める。迷ったら本規約の「プレイヤー向け／開発者向け」分類を参照。
...
### per-member `_move_policy` 決定ロジック（3 層構造・2026-04-26 確立）

各メンバーの `_move_policy` は `PartyLeader._assign_orders()` で以下の 3 層階で決定する：

| 層 | 内容 | 適用範囲 |
|---|---|---|
| 層 1 | ベースライン：`_global_orders.move`（全体方針） | 全パーティー全メンバー均一 |
| 層 2 | リーダー上書き：モード固有値（`stairs_*` / `explore` / `guard_room`） | リーダーのみ・joined_to_player は対象外 |
| 層 3 | 種族固有上書き：種族 AI のフックで差し替え | 敵 AI 種族別実装（任意） |

**設計思想**：

- リーダーだけがモード固有の移動目的を持つ。非リーダーは全体方針 `_global_orders.move` をそのまま継承する
- 階段ナビ・探索・縄張り帰還といったリーダーの判断結果を、非リーダーの `_move_policy` に伝播させない（`cluster` ハードコード等の per-member 上書きは禁止）
- `_global_orders.move` は固定値ではなく可変（OrderWindow 操作 / NPC ベースライン / 敵ベースラインで決まる）。将来 `"cluster"` 等に切り替えても本構造は同じ思想で動作する

**`_global_orders.move`（全体指示・パーティー全体均一）の値域**：

`follow / cluster / same_room / standby`（OrderWindow 4 値に限定）

- 味方：プレイヤーが OrderWindow で設定する（NPC は現在 `follow` 固定）
- **敵：`follow` で固定**（`EnemyLeaderAI._build_orders_part()` で初期化・種族 AI が書き換えない・将来も書き換えない方針）

**`_move_policy`（per-member 実値）の値域**：

OrderWindow 4 値 ＋ `stairs_down / stairs_up / guard_room / explore`

- ベースラインは `_global_orders.move` から継承（層 1）
- リーダーは状況に応じて `stairs_*` / `explore` / `guard_room` に上書きされる（層 2）
- 種族 AI が必要に応じて per-member 上書き（層 3・敵のみ）

**`explore` 値の意味（重要・2026-04-28 確立）**：

`explore` は **リーダー個別行動専用の値**である（`stairs_*` / `guard_room` と同じく層 2 専用）。OrderWindow 4 値（層 1 ベースライン）には含まない。

旧実装（〜2026-04-27）では OrderWindow 5 値の 1 つとして「パーティー全員がバラバラに探索」の意味でも使われていたが、層 1 と層 2 で同じ文字列が混在する設計上の歪みを解消するため、OrderWindow からは削除した。「全員バラバラ探索」の指示自体が廃止された結果、非リーダーが独自に `_find_explore_target` を呼んで分散する隊形崩壊の温床も解消された。`NpcLeaderAI._build_orders_part()` の `hint["move"] = "explore"` 書き込みも併せて撤去（4/28）。

**`guard_room` の位置づけ**：`stairs_*` / `explore` と同じ扱い。リーダー固有のモード固有値・全体指示としては設定不能・per-member の `_move_policy` でのみ存在。
...
### リーダー本人の `_move_policy` 自己参照回避（2026-04-26 確立）

層 2 のリーダー上書きが空振りすると層 1 のベースライン `_global_orders.move = "follow"` がリーダー本人に継承され、`formation_ref` 構築ロジックで以下のいずれかになる：

- hero リーダー（player party）：`_leader_ref = self`（**完全な自己参照**）
- NPC リーダー：`_leader_ref = null`（参照点なし）
- 敵リーダー：`_leader_ref = null`（参照点なし）

UnitAI 側の防御コード（`_leader_ref == _member` / `_leader_ref == null` チェック）でクラッシュは回避されるが、`_formation_satisfied()` が常に true を返してリーダーが立ち止まる**退化挙動**になる。target 側と対称な「リーダー個別判断で上書き」パターンで解消する。

#### 仕組み

層 2 上書きの最優先フック `_decide_leader_move_override()` を活用し、味方リーダーには `"explore"` をフォールバック値として返す：

- `PartyLeaderPlayer._decide_leader_move_override()` → **常に `"explore"`**
  - プレイヤーパーティーは `_party_strategy` を持たないため `_is_in_explore_mode()` / `_is_in_guard_room_mode()` がいずれも false を返す。本フックで上書きしないと層 1 ベースライン `"follow"` がリーダーに継承される
  - **階段判断は行わない**：プレイヤーパーティーの階段移動はプレイヤーの意思で行うため、NPC のような `_get_explore_move_policy()` 経由の `stairs_*` 判定は入れない。`explore` 固定で「フロアを歩き回る」挙動のみ
- `NpcLeaderAI._decide_leader_move_override()` → **ラダー判定で常に非空値を返す（2026-04-28 改訂・案 G）**
  - 優先順位（上から判定）：
    1. `battle_policy == "retreat"` + `current_floor >= 1` + `current_area not in refuge_areas` → `"stairs_up"`（フロア間 FLEE 経路の起点）
    2. `target_floor < current_floor` → `"stairs_up"`（HP 補正等でフロア降格・retreat 中の refuge area 内も該当）
    3. `target_floor > current_floor` → `"stairs_down"`（戦力余裕で昇格）
    4. それ以外 → `"explore"`（敵検知中も含む通常時のフォールバック・層 1 `follow` 退化防止）
  - 旧実装は敵検知中に常に `"explore"` を返していたため、退避路（stairs_up）を上書きしてマリアケース（F1 retreat で停止）を引き起こしていた
  - 新実装で `_get_explore_move_policy()` 経路は事実上使われない（本フックがラダー判定を内包）。基底 `_assign_orders` の `elif _is_in_explore_mode()` 分岐は敵リーダー専用に残置

#### プレイヤー操作中の競合回避

プレイヤーが直接操作中の hero では `UnitAI._process()` が `is_player_controlled` で早期リターンするため、本フックの値（`_move_policy = "explore"`）は AI 動作に反映されない（PlayerController と競合しない）。操作キャラ切替で hero が AI 操作になった瞬間から explore 行動が発火する。

#### 敵リーダーは今回スコープ外

`EnemyLeaderAI._decide_leader_move_override()` は area_id ベース guard_room 判定で `""` を返した場合、層 1 `"follow"` がリーダーに継承される（縄張り外で追跡中など）。敵リーダーは `_leader_ref = null` で自己参照ではないが、NPC と同様に立ち止まる退化挙動が起きうる。実機検証案件として残置（`EnemyLeaderAI` 基底に `"chase"` 等の独自フォールバック値を入れるか、`"explore"` を流用するかは実プレイ観察で判断）。
...
- `_evaluate_strategic_status()`: 統合戦略評価ルーチン。戦力計算と戦況判断を 1 箇所に集約し、3 種類のメンバー集合で 1 度ずつ統計を算出する（重複計算なし）：
  ...
  - 戦力 = `rank_sum + bonus_sum × ITEM_BONUS_STRENGTH_WEIGHT`（HP 抜き・4/28 用語ガバナンス改訂）
  - 戦況比 = `(my_strength × my_hp_ratio) / (enemy_strength × enemy_hp_ratio)`（戦況計算で別途 × HP 率を適用）
  - bonus_sum = 各メンバーの装備 bonus 段階合計の平均をパーティー全員で合計（装備本数差をクラス間でフラットに吸収）
  - HP 率：自パ側は実 HP + ポーション、他パ・敵は condition ラベルから推定（敵ステータス直接参照禁止ルール準拠）。strength とは独立に `*_hp_ratio` フィールドとして辞書に保持
...
### FLEE / fall_back の逃走先決定ロジック（2026-04-25 改訂：自律判断に統一）

味方パーティーの FLEE / fall_back は、メンバーが自律的に出口を選定する設計。リーダーは「最寄り避難先エリア ID」のみを配布する。敵パーティーは現状スコープ外（旧 `_find_flee_goal_legacy`：脅威から 5 タイル離れる実装）。

> 旧設計（2026-04-21〜2026-04-25 朝）はリーダーが推奨出口タイル座標を決定して全メンバーに配布する二段階構造だったが、メンバーがリーダーより先行した瞬間に「推奨タイルがメンバーの `exit_tiles` リストに含まれず座標一致判定が常に偽」となり境界往復するバグが発覚。リーダー推奨機構を廃止して自律判断に統一した。経緯は docs/history.md 参照。

#### `combat == "flee"` 指示の意味論

`combat == "flee"` 指示中は、**敵の有無に関わらず避難先方向への退却を継続**する（明示指示尊重型）。旧仕様では「敵が見えれば flee・見えなければ wait」の自動応答型ロジックを採用していたが、敵検知が事実上部屋単位で動作し境界往復・WAIT 切替で前進中断する温床になっていたため変更した（2026-04-25）。

flee 状態の解除はメンバー側で能動的に行わない。以下のいずれかで自然に解除される：

1. **個体 FLEE（`on_low_hp == "flee"` 由来）**：HP が `critical` 状態を脱せば発動条件外れで解除
2. **パーティー FLEE（`battle_policy == "retreat"` 由来）**：リーダーの `battle_policy` が `retreat` → `attack` に戻る
   - NpcLeaderAI：`_evaluate_and_update_battle_policy()` が戦況 SAFE 判定で自動切替
   - プレイヤー操作：OrderWindow で手動切替

なお「flee 解除」と「flee 中の移動先 = 現在地（前進不要）」は別概念。前者は状態そのものが終わること、後者は flee 状態を維持したまま移動だけ停止すること。

#### メンバー自律出口選択（`UnitAI._evaluate_exit_costs()`）

各メンバーが自分の現エリアの全外側出口を列挙し、出口ごとに以下の総合コストを計算する：
...
total_cost = reach_cost + refuge_dist_cost
  ...
  refuge_dist_cost = get_area_distance(area_of(exit_tile), _flee_refuge_area_id)
                     ...
                     避難先未設定なら 0・到達不能なら 999.0 ペナルティ
```

最小 `total_cost` の出口を採用する。

##### 設計意図（パーティー分散防止と脅威回避の両立）

- **脅威がない場合**：`reach_cost` が出口間でほぼ等しく、`refuge_dist_cost` が支配的になる → 全メンバーが避難先方向の同じ出口に収束（パーティー分散しない）
- **脅威がある場合**：`reach_cost` で経路上の敵を避ける別出口を選べる柔軟性 → 各メンバーが独立に最適化
- リーダー位置は不要：各メンバーが自分の現エリアから自律判断する

#### リーダー側の役割（避難先エリア決定のみ）

- `PartyLeader._update_flee_refuge_area_id()` が `_assign_orders()` ごとに更新
- 現フロアの避難先エリア一覧（`MapData.get_refuge_area_ids(floor)`）から、リーダー現エリアに最も近いものを **BFS ホップ距離**で選定
- `_flee_refuge_area_id` に格納し、`receive_order` 経由で全メンバーに配布
- 敵検知中の味方パーティーで常時計算（FLEE / fall_back のいずれでも使用）。敵パーティー・敵不在は `""`（リセット）

#### flee 中の移動先決定（前進不要判定）

flee 中の移動先は `UnitAI._find_flee_goal()` で決定する。フローは以下：

1. **避難先到達判定**：`_is_in_refuge_area()` が真なら現在地を返す（移動先 = 現在地・既存）
2. **脅威距離判定**（2026-04-25 追加）：`_calc_nearest_threat_distance() >= FLEE_SAFE_DISTANCE` なら現在地を返す（移動先 = 現在地・前進しない）
3. それ以外は `_evaluate_exit_costs()` で全出口の総合コストを計算し、最小コストの出口を返す

`_calc_nearest_threat_distance()` は同フロア・対立陣営・生存中のキャラクター全員から最近接のマンハッタン距離を返す。脅威が存在しない場合は INT_MAX 相当（前進不要扱い）。

**設計思想**：flee 中は「脅威から距離を取った避難先方向の位置」を移動先として選ぶ。脅威から十分離れた後は移動先 = 現在地となり、結果として停止する。これは「停止条件」ではなく「移動先決定の自然な帰結」として実装する。flee 状態自体は維持されるため、敵が再接近すれば次の `_find_flee_goal()` 呼び出しで再び出口選択が走る。

「敵の向こう側に安全部屋がある」ようなレイアウトでは、`reach_cost` に内包される脅威コストにより、敵に向かう方向の出口は選ばれない。脅威がない方向で避難先までの距離が最短になる出口が選ばれる。

#### fall_back との関係

`UnitAI._find_fall_back_goal()` は `_find_flee_goal()` と**同じコア関数 `_evaluate_exit_costs()`** を共用する。違いは停止条件のみ：

- **FLEE**：避難先エリアに到達するまで連鎖移動（`_is_in_refuge_area()` で待機判定）+ `FLEE_SAFE_DISTANCE` 以上で前進不要
- **fall_back**：脅威との距離が `attack_range + FALL_BACK_MARGIN` 以上なら停止（`_is_far_enough_from_threats()`）

両者は別関数・別定数で独立。fall_back は「戦闘継続しつつ射程外まで下がる」、FLEE は「戦闘から離脱して安全な距離まで下がる」と意味が異なる。

#### keep_distance との関係（2026-04-25 専用ロジックに分離・2026-04-27 敵縄張り考慮追加）

`UnitAI._find_keep_distance_goal()` は flee / fall_back とは**別ロジック**。違い：

- **flee / fall_back**：パーティーレベルの方向性（避難先エリア）に従う移動
- **keep_distance**：個別の戦闘位置（自分の攻撃ターゲット）に従う距離維持

実装：ターゲットからのマンハッタン距離が `[KEEP_DISTANCE_MIN, KEEP_DISTANCE_MAX]` の同心円帯から候補タイルを列挙し、`threat_cost + reach_cost × KEEP_DISTANCE_REACH_WEIGHT + home_dist_cost` 最小のタイルを選ぶ。候補なしなら現在地に留まる。

使用箇所：rear 隊形の全味方クラス（基底 `_generate_queue` の rear 分岐）+ GoblinArcher / Salamander（種族 AI override）。

##### 敵専用：縄張り距離ペナルティ（2026-04-27）

`is_friendly == false` のメンバー（敵側 keep_distance）には `home_dist_cost` を加算：

- `home_area_id` = `PartyManager._room_id`（初代スポーン位置の area_id）を `get_parent().get_parent()` 経由で取得
- 候補タイルが属するエリア（`MapData.get_area(tile)`）と `home_area_id` の **BFS 距離**（`MapData.get_area_distance`）を計算
- `home_dist_cost = area_dist × KEEP_DISTANCE_HOME_WEIGHT`
- 縄張り内（`area_dist == 0`）/ home_area 空 / 候補エリア空 / 到達不能（`area_dist == -1`）はペナルティ 0

**設計意図**：プレイヤーが弓兵を追跡する状況で、毎サイクル keep_distance のリングがプレイヤー中心に再計算される結果、弓兵が縄張りから少しずつ引きずり出される現象を防ぐ。縄張り内に候補があれば必ず縄張り内を優先し、プレイヤーが詰めた場合のみ縄張りに最も近い隣接エリアの候補から選ぶ自然な fallback。

味方 rear 隊形には適用しない（`is_friendly == true` ガード）。「賢い敵は縄張りを考慮する」という設計感覚と整合。

#### 脅威コスト（`_calc_threat_cost` / `_build_threat_map`）

- 自分と対立陣営（`is_friendly != _member.is_friendly`）の生存キャラ・同フロアを対象
- 各敵について `max(0, FLEE_THREAT_RANGE - manhattan) × FLEE_THREAT_WEIGHT` を合計
- A* 内の毎ステップ呼び出しを避けるため `_build_threat_map()` で事前計算（Dictionary 参照に簡略化・~150 体 × ~41 タイル ≒ 6,150 ops）
- 敵ステータス直接参照禁止ルール準拠（位置と `is_friendly` のみ参照）

#### 避難先到達後の挙動

- 避難先エリア（安全部屋 / 上り階段エリア）に到達したメンバーは `_is_in_refuge_area() == true` となる
- **WAIT 戦略の wait 特例**（[unit_ai.gd:889](scripts/unit_ai.gd:889)）：`_combat=="flee"` + `_is_in_refuge_area()` + `_move_policy not in ["stairs_up","stairs_down"]` で wait を返す（避難完了後はそこで停止）
- **階段ナビ中は wait をスキップ**（2026-04-28 改訂・案 G）：`_move_policy in ["stairs_up","stairs_down"]` のとき wait 特例は除外され、`_generate_move_queue()` 経由で `_generate_stair_queue()` が動く。F1+ で `battle_policy=retreat` のリーダーは [npc_leader_ai.gd:_decide_leader_move_override()](scripts/npc_leader_ai.gd:138) でラダー判定により `"stairs_up"` 上書きが入るため、refuge エリア進入時も停止せず階段タイルへ進み、フロア遷移してさらに上階の避難先へ向かう連鎖が成立する
- FLEE(1) 戦略の wait 特例（[unit_ai.gd:808](scripts/unit_ai.gd:808)）も refuge エリア到達時に wait を返すが、`_determine_effective_action()` が refuge 内では WAIT(2) を返すため通常経路では発火しない（防御的コード）

#### 関連定数（Config Editor UnitAI カテゴリ）

| 定数 | 既定値 | 説明 |
|---|---|---|
| `FLEE_THREAT_RANGE` | 5 | 敵から何マス以内を危険とみなすか（脅威コスト計算用） |
| `FLEE_THREAT_WEIGHT` | 3.0 | 危険マス 1 つあたりのコスト加算量 |
| `FLEE_AREA_DISTANCE_WEIGHT` | 10.0 | 出口が属するエリア → 避難先の BFS 距離係数（パーティー分散防止のウェイト） |
| `FLEE_SAFE_DISTANCE` | 8 | 最近接脅威との距離がこの値以上なら前進不要（flee 中の移動先 = 現在地） |
| `KEEP_DISTANCE_MIN` | 3 | rear 隊形の理想距離下限（ターゲットからのマンハッタン距離・これ未満で下がる） |
| `KEEP_DISTANCE_MAX` | 4 | rear 隊形の理想距離上限（これ超で近づく） |
| `KEEP_DISTANCE_REACH_WEIGHT` | 0.5 | keep_distance goal 選定時の到達コスト重み（小さいほど安全タイル優先） |
| `KEEP_DISTANCE_HOME_WEIGHT` | 10.0 | keep_distance goal 選定時の縄張り距離ペナルティ重み（敵専用・味方 rear では無効）。BFS 距離 × この値を加算 |
...
- `get_exit_tiles_from(area_id: String) -> Array[Vector2i]`：エリアの**外側出口タイル**一覧（area_id とは異なるエリアに属するタイルのうち、4 近傍に area_id のタイルがあるもの = 一歩踏み出した先のタイル）。2026-04-25 に内側 → 外側へ定義変更。理由は項目末尾参照
- `get_adjacent_area_ids_of_exit(exit_tile: Vector2i) -> Array[String]`：外側出口タイルが属するエリア ID（= 向こう側エリア）。外側前提では各タイルが 1 エリアにしか属さないため、戻り値は最大 1 要素（旧 API 互換のため Array[String] を継続）

##### 出口タイル定義変更の経緯（2026-04-25）

旧定義（内側）では、メンバーが FLEE goal（出口タイル）に到達してもエリア ID が変わらず、
連鎖再評価のトリガーが発火しないまま立ち往生する問題があった（部屋を 1 つしか抜けられない）。

外側定義に変えたことで、goal 到達 = エリア境界跨ぎになる。各メンバーが新エリアに入った
時点で `UnitAI._evaluate_exit_costs()` が新エリアの出口を再評価するため、連鎖走行
（部屋 → 通路 → 部屋 → … → 安全部屋）が自律的に成立する（2026-04-25 のリーダー推奨機構
廃止後はメンバー自律判断に統一されたため、エリア変化通知は不要になった）。

A* との整合性：`_astar_with_cost(..., area_limit_id=current_area)` は中間タイルが
area_limit_id 外ならスキップするが**ゴール自体は例外的に許可**するため
（`unit_ai.gd` の `neighbor != goal` 分岐）、外側 goal でも経路探索は成立する。
...
#### target 死亡後の即時再選定（2026-04-26 確立）

`receive_order()` 内で「ATTACK 戦略 + ordered_target が null（freed）」のとき、親 `PartyLeader` の `select_target_for(_member)` 公開 API を呼んで即時再選定する。

**経緯**：attack 中に target が死亡すると以下の経路で問題が起きていた。

1. attack キューが消化される（target invalid で `_complete_action`）
2. `_queue` 空 → `_process` IDLE 分岐 → `receive_order(_order)` 再発火
3. `_order["target"]` は freed なオブジェクト → `is_instance_valid` false → `ordered_target = null`
4. `_determine_effective_action()` は `_combat_situation` 等から ATTACK(0) を返す（戦況スナップショットは古い・1.5 秒前）
5. `_generate_queue(0, null)` が `target == null` 経路で `_generate_move_queue()` に流れる
6. `_move_policy = "explore"`（NPC リーダーの 4/26 fix）→ `_generate_explore_queue` → **move_to_explore** 発行
7. リーダーが部屋を出て探索開始（同部屋に他の敵が残っていても素通り）
8. PartyLeader._assign_orders の次サイクル（最大 1.5 秒後）で target = B が再選定されて attack 復帰 → **無駄歩き発生**

**対策**：`receive_order` で `_determine_effective_action()` の直後に再選定を試みる。

```
if ordered_target == null and new_effective == 0:  # ATTACK 戦略限定
    var leader_node := get_parent()
    if leader_node != null and leader_node.has_method("select_target_for"):
        var fresh: Variant = leader_node.call("select_target_for", _member)
        if fresh != null and is_instance_valid(fresh):
            ordered_target = fresh as Character
            _target = ordered_target
            _order["target"] = ordered_target  # 次の再発火で stale を拾わない
```

**設計判断**：

- 対象戦略は `ATTACK(0)` のみ。FLEE / FALL_BACK / WAIT は target 不要 or null 許容なので再選定不要
- `select_target_for` は `PartyLeader` に新設した公開ラッパー（内部は `_select_target_for` に委譲・サブクラス override 経路は変えない）
- 再選定で null が返れば（本当に敵不在）従来通り `_generate_move_queue` に流れる（自然な explore 復帰）
- `_order["target"]` も同時に更新するため、次の `receive_order(_order)` 再発火でも同じ stale target を拾わない
- `撤退指示（battle_policy="retreat"）` 中は `_combat="flee"` で `_determine_effective_action` が FLEE(1) を返すため再選定は発火しない（副作用なし）
- 全パーティー種別の UnitAI で動作する。`PartyLeaderPlayer` / `NpcLeaderAI` / `EnemyLeaderAI` 派生のいずれの `_select_target_for` も呼ばれる経路
...
|---|---|---|---|---|
| `keep_distance` | 距離確保 | ターゲット中心の距離維持（戦闘継続） | 基底 `_generate_queue` の rear 隊形分岐・種族 AI override | rear 隊形の全味方クラス + GoblinArcher / Salamander |
| `fall_back` | 後退 | 避難先方向への後退（戦闘継続・HP 低下時の選択肢） | 基底 `_generate_queue` の `strategy == _STRATEGY_FALL_BACK(3)` 分岐 | `on_low_hp = "fall_back"` 設定のメンバー（デフォルト・状態が `critical` 時） |
| `flee` | 逃走 | 戦闘から離脱する（パーティーから離れて安全な場所へ） | 基底 `_generate_queue` の `strategy == 1(FLEE)` 分岐 | `on_low_hp = "flee"` / `combat = "flee"` / 敵 `_party_strategy == FLEE` / ゴブリン系 HP < 30% 自己逃走 |

- **実行ロジック**（2026-04-25 改訂）：
  - `flee` → `_find_flee_goal()`（味方：共通コア `_evaluate_exit_costs()` で最小コスト出口採用 / 敵：legacy 直線）
  - `fall_back` → `_find_fall_back_goal()`（味方：同じ `_evaluate_exit_costs()` を共用 + 射程外判定 `_is_far_enough_from_threats()` で早期停止 / 敵：legacy 直線）
  - `keep_distance` → `_find_keep_distance_goal()`（ターゲットからのマンハッタン距離が `[KEEP_DISTANCE_MIN, KEEP_DISTANCE_MAX]` のタイル群から脅威コスト最小を選ぶ。`_evaluate_exit_costs()` とは別ロジック。flee / fall_back はパーティー避難先方向、keep_distance はターゲット中心と思想が異なる）
- **rear 隊形の動作**（2026-04-25 改訂）：理想範囲内 + 射程内なら attack、それ以外は keep_distance。「後衛前進問題」（敵接近時に下がるロジックなし）を解消。近接クラスを rear 指定すると理想範囲内でも攻撃できない（仕様・プレイヤー責任）。**味方 rear と敵 keep_distance は同じ `_find_keep_distance_goal()` を共用するが、敵側のみ縄張り距離ペナルティが加算される**（`is_friendly == false` ガード・2026-04-27）
...
- 低詳細度の UnitAI フラグ行に `kd→(x,y)` / `fb→(x,y)` / `flee→(x,y)`（メンバー本人の `_last_flee_goal`・味方のみ）を表示。現在のキューアクションでラベルが切り替わる
...
Phase 14（Steam 配布準備）でバランス調整を本格化する前に認識しておくべき、リファクタリング由来の影響と観察事項：

#### 敵が盾で防御するようになった
- SkillExecutor 統一（4/18 完了）により、Player 側でしか機能していなかった防御判定ロジックが AI 側でも正しく機能するようになった
...
#### Player / AI の計算式統一完了
- 4/18 より前に調整した値は、片方でしか機能していない可能性がある（`damage_mult` / `duration` / `heal_mult` / AI の水弾未判定 等）
...
- **戦況評価閾値**：`COMBAT_RATIO_*` / `POWER_BALANCE_*`（定数タブ > PartyLeader カテゴリ）。HP 関連は段階 enum 廃止のため用途別 float 定数（`SPECIAL_SKILL_DISADVANTAGE_HP_RATIO` 等）で個別管理
...
### ヒーラーの行動規約（2026-04-25 確立）

ヒーラーは他クラスと同じく `combat` 指示（`attack` / `defense` / `flee`）に従って動作する。ただし以下の非対称性を持つ：

- **射程内の対象だけを回復する**：射程外の回復対象を追って移動しない。`Character.attack_range`（装備補正込み・最終値）と `_manhattan(_member.grid_pos, target.grid_pos)` を比較して判定（同フロアのみ対象）
- **戦闘行動のために移動しない**：移動は move 指示（`follow` / `cluster` / `explore` 等）または combat 指示（`flee` 時の避難先方向）に従う
- **回復は移動と並行**：`combat` 値に関わらず、射程内に対象がいれば回復行動が発動する。自分自身も対象に含む（距離 0 で常に射程内）。撤退中（`combat == "flee"`）も射程内対象を回復し続ける
- **回復優先**：射程内にアンデッド敵 + 回復対象が同時にいた場合、回復を優先する（`_generate_heal_queue()` がアンデッド攻撃判定より前に走る）

通常はリーダー追従（`move == follow` 等）により自軍メンバーが自然と射程内に入る設計。射程外の味方を救えないケースは現状の仕様の自然な帰結として受容する（プレイヤーが追従指示を維持する責務）。

実装：[`unit_ai.gd:_find_heal_target()`](scripts/unit_ai.gd) の各 `heal_mode` 分岐に `_is_in_heal_range()` フィルタを通す。射程内に対象がいなければ `null` を返し、`_generate_queue` の通常フロー（move 指示・FLEE 戦略・WAIT 戦略）に合流する。
...
**1 人パーティー**：**主人公の役割分類に応じて 4 枠**に動的決定する（`SOLO_PARTY_SLOTS_BY_HERO_CATEGORY` 定数）。主人公と同じ分類を 1 つ減らすことで、味方陣営全体での分類バランスを保ち、NPC 合計を 14 人（4 + 2×2 + 2×3）に抑える設計。

| 主人公分類 | 1 人 P 枠 | 1 人 P 数 |
|---|---|---|
| `MELEE`（剣士・斧戦士・斥候） | 近 1 + 遠 2 + 癒 1 | 4 |
| `RANGED`（弓・火魔・水魔） | 近 2 + 遠 1 + 癒 1 | 4 |
| `HEALER` | 近 2 + 遠 2 + 癒 0 | 4 |

JSON 側の 1 人パーティーエントリは **5 個のまま維持**（座標を残すため）。コード側でフロア単位にシャッフルしてから先頭 1 つをスキップ（毎起動で空になる位置がランダム）し、残り 4 つに上記スロット順で割り当てる。スキップされたパーティーは `class_assignments[idx] = []`（空配列）として返り、`_setup_floor_npcs()` 側でスポーン処理がスキップされる。

各分類内のクラス選択：
- `MELEE` 枠：3 クラス（sword/axe/scout）から重複なしランダム選択
- `RANGED` 枠：3 クラス（archer/fire/water）から重複なしランダム選択
- `HEALER` 枠：単一クラス固定
...
### NPC の階段位置の認識モデル（2026-04-09 確立・視界ベース）

NPC は階段の位置を**最初から知ってはおらず、訪問済みエリアに含まれる階段のみを目標にできる**。地図持ち（god mode）にしたい場合は `GlobalConstants.NPC_KNOWS_STAIRS_LOCATION` を `true` に切り替える（デバッグ・比較用、既定 `false`）。

旧仕様（2026-04-09 以前）は階段位置を全 NPC が既知の前提で動いていた。`993997d` で視界ベースに変更し、同時に階段を各方向 6 か所に増設してフロア全体に分散配置した（NPC が探索すれば必ずどれかを発見できる構成）。

#### 2 層判定構造

NPC リーダーの `_move_policy` 決定（[npc_leader_ai.gd:204](scripts/npc_leader_ai.gd:204) `_get_explore_move_policy()`）と、実際の階段経路探索（[unit_ai.gd:938](scripts/unit_ai.gd:938) `_generate_stair_queue()`）は別判定で動く：

**層 1：`_get_explore_move_policy()`（戦力ベースの方針決定・階段位置を知らない）**
- `suppress_floor_navigation == true` または メンバー空 → `"explore"`（フロア遷移を抑止）
- `target_floor > current_floor` → `"stairs_down"`（戦力 F が次フロアの `FLOOR_X_Y_RANK_THRESHOLD` 以上）
- `target_floor < current_floor` → `"stairs_up"`（HP 充足率不足等で適正フロアより 1 つ上）
- `target_floor == current_floor` → `"explore"`（既に適正フロアにいる）

**層 2：`_generate_stair_queue()`（視界ベースの経路探索・階段位置を視界依存で扱う）**
- リーダーの `_move_policy = "stairs_down"` / `"stairs_up"` を受けて UnitAI 側で実行
- `NPC_KNOWS_STAIRS_LOCATION == false` かつ `ignore_visited == false` のとき：
  - 訪問済みエリア（`_visited_areas`）に含まれる階段のみ候補にする
  - 通路・境界（`area_id` が空のタイル）はマンハッタン 3 タイル以内なら既知扱い
  - 既知階段がない → `_generate_explore_queue()` にフォールバック（探索モード相当の挙動）
  - 既知階段がある → 最近距離の階段へ `move_to_explore`（移動アクションだが実態は階段への直行）
- `ignore_visited == true`（フロア間メンバー追従経路）：視界チェックをスキップして全階段を対象にする

**結果**：層 1 が `"stairs_down"` を返しても、層 2 が「視界に階段なし」と判定すれば実際の挙動は探索になる。`PartyStatusWindow` のメンバー行表示 `move:↓階段` は層 1 の値を映すため、表示と実挙動（move_to_explore）が乖離するケースが構造的に存在する。これは仕様上正しく、未発見階段を探すための行動として整合する。

#### `_visited_areas` の単一管理ルール（2026-04-28 確立）

NPC パーティーの「視界」は `PartyLeader._visited_areas`（[party_leader.gd:34](scripts/party_leader.gd:34)）が唯一の真実。同一 dict が `set_visited_areas()` で全 UnitAI に共有される（パーティー内で値を共有する仕組みは Dictionary が参照型である GDScript の仕様を利用）。

**書き込み**：[unit_ai.gd:730-732](scripts/unit_ai.gd:730) `_generate_queue()` 冒頭で `_member.grid_pos` のエリア ID を毎回追加。`_assign_orders()`（PartyLeader._reeval_timer・1.5 秒間隔）で全メンバーに `receive_order` が発火し、全員の現エリアが反映される。`is_player_controlled` の hero でも `receive_order` 経由で `_generate_queue` が走るため、プレイヤーパーティーの `_visited_areas` も自然に蓄積される。

**読み込み**（同じ dict を参照する 2 経路に統一・2026-04-28）：
1. [unit_ai.gd:_generate_stair_queue()](scripts/unit_ai.gd:938) — 階段候補の絞り込み（既知階段のみ）
2. [unit_ai.gd:_find_explore_target()](scripts/unit_ai.gd:1040) — explore 目的地の未訪問エリア選定

**`VisionSystem.is_area_visited()` との違い**：

| 軸 | `_visited_areas`（パーティー固有） | `VisionSystem.is_area_visited()`（プレイヤー視点） |
|---|---|---|
| 範囲 | 自パで踏んだエリアのみ | プレイヤーパーティーが踏んだエリアのみ |
| 書き込み主体 | 全パーティーが自分の dict を更新 | `_visit_area(PLAYER_PARTY_ID, ...)` のみ |
| 用途 | NPC の AI 判断（階段探索 / explore 目的地） | プレイヤー UI（タイル可視化 / debug_show_all 等） |

旧 `_find_explore_target` は VisionSystem に依存していたが、これは「NPC は他パーティー（プレイヤー）の探索状況に影響される」という不自然な設計だった（自パで何度部屋を踏んでも未訪問扱いされ、プレイヤーがフロアを踏破するまで NPC の `unvisited` 候補リストが減らない）。`_visited_areas` への統一でこの副作用を解消（2026-04-28 改訂）。

### フロア間メンバー追従
- 移動方針が **`cluster` / `follow` / `same_room`** のメンバーは「リーダー追従系」とみなし、リーダーが別フロアに居る場合は `_generate_stair_queue(dir, ignore_visited=true)` で対応する階段を目指す（**追従経路は視界チェックを無視する** = 階段位置既知扱い。リーダーが先行している以上「同じ階段で降りる」前提が成立するため）
...
- NPC パーティーの探索モード時はリーダー個人のみ `_get_explore_move_policy()` の戻り値（`stairs_down` / `stairs_up` / `explore`）を per-member で配布する（`_assign_orders()` 内の分岐）。非リーダーは `cluster` 固定。階段ナビ値（`stairs_*`）は **`_global_orders.move` には書き込まない**（パーティー全体指示ではなくリーダー個人の動的目標として扱う）
- 合流済みパーティー（`joined_to_player=true`）：2026-04-24 改修でメンバーが `_hero_manager` に完全移管される方式に変更されており、独立した「合流済み NPC パーティー」は存在しない。`_generate_floor_follow_queue()` のコードは残存しているが、発動経路（`_follow_hero_floors=true` のセット）が不透明（残タスクで整理予定）
...
- 定数は Character / PartyLeader / NpcLeaderAI / EnemyLeaderAI / UnitAI / SkillExecutor / Effect / Item の各タブに配置。NpcLeaderAI タブは 6 定数（`FLOOR_0_RANK_THRESHOLD`〜`FLOOR_4_RANK_THRESHOLD` / `NPC_POLICY_CHANGE_COOLDOWN`・`FLOOR_RETREAT_RATIO` は 4/28 用語ガバナンス改訂で廃止）。UnitAI タブに FLEE 系 4 定数（`FLEE_THREAT_RANGE` / `FLEE_THREAT_WEIGHT` / `FLEE_AREA_DISTANCE_WEIGHT` / `FLEE_SAFE_DISTANCE`）+ keep_distance 系 3 定数（`KEEP_DISTANCE_MIN` / `KEEP_DISTANCE_MAX` / `KEEP_DISTANCE_REACH_WEIGHT`）が登録済み（2026-04-25 のリーダー推奨機構廃止に伴い `FLEE_NON_RECOMMENDED_PENALTY` / `FLEE_REEVAL_MIN_INTERVAL` を削除し、同日に前進不要判定用の `FLEE_SAFE_DISTANCE` と rear 隊形用の keep_distance 3 定数を追加）。未登録の定数が見つかった場合は運用ルール 1〜5 に従って追加する
...
{
  "sword": [
    { "name": "兵士の剣", "stats": { "power": 10, "block_right_front": 10 }, "bonuses": { "power": 1, "block_right_front": 1 } },
    { "name": "断罪の魔剣", "stats": { "power": 30, "block_right_front": 10 }, "bonuses": { "power": 3, "block_right_front": 1 } },
    ...
  ]
}
```

- **総当たり 9 パターン + 無 bonus の 1 パターン**：各装備タイプは 2 ステータスを low/mid/high の 3 bonus 段階で組み合わせた 9 パターン ＋ 初期装備用の全 bonus=0（none）エントリ 1 個。盾のみ単一ステータスで 3 + 1 パターン
- **bonus / tier の概念分離**（2026-04-19 導入・2026-04-27 整理）：
  - **bonus 段階**（一次情報）：generated JSON の `bonuses` フィールドに各ステータスごとの 0=無 / 1=小 / 2=中 / 3=大 を保存。`stats` 値は表示・実効補正用、`bonuses` は格付け比較用
  - **tier**（派生値）：`ItemGenerator.calc_tier(item)` が `bonuses` から `ITEM_TIER_POLICY` に従って導出（既定 "max"）。**ドロップ重み付けでのみ使用**。戦力計算・装備比較では使わない
- **bonus 値の計算**：`stats[stat] = _max × ITEM_BONUS_{LOW|MID|HIGH}_RATIO`（デフォルト 0.33 / 0.67 / 1.0）。逆に bonuses[stat] は `round(value / _max × 3)` で復元可能
- **tier 0（none）は初期装備専用**：全 bonus が 0。ドロップには出現しない（`ItemGenerator.generate()` の重み計算で除外）。`ItemGenerator.generate_initial(item_type)` 経由で取得
- **ランダム補正なし**：同じ名前の装備は常に同じ stats / bonuses（プレイヤーが「鋭利な剣」を見つけたら値も固定）
...
- `ItemGenerator.generate(item_type, floor_index)` が tier≥1 の全エントリから重み付き選択（ドロップ用）。tier は `calc_tier(entry)` で `bonuses` から導出
- `ItemGenerator.generate_initial(item_type)` が全 bonus=0 のエントリを返す（初期装備用・ポーションは `generate_consumable` に委譲）
...
- **装備アイテム戻り値に `bonuses` を含む**（2026-04-27〜）：`{ item_type, category, item_name, stats, bonuses }`。戦力計算（`party_leader._character_bonus_sum_avg()`）と NPC 自動装備比較（`npc_leader_ai._item_bonus_sum()`）が参照する。tier は派生値（`calc_tier()`）でドロップ重み付け専用。ポーションは bonuses を持たない（戦力評価対象外のため）
...
### アイテムのドロップ（部屋制圧方式）
- 部屋に配置された enemy_party の全員が**死亡 or 個体 FLEE で部屋外へ離脱**したら「制圧完了」
- 制圧時、敵パーティーの所持アイテムが**部屋の床タイルにランダムに散らばる**（1マスに1個）
- 出現は1回きり（敵が戻っても再出現しない）
- **設計意図**：「敵を撃退すれば報酬が得られる」前提を成立させ、ポーション収集 → 装備強化サイクルの基盤とする。プレイヤーが敵を誘導して部屋外に出しただけ・敵が追跡で部屋外に出ただけではドロップしない（戦闘ゼロでアイテムが手に入る抜け道を防ぐ）
- **制圧判定の詳細**（`party_manager._check_room_suppression()`・2026-04-27 改訂）：
  - 死亡 → 制圧対象
  - 縄張り部屋（`_room_id`・初代スポーン位置の area_id）の中で生存 → 制圧対象にしない
  - 部屋の外にいる かつ **個体 FLEE**（`UnitAI._strategy == 1`）→ 離脱扱い・制圧対象
  - 部屋の外にいる かつ ATTACK / WAIT / FALL_BACK → 制圧対象にしない（追跡・誘導・後退いずれも戦闘継続中扱い）
  - 判定タイミング 2 系統：
    - メンバー死亡時（`_on_member_died` 経由）
    - メンバーがエリア変化したとき（`_process` のポーリング検知経由・`_per_member_last_area` 辞書で追跡）
  - 二重発火防止：`party_wiped` 発火後に `_room_id` を空文字にクリア
- 個体 FLEE は `_determine_effective_action()` の戻り値 1 を `_strategy` に保存したもの（`_party_fleeing` / `_should_self_flee` / `on_low_hp == "flee"` + critical / `combat == "flee"` の 4 経路を網羅）。FALL_BACK（戦闘継続後退）は含めない
...
#### ドロップ組成の二系統（2026-04-27 改訂）
ドロップは**装備系（JSON 由来）**と**ポーション系（ランタイム生成）**の 2 系統に分離。
NPC が「装備で戦力強化・ポーションで消耗戦をサポート」というサイクルを成立させるため。

**装備系**（JSON 由来）：
- `assets/master/maps/dungeon_handcrafted.json` の各 `enemy_party.items` 配列に item_type を直書き
- ルール：1 パーティーにつき装備 0〜1 個。0 個のパーティーは「ポーションのみ落とす小戦闘」相当（15 件・全 87 戦闘パーティー中）
- ItemGenerator.generate(item_type, floor_idx) で stats 付きアイテムに展開（既存仕様）

**ポーション系**（ランタイム生成）：
- `_on_enemy_party_wiped`（game_map.gd）が制圧時に動的生成
- 個数 = `[POTION_DROP_MIN_PER_PARTY, POTION_DROP_MAX_PER_PARTY]` の一様乱数（既定 1〜2 個）
- 種別 = 各ポーション 1 個ごとに `POTION_DROP_HEAL_RATIO`（既定 0.5）で heal/energy を判定
- ItemGenerator.generate_consumable(pot_type) で生成
- 3 定数とも ConfigEditor Item カテゴリで調整可能
...
### 戦力計算の 3 指標と用途（2026-04-28 整理・同日後に用語ガバナンス改訂）

`_evaluate_strategic_status()` の出力は 3 つの異なる指標として消費される。それぞれ計算式・用途・PartyStatusWindow ヘッダー上のラベルが異なる：

| 指標 | ヘッダーラベル | 計算式 | 主な用途 |
|------|-------------|--------|---------|
| **戦況**（CombatSituation） | `戦況:` | `(my_strength × my_hp_ratio) / (enemy_strength × enemy_hp_ratio)` → 6 段階 enum 分類 | NPC battle_policy 自動切替（CRITICAL → retreat / SAFE → attack）・特殊攻撃「劣勢なら使う」発動・アイテム取得許可（SAFE 限定） |
| **戦力**（strength） | `戦力:` / `目標F:`（NPC のみ）| `rank_sum + bonus_sum × ITEM_BONUS_STRENGTH_WEIGHT`（HP 抜き・装備込み・実数値） | NPC フロア降下判定（[npc_leader_ai.gd:159](scripts/npc_leader_ai.gd:159) `_get_target_floor()` が `full_party_strength` を `FLOOR_X_Y_RANK_THRESHOLD` と直接比較・ラダー判定。判定結果は PartyStatusWindow ヘッダーの `目標F:N` でも可視化） |
| **戦力比**（PowerBalance） | `戦力比:` | `my_rank_sum / enemy_rank_sum`（rank_sum のみ・HP/装備無視）→ 5 段階 enum 分類 | 特殊攻撃「強敵なら使う」発動判定（`POWER_BALANCE_INFERIOR` 以下） |

**設計意図**：
- 「戦況」は **戦力 × HP 率** の比なので「今この瞬間の力関係」を表す。リアルタイムな撤退判断に使う
- 「戦力」は HP 抜きなので装備とメンバー数で決まる安定した中長期指標。閾値定数との直接比較で「このパーティーがどのフロアに居られるか」を判定する（ラダー判定：戦力 ≥ 閾値の最上階を採用）
- 「戦力比」は HP/装備を抜いた素のランク差なので「相手の素の格」を表す。HP が低くても格上には特殊攻撃を切る、といった判断に使う

**用語間の関係**（用語ガバナンスのルール 2「用語間の関係を素直に成立させる」）：
- 戦力（HP 抜き）と戦力比（rank_sum のみ）はどちらも瞬間的 HP 状態を含まない安定指標。「絶対値・装備込み or 抜き」の違いのみで重複しない関係
- 戦況（瞬間的力関係）は戦力に HP 率を乗算した比として独立に位置付け。用語の流用ではなく別概念
- 旧仕様（〜2026-04-28 前半）は戦力に HP 率を含めていたため「戦力比 = 戦力 / 敵戦力」と読めない非対称があったが、4/28 後半の HP 抜き化で解消

**用語衝突に注意**：
- 旧仕様（〜2026-04-27）は PartyStatusWindow ヘッダーで「戦力:」ラベルが PowerBalance enum と strength 内訳を同居させていた（用語衝突）
- 2026-04-28 改訂で `戦力:`（strength 内訳）と `戦力比:`（PowerBalance enum）に分離
- ドキュメント上は **戦況比 = CombatSituation の根拠となる比**（戦力 × HP 率）、**戦力比 = PowerBalance enum**（rank_sum のみ）と区別する
...
- 戦力式：`rank_sum + bonus_sum × ITEM_BONUS_STRENGTH_WEIGHT`（HP 抜き・4/28 用語ガバナンス改訂）
- 戦況比 = `(my_strength × my_hp_ratio) / (enemy_strength × enemy_hp_ratio)`（戦況計算で別途 × HP 率を適用）
  - `bonus_sum` = メンバーごとの装備 bonus 段階合計の平均（`_character_bonus_sum_avg`）をパーティー全員で合計
  - `ITEM_BONUS_STRENGTH_WEIGHT`：Config Editor PartyLeader カテゴリ・デフォルト 0.165（旧 `ITEM_TIER_STRENGTH_WEIGHT=0.33` × tier 平均 2 と等価）
- HP 率計算：
  - 自パ部分：実 HP + ヒールポーション回復量
  - 同陣営他パ部分：condition ラベルから推定（他パのポーション所持は把握不可）
  - 敵部分：condition ラベルから推定（敵ステータス直接参照禁止ルール）
- HP 率の `nearby_allied_hp_ratio` / `full_party_hp_ratio` は**自軍ベースで計算**。`nearby_enemy_hp_ratio` は敵 condition ラベルから推定（敵ステータス直接参照禁止ルール準拠）

| 戦況 | 戦況比（HP 込み戦力比） | NPC行動への影響 | アイテム拾い |
|------|------------------------|----------------|-------------|
...
2026-04-25 に `HpStatus` enum（FULL/STABLE/LOW/CRITICAL）を廃止。HP 関連表現は以下に整理：

- **個体レベル**：`Character.get_condition()` の段階ラベル（healthy / wounded / injured / critical・閾値 50%/35%/25%）。AI 判断の HP 基準は全てこのラベル経由（後述「AI 行動判定における状態ラベル経由の原則」）
- **パーティーレベル**：`_combat_situation` 辞書の生 float 値を直接参照
  - `nearby_allied_hp_ratio` — 自パ近接 + 同陣営他パ近接の平均 HP 充足率
  - `nearby_enemy_hp_ratio` — 近接敵の平均 HP 充足率（推定）
  - `full_party_hp_ratio` — 自パ全員の平均 HP 充足率（NPC フロア降下判定用）
- **段階表現の専用閾値**：必要な行動判定ごとに float 定数で持つ（旧 enum 段階比較を廃止）
  - `SPECIAL_SKILL_DISADVANTAGE_HP_RATIO`（0.5）：特殊攻撃「劣勢なら使う」発動閾値
  - `NPC_FLOOR_DOWNGRADE_HP_RATIO`（0.5）：NPC フロア降下判定の閾値

PartyStatusWindow は HP 独立表示を廃止し、戦況比の括弧内に HP 率を組み込む構造化表示に変更（後述「戦況表示の構造化」）。
...
| 劣勢なら使う | `nearby_allied_hp_ratio ≤ SPECIAL_SKILL_DISADVANTAGE_HP_RATIO`（既定 0.5）かつ MP/SP が足りている |
| 使わない | 使わない |
...
#### 戦況系
本セクション上部の「戦況判断（CombatSituation）」「戦力比（PowerBalance）」表の閾値は、それぞれ以下の定数で定義されている:
...
- HP 率は段階 enum を持たず生 float（`nearby_allied_hp_ratio` 等）で扱う。判断用閾値は用途ごとに専用定数（`SPECIAL_SKILL_DISADVANTAGE_HP_RATIO` / `NPC_FLOOR_DOWNGRADE_HP_RATIO` 等）
...
### 敵の縄張りシステム（2026-04-26 改訂）

敵パーティーの「縄張り」は **部屋単位（area_id）** で表現される。パーティーリーダーの初期スポーン位置の area_id を `_home_area_id` として `EnemyLeaderAI.setup()` で保持・固定する（リーダー交代時も維持・縄張りはパーティー単位で固定）。

#### GUARD_ROOM の発火条件

`EnemyLeaderAI._decide_leader_move_override()` がリーダーの判断結果として以下を全て満たすと `"guard_room"` を返し、`PartyLeader._assign_orders()` でリーダーの `_move_policy = "guard_room"` に上書きされる：

1. リーダーが `_home_area_id`（縄張り部屋）にいる
2. 縄張り部屋内に敵（friendly）がいない
3. リーダーから `chase_range`（マンハッタン距離）内に敵がいない

非リーダーは既存通り `_global_orders.move`（敵は `"follow"` 固定）を継承する（リーダー追従）。リーダーが部屋を守れば自然と部屋に集まる。リーダーが敵を追えば非リーダーも追従する。

#### chase_range の役割

リーダーの視認範囲（マンハッタン距離・同フロア）。縄張り外でもこの範囲内に敵がいれば追跡対象とする（`_decide_leader_move_override()` の判定 3）。壁越し判定は当面行わない（マンハッタン距離のみ・将来「壁越し攻撃の禁止」と一緒に対応）。

#### territory_range は実質デッドコード

旧実装は `territory_range` の数値半径による縄張り判定を `_apply_range_check()` で行っていたが、area_id ベースの本実装に置き換えられた。`territory_range` および `_apply_range_check()` 関連メソッドは将来の `_party_strategy` 廃止と一緒に削除予定（残タスク参照）。

#### `_party_strategy.GUARD_ROOM` との関係

`_decide_leader_move_override()` は `_party_strategy` を経由しない独立経路。`_is_in_guard_room_mode()` フック（`_party_strategy == GUARD_ROOM` を判定）も基底に残っているが、`_decide_leader_move_override()` のフォールスルー先として残置される（敵では現状 `_party_strategy.GUARD_ROOM` への遷移が起きないため、結果的に発火しない）。将来の `_party_strategy` 廃止と一緒に整理する。
...
直近では取り組まないが、長期的に検討したい大型機能・ゲームデザイン上の構想。
着手判断が出た時点で「残タスク」へ移動する。
