...
##### アイテム取得サイクル動作観察（4/27 改修後）

4/27 改修（item_pickup aggressive 化・ドロップ組成「装備 1 + ポーション 1〜2」分離）の実プレイ観察。実プレイで動作異常なし・NPC が能動的にアイテム取得することは初回確認済み。複数プレイ重ねて以下を観察：

- ヒーラー不在パーティーのサイクル成立（目標階苦戦 → 退避 → 装備・ポーション収集 → 再挑戦）
- ポーション 1〜2 個ドロップが「足踏み（再挑戦のためのリソース蓄積）」サポートとして十分か
- NPC が上階に長く滞在しすぎないか（過剰退避）
- 装備強化による戦力底上げが実際に観察できるか（`F(R+B)s` 表示で確認）
- 4/26 改修で「敵が部屋に留まる」「NPC が敵を連れ回さない」が解消されているか
- 気になる挙動があれば随時議論項目に追加

調整候補：`POTION_DROP_MIN/MAX_PER_PARTY`・`POTION_DROP_HEAL_RATIO`・`ITEM_BONUS_STRENGTH_WEIGHT`・`FLOOR_*_RANK_THRESHOLD`・`NPC_FLOOR_DOWNGRADE_HP_RATIO`。
