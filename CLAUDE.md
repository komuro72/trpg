...
- **PartyStatusWindow 戦力表示の整理（戦況/戦力/戦力比 の 3 指標を分離）**：旧 `戦力:OVERWHELMING F(14+0.7)14.1 C(13+0.7)13.1 E(3+0.0)3.0` は (a) 表示の B 部分が WEIGHT 適用前の bonus_sum 素値で `R+B ≠ s` に見える、(b) PowerBalance enum と strength 内訳が同じ「戦力:」ラベル下に同居して用語衝突、の 2 つの問題があった。新表示は `戦力: F=14.1(14+0.1) C=13.1(13+0.1) E=3.0(3+0.0)  戦力比:OVERWHELMING` で、(a) `F=s(R+B)` の B を `bonus_sum × ITEM_BONUS_STRENGTH_WEIGHT`（WEIGHT 込み寄与値）に変更し HP 率 1.0 のとき `R+B = s` が成立、(b) PowerBalance を末尾の `戦力比:` 独立ラベルに分離。3 指標（CombatSituation = 戦況 / strength = 戦力 / PowerBalance = 戦力比）の用途は「戦力計算の 3 指標と用途」セクション参照（4/28）
...
- **Floor 0 拡張（20 → 30 部屋・4/28）後の探索完遂時間・敵殲滅率・適正フロアサイクル成立度**
...
4/28 の Floor 0 拡張（20 → 30 部屋）は Floor 0 のみ対象。Floor 1〜3 は 20 部屋のまま。プレイヤー / NPC のフロア遷移は `find_stairs(spawn_type)` の **`spawns[0]` を一律使用**（[game_map.gd:2036](scripts/game_map.gd:2036)）するため、Floor 1+ の階段数と Floor 0 の階段数は独立に動作することを確認済み（接続不整合なし）。ただし「F0 が広く F1 が狭い」となるとサイクル後半で探索密度が偏る可能性がある。実プレイで観察し、必要なら Floor 1〜3 も同様に拡張する。Floor 4（ボス部屋）は拡張しない（仕様）。

##### VisionSystem の別フロアスキップ（Floor 0 拡張に伴う性能最適化候補）

[vision_system.gd:138](scripts/vision_system.gd:138) `_process` が毎フレーム全フロア合算の `_enemy_managers` / `_npc_managers` に対して `update_visibility` を呼び出している。`em.get_floor() != _current_floor_index` で早期 continue を入れれば走査量が **1/5 に削減**できる（5 フロア合算 → 現フロアのみ）。Floor 0 を 30 部屋に拡張した今、最も効果が出やすい改修。`_per_floor_enemy_managers` への置き換えで実装可能。
...
4/26 の NPC 構成整理（1 人 P × 4 + 2 人 P × 2 + 3 人 P × 2）を前提に、フロア降下判定（`FLOOR_*_RANK_THRESHOLD` / `FLOOR_RETREAT_RATIO` / 装備 tier 反映）を見直す。3 人 P パターン A（近遠癒）は鉄板構成だが C ランクばかりだと B1F でも苦戦する実例（`snapshot_20260426_162348_136.log` で観測：ノエルパーティーが CRITICAL）が既に観測されている。実プレイで「適正フロア = 戦力閾値」と「実際の戦闘テンポ」のキャリブレーションが必要。

調整時の観察ツールとして PartyStatusWindow ヘッダーの NPC 行に `目標F:N` 表示が利用可能（4/28 追加・優先度「高」）。`戦力: F=s(R+B)` の `s` と `目標F:` を並べて見れば閾値判定の挙動が直接観察できる。

Floor 0 拡張後の観察項目（4/28）：30 部屋・5 階段化により NPC が階段を発見するまでの探索時間が伸びている。サイクル成立確認（弱 NPC が F0 で装備を集めて強くなる → F1 へ降りる → 苦戦したら F0 に戻る）の挙動を実プレイで継続観察。`F=s(R+B)` の strength `s` が `FLOOR_1_RANK_THRESHOLD`（既定 13）に届くまでの時間、`目標F:0/1` の切り替わり頻度などを追跡する。

派生課題（命名乖離）：`FLOOR_X_Y_RANK_THRESHOLD` の定数名は「ランク和の閾値」を示唆するが、実態は **weighted strength**（`rank_sum + bonus_sum × ITEM_BONUS_STRENGTH_WEIGHT × HP率`）との比較（[npc_leader_ai.gd:159](scripts/npc_leader_ai.gd:159) `_evaluate_target_floor()` 経路）。`_STRENGTH_THRESHOLD` への改名か、計算ルールを命名に合わせて純粋ランク和比較に戻すか、本タスクと一緒に判断する（4/28 追記）。
...
FLEE システムは 4/21 ステップ 1〜3 + 4/24 fall_back 実装 + 4/25 出口外側化 + 4/25 リーダー推奨機構廃止（自律判断統一）+ 4/25 rear 隊形を keep_distance ベースに変更（後衛前進問題解消）まで完了。残課題：
1. **敵パーティーへの FLEE 自律ロジック適用**：現状 `_find_flee_goal_legacy`（脅威から 5 タイル離れる）のまま。味方と同じ `_evaluate_exit_costs()` 経由に切り替え。避難先は敵種族ごとに異なる可能性（縄張り戻り・階段逃走）の設計検討必要
...
`FLOOR_*_RANK_THRESHOLD` / `FLOOR_RETREAT_RATIO` の調整。装備 bonus 戦力反映（4/27 bonus_sum 化）により同じ基準値でも降下しやすくなっている。PartyStatusWindow の `戦力: F=s(R+B)` の `s` と `目標F:N`（4/28 追加）を並べて観察しながら調整できる。Floor 0 拡張（4/28・20→30 部屋）に伴いサイクル所要時間が変わる可能性があるため、合わせて再評価する。
...
##### NPC が行く手をはばむ問題（以前からの課題・継続観察中）

プレイヤーや NPC リーダーが移動しようとした際、進路上に他の味方キャラが立ち止まっており通行できない現象が散発する。過去に対症療法として「A* 1 歩目の味方迂回フォールバック」（Phase 関連の修正）が入っているが、根本解決していない。

候補となる原因と対処方針：
- 立ち止まっている側が `wait` 状態（`_formation_satisfied` 等）で動く動機がない場合、通行待ちのキャラが永続的にブロックされる
- 「通せんぼ」を検知して、立ち止まっている側を一時的に避けさせる仕組みが必要かもしれない
- swap（位置入れ替え）の導入も選択肢

「NPC スタック問題（複数メンバーが固まって動かない）」とは別現象。本タスクは「他のキャラの動線を塞いでいる」現象で、当事者本人は問題なく動ける状態が多い。
...
- **味方（プレイヤー・NPC）**：`[種別]  生存:x/y  area:<id>  戦況:<ENUM>(自軍×自軍HP率 / 敵×敵HP率 = 戦況比) 戦力: F=s(R+B) C=s(R+B) E=s(R+B) 戦力比:<ENUM> [目標F:N(NPCのみ・高)]  [mv=... battle=... tgt=... hp=... item=...](中以上)`
  ...
  - `戦力:` の `F=s(R+B)` 内訳：F=full_party / C=nearby_allied / E=nearby_enemy。s=strength（小数 1 桁）、R=rank_sum（素値）、B=`bonus_sum × ITEM_BONUS_STRENGTH_WEIGHT`（**WEIGHT 込みの strength 寄与値・小数 1 桁**）。HP 率 1.0 のとき `R + B = s` が成立し、検算が見た目に通る設計
  - 戦況の括弧内：`<自軍戦力 base>×<自軍 HP 率> / <敵戦力 base>×<敵 HP 率> = <戦況比>` の形式で計算過程を完全可視化
...
- `NpcLeaderAI._decide_leader_move_override()` → **戦闘中のみ `"explore"`**
  - 探索中（敵未検知・`_is_in_explore_mode() == true`）：`""` を返して既存の `_get_explore_move_policy()` 経路（`stairs_*` / `explore`）に委ねる
  - 戦闘中（敵検知中・`_is_in_explore_mode() == false`）：`"explore"` でフォールバックし、層 1 `"follow"` の退化を防ぐ
...
- `_evaluate_strategic_status()`: 統合戦略評価ルーチン。戦力計算と戦況判断を 1 箇所に集約し、3 種類のメンバー集合で 1 度ずつ統計を算出する（重複計算なし）：
  ...
  - 戦力 = `(rank_sum + bonus_sum × ITEM_BONUS_STRENGTH_WEIGHT) × 平均HP充足率`
...
- 定数は Character / PartyLeader / NpcLeaderAI / EnemyLeaderAI / UnitAI / SkillExecutor / Effect / Item の各タブに配置。NpcLeaderAI タブは 7 定数（`FLOOR_0_RANK_THRESHOLD`〜`FLOOR_4_RANK_THRESHOLD` / `FLOOR_RETREAT_RATIO` / `NPC_POLICY_CHANGE_COOLDOWN`）。UnitAI タブに FLEE 系 4 定数（`FLEE_THREAT_RANGE` / `FLEE_THREAT_WEIGHT` / `FLEE_AREA_DISTANCE_WEIGHT` / `FLEE_SAFE_DISTANCE`）+ keep_distance 系 3 定数（`KEEP_DISTANCE_MIN` / `KEEP_DISTANCE_MAX` / `KEEP_DISTANCE_REACH_WEIGHT`）が登録済み（2026-04-25 のリーダー推奨機構廃止に伴い `FLEE_NON_RECOMMENDED_PENALTY` / `FLEE_REEVAL_MIN_INTERVAL` を削除し、同日に前進不要判定用の `FLEE_SAFE_DISTANCE` と rear 隊形用の keep_distance 3 定数を追加）。未登録の定数が見つかった場合は運用ルール 1〜5 に従って追加する
...
### 戦力計算の 3 指標と用途（2026-04-28 整理）

`_evaluate_strategic_status()` の出力は 3 つの異なる指標として消費される。それぞれ計算式・用途・PartyStatusWindow ヘッダー上のラベルが異なる：

| 指標 | ヘッダーラベル | 計算式 | 主な用途 |
|------|-------------|--------|---------|
| **戦況**（CombatSituation） | `戦況:` | `my_combat_strength / nearby_enemy_strength`（HP 込み戦力比）→ 6 段階 enum 分類 | NPC battle_policy 自動切替（CRITICAL → retreat / SAFE → attack）・特殊攻撃「劣勢なら使う」発動・アイテム取得許可（SAFE 限定） |
| **戦力**（strength） | `戦力:` / `目標F:`（NPC のみ）| `(rank_sum + bonus_sum × ITEM_BONUS_STRENGTH_WEIGHT) × HP率`（実数値） | NPC フロア降下判定（[npc_leader_ai.gd:159](scripts/npc_leader_ai.gd:159) `_get_target_floor()` が `full_party_strength` を `FLOOR_X_Y_RANK_THRESHOLD` と直接比較。判定結果は PartyStatusWindow ヘッダーの `目標F:N` でも可視化） |
...
**設計意図**：
- 「戦況」は HP 込みなので「今この瞬間の力関係」を表す。リアルタイムな撤退判断に使う
- 「戦力」は絶対値（HP 込みだが ENUM ではなく実数）なので「このパーティーがどのフロアに居られるか」を表す。閾値定数との直接比較に使う
...
- ドキュメント上は **戦況比 = CombatSituation の根拠となる比**（HP 込み）、**戦力比 = PowerBalance enum**（rank_sum のみ）と区別する
...
- 戦力式：`(rank_sum + bonus_sum × ITEM_BONUS_STRENGTH_WEIGHT) × 平均HP充足率`
