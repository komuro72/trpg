...
FLEE システムは 4/21 ステップ 1〜3 + 4/24 fall_back 実装 + 4/25 出口外側化 + 4/25 リーダー推奨機構廃止（自律判断統一）まで完了。残課題：
...
4. **`keep_distance` を `_evaluate_exit_costs()` 経由に切り替え**：現状は旧 `_find_flee_goal()` を流用。距離確保の意味（射程維持・後退ではなく脇に退避）に合わせ、`attack_range` 参照のロジック差別化を加える。派生課題 3 と合わせて対応
5. **戦況判断と避難先計算の統合**：`_evaluate_strategic_status()` と `_update_flee_refuge_area_id()` を統合し、避難先情報を `combat_situation` 辞書に含める設計検討
...
旧 `NPC_HP_THRESHOLD` / `NPC_ENERGY_THRESHOLD` 廃止により、現在は HpStatus のパーティー平均HP率で判定している。元の「最低HP率」「エネルギー率」ベースの判定は NPC 行動として意味があるため、戦況判断（`_evaluate_strategic_status()`）の副情報として最低HP指標・エネルギー指標を追加することを検討。別系統の判定を走らせず、戦況判断に一元化する設計方針を維持する。
...
#### keep_distance との関係（派生課題）

GoblinArcher / Salamander 用の `keep_distance` も将来的に `_evaluate_exit_costs()` 経由に切り替える予定（FLEE 派生課題）。現状は旧 `_find_flee_goal()` 同等のままで動作。
...
- **実行ロジック**（2026-04-25 改訂・自律判断統一後）：
  ...
  - `keep_distance` → `_find_flee_goal()`（ゴブリンアーチャー等の現挙動維持。将来 `_evaluate_exit_costs()` 経由に切り替える派生課題）
...
- 低詳細度の UnitAI フラグ行に `fb→(x,y)` / `flee→(x,y)`（メンバー本人の `_last_flee_goal`・味方のみ）を表示。現在のキューアクションでラベルが切り替わる
