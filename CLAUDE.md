...
#### 2026-04-21（本日の成果：FLEE システム大改訂 + PartyStatusWindow 統合 + Character ステータス統一）
...
2. CLAUDE.md「次セッションで検討するタスク」→「最優先：Phase 14 バランス調整の実プレイ検証」「FLEE 実装の完了状況 / FLEE 派生課題」
3. CLAUDE.md「要調査・要整理項目」→「優先度別インデックス」で FLEE 以外の未完了タスクを俯瞰
4. `docs/history.md` の 2026-04-21 / 2026-04-20 / 2026-04-19 エントリ群で直近の経緯
...
#### 🔥 最優先：Phase 14 バランス調整の実プレイ検証

2026-04-21 の FLEE 関連実装（ステップ 1〜3 + アクション分離 + 避難先表示）により、「NPC がダメージを受けたら安全部屋に逃げ戻る」機構が**一通り完成**した。当初目的だった「味方が 2〜3 発で死なない」「ダメージを受けたら逃げ戻る」のうち、後者は機構として整備された状態。次は**実プレイでの検証**を行い、必要に応じて数値調整する段階に入る。

**検証項目**：

1. NPC が戦況 CRITICAL で実際に逃げ戻るか（`[NPC戦況判断]` ログの出現・F7 スナップショットでの `battle_policy = retreat` 確認）
2. 単独 NPC が死亡前に安全部屋に退避できるか（脅威コストでうまく回避経路を選べるか）
3. プレイヤー共闘で敵に勝ち、仲間に加える流れが成立するか
4. **耐久力不足（2〜3 発で死ぬ）の問題が残っているか**（これは FLEE 機構では解消できない・HP ベース値 / ダメージ倍率 / 耐性の調整が必要）
5. 推奨出口 `避難先:xxx(d:n)@(x,y)` 表示が実挙動と一致しているか（フォールバック `!fb` が多発する場合はマップ設計 or 実装不備の疑い）

**観察結果次第で調整する候補**：

- **数値調整**：HP ベース値 / `damage_mult`（攻撃タイプ別倍率）/ 耐性（`physical_resistance` / `magic_resistance` の `score/(score+100)` カーブ）
...
4. **個体アクションの実行ロジック差別化**
   ...
   - 目標の差別化：
     ...
     - `flee`：安全な場所へ完全離脱（ステップ 3 の避難先情報を使う）
...
- **個体 FLEE**（`on_low_hp = "flee"` + HP < `NEAR_DEATH_THRESHOLD` でパーティーは平常時）：`_flee_recommended_goal == Vector2i(-1, -1)` で配布 → メンバーはバイアスなしで自律判断
