...
階段マスにキャラが立ち止まり、別のキャラのフロア遷移をブロックする現象。実プレイで以下を観測：
- **NPC 版**：味方系メンバーが follow 隊形で偶然階段マスに立ち止まり、リーダーの階段使用をブロック
- **敵版**：敵が階段マス上に立ち止まり、プレイヤーや NPC のフロア遷移をブロック（プレイヤーがフロア遷移できなくなる事例観測・2026-04-28）

修正方針候補：
- **修正 X**：階段マスを goal 候補から除外（通過は OK・最終目的地として選ばれない）。ただし NPC が長居すると永久ブロックの懸念あり → タイムアウトや待機表示が必要
- 階段マス占有時はフロア遷移を保留・隣接マスへの「横ずらし」（[`_find_free_adjacent_to()`](scripts/game_map.gd:1951)）を廃止 → 階段着地時の重なり問題（後述）と統合検討

関連：「敵が階段を能動的に使う仕組み」（後述「敵のフロア間移動の設計検討」）が未実装の点と密接に関連。階段マス除外を採用する場合、敵が階段を使う経路の整備とセットで検討する必要がある。
...
フロア遷移後、対応する階段マス（spawns[0]）に既存キャラ（NPC・敵・味方メンバー）がいると、`_find_free_adjacent_to()`（[game_map.gd:1951](scripts/game_map.gd:1951)）で隣接マスへフォールバック → そこに別キャラがいると視覚的に重なる現象を観測（2026-04-28）。

修正方針候補：
- **修正 X**：「占有時は下りない（待機）」+ 「横にずらす仕様廃止」。階段マス占有問題（上記）と統合検討
- 副作用：NPC が長居すると永久ブロック → タイムアウトや待機表示と組み合わせる必要あり

階段マス占有問題（NPC 版・敵版）と密接に関連するため、本タスクは独立解決ではなく統合タスクとして扱う候補。
