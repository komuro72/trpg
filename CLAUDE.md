...
2026-04-21 に「味方側 `_party_strategy` / `party_fleeing` 廃止（ステップ 1）」「NpcLeaderAI CRITICAL 時 `battle_policy` 自動書き換え（ステップ 2）」を完了。残り 1 ステップと派生課題:
...
3. **ステップ 3：FLEE 逃走先決定ロジック（`flee_recommended_goal` キー新設）**（次の最優先タスク）
   - 脅威コスト付き A* + エリア BFS の 2 段階ロジック
   - 詳細仕様は本セッションで議論済み（脅威距離・避難先エリア・出口タイル評価）
   - ステップ 2 完了により FLEE の発火経路が整った（味方: battle_policy=retreat / 敵: `_party_strategy=FLEE`）ので、次は脅威からの逃走先を賢く決める段階
...
- 定数は Character / PartyLeader / NpcLeaderAI / EnemyLeaderAI / UnitAI / SkillExecutor / Effect / Item の各タブに配置。NpcLeaderAI タブは 7 定数（`FLOOR_0_RANK_THRESHOLD`〜`FLOOR_4_RANK_THRESHOLD` / `FLOOR_RETREAT_RATIO` / `NPC_POLICY_CHANGE_COOLDOWN`）。未登録の定数が見つかった場合は運用ルール 1〜5 に従って追加する
